  ...
  function runChecks(db, year) {
    ...
    if (byDate.n !== byStatus.n) {
      ...
      /* ЧЕТВЪРТАТА ПРИЧИНА: ОТЧИСЛЕН С АКТ, НО „НАЛИЧЕН“ ПО СТАТУС (v2.4.61).
         Огледалният случай на orphan по-горе и също толкова възможен: редът има
         дата на отчисляване (и акт), но статусът му е „наличен“ / „липсващ“ /
         „изгубен“. Ключът „регистър“ НЕ го брои (гледа deaccession_date), ключът
         „налично днес“ ГО брои (гледа статуса) — тоест таблото показва един
         документ повече от КДБФ, при това документ, който вече е отчислен с
         подписан акт и стои в Приложение № 3.
         Дотук проверката не познаваше този случай и когато той беше ЕДИНСТВЕНАТА
         причина за разликата, находката излизаше като „бележка“ с обяснението
         „Разлика се появява при документи, вписани след 31.12.Y“ — изречение,
         което е просто НЕВЯРНО за този ред и праща библиотекаря да търси дати,
         вместо да анулира или да довърши акта. Затова причината се брои поименно
         и находката става „важно“: подписаният акт и живият фонд си противоречат.
         Самото създаване на такъв ред вече е затворено (books:update отказва да
      ...
      add({
        ...
        todo: bad.n
          ...
          : (orphan.n ? 'Съставете акт за отчислените без акт („Проверка на данните“).'
            : (backInFund.n
              ? 'Документ с дата на отчисляване, но с обратно върнато състояние, се среща само след ръчна '
                + 'намеса или внос. Ако документът наистина се връща във фонда, анулирайте акта от '
                + '„Отчисляване“ — тогава отпадат и датата, и редът в Приложение № 3. Ако актът е верен, '
                + 'върнете състоянието „отчислен“.'
              : ''))
      });
    }
