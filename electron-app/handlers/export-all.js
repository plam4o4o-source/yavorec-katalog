...
module.exports = function registerExportAllHandlers(ipcMain, deps) {
  ...
  /* ОДИТНАТА СЛЕДА Е ВТОРОТО МЯСТО, НА КОЕТО ЖИВЕЯТ АДРЕСЪТ И ТЕЛЕФОНЪТ
     (v2.4.65, кръг 42, находка А8).
     ==========================================================================
     (а) КАКВО СТАВАШЕ ДОТУК. Списъците по-долу знаеха само за `readers`,
     `housebound_*` и `acquisitions`. При ЗАКЛЮЧЕНА защита `chitateli.csv`
     излизаше както трябва — „(скрито)“ на мястото на адреса, телефона и
     имейла — но в СЪЩИЯ архив `odit-sleda.csv` даваше ред
         "Редакция на читател";"карта 777 — Иван Петров Стоянов";
         "[{""field"":""address"",""before"":""с. Яворец, ул. Стара планина 3"",
           ""after"":""с. Яворец, ул. Нова 7""}]"
     тоест СТАРИЯ И НОВИЯ адрес и телефон на всеки читател, чийто картон някога
     е поправян, в чист текст. А ПРОЧЕТИ-МЕ.txt в същия архив твърдеше
     обратното: „скрити са и адресът, телефонът, имейлът…“.

     (б) ЗАЩО Е ГРЕШНО. Този архив е писан, за да се носи на флашка и да се
     праща по имейл (виж заглавния коментар на файла) — тоест точно затова
     заключената защита скрива личните колони: тя значи, че библиотекарката не е
     доказала правото си да ги вижда СЕГА. Вратичката обезсмисля ключалката, а
     обещанието в ПРОЧЕТИ-МЕ я прави по-опасна от липсваща: човекът праща файла
     спокойно, защото е прочел, че е чист.

     (в) ЗАЩО ПОПРАВКАТА Е ТОЧНО ТАЗИ.
       • `audit_log.diff` — ВИНАГИ. В тази колона няма нищо друго освен „преди
         и след“ на редактирани полета, а за читателите това са именно адрес,
         телефон, имейл, ЕГН и № на лична карта. ЕГН-тата не излизат в никакво
         състояние (виж заглавния коментар); `diff` е техният втори вход и
         затова стои при `PII_ALWAYS`, а не при „когато е заключено“.
       • `audit_log.detail` — ПРИ ЗАКЛЮЧЕНА защита. Тук има и имена на читатели
         („карта 777 — Иван Петров Стоянов“), но има и целият смисъл на следата
         за проверяващия („инв. № 101 — Под игото“, „акт № 1/2026“). Затова не
         отпада винаги, а само когато отпадат и останалите данни за връзка —
         точно както `readers.note`.
       • И двете влизат в списъка „Скрити колони“ на ПРОЧЕТИ-МЕ.txt, който се
         съставя от същите тези списъци — тоест обещанието и действието остават
         едно нещо и не могат да се разминат пак. */
  const PII_ALWAYS = {
  ...
  function buildZip(entries) {
    ...
    for (const { name, data } of entries) {
      ...
      let method = 8;
      /* НИВО НА СВИВАНЕ 6, А НЕ 9 (v2.4.65, кръг 42, находка Г3 — измерено).
         На истинската база от измерването (15 000 документа, 3 000 читатели,
         12 000 заемания, 19,47 МБ) целият `exportAll:run` струваше 1 490 ms, от
         които заявките 232 ms, съставянето на CSV 167 ms — и `deflateRawSync`
         на level 9 цели 544 ms, тоест над една трета от действието за едното
         свиване. Същите данни на level 6 се свиват за 167 ms и дават архив
         1,58 МБ вместо 1,52 МБ: спестени ~300 ms срещу 4 % по-голям файл.
         Изборът е в полза на времето, защото 60 kB разлика не се усещат по
         никакъв начин (архивът се записва на диск и се носи на флашка), а
         половин секунда изчакване без индикатор се усеща. Level 6 е и
         подразбиращото се ниво на zlib — тоест това, което всеки друг
         архиватор би дал за същия файл. */
  ...
  ipcMain.handle('exportAll:run', async () => {
    try {
      ...
      const readme = [
        ...
        '',
        /* ОДИТНАТА СЛЕДА СЕ НАЗОВАВА ИЗРИЧНО (v2.4.65, А8). Дотук този абзац
           изброяваше само колоните на chitateli.csv и точно затова подвеждаше:
           човек, прочел „скрити са адресът и телефонът“, няма причина да отвори
           odit-sleda.csv и да провери. Сега и двата файла са казани поименно —
           и в двете посоки, защото при ОТКЛЮЧЕНА защита следата наистина излиза
           с имената и трябва да се пази като личните данни, които съдържа. */
        pdpLocked
          ? 'Защитата на личните данни е ЗАДАДЕНА и в момента ЗАКЛЮЧЕНА, затова\n'
            ...
            + 'на гаранта. По същата причина в odit-sleda.csv е скрита и колоната\n'
            + '„detail“ — в нея стоят имената на читателите („карта 777 — …“).\n'
            + 'Ако този износ Ви трябва с тях, отключете защитата от\n'
          ...
          : (pdpConfigured
            ? 'Защитата на личните данни е зададена и отключена в тази сесия,\n'
              ...
              + 'Файловете chitateli.csv и odit-sleda.csv съдържат лични данни\n'
              + '(в следата стоят и имената на читателите) — пазете ги съответно.'
            : 'Защита на личните данни не е задавана в тази програма, затова\n'
              + 'адресът, телефонът и останалите данни за връзка са изнесени.\n'
              + 'Файловете chitateli.csv и odit-sleda.csv съдържат лични данни\n'
              + '(в следата стоят и имената на читателите) — пазете ги съответно.'),
        '',
        'Колоната „diff“ на odit-sleda.csv (какво е било и какво е станало при',
        'всяка редакция) НЕ се изнася при никакви обстоятелства: при читателите',
        'в нея стоят старият и новият адрес, телефон, имейл и ЕГН.',
        '',
