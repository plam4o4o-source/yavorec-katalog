...
module.exports = function registerExportAllHandlers(ipcMain, deps) {
  ...
  const TABLES = {
    ...
    /* „Полезни връзки“ беше просто грешно име (v2.4.61): в тази таблица няма
       нито един интернет адрес. Тя е краеведският указател — кой запис в
       „Персоналии“ и в „Летопис“ към кой документ, статия, персоналия или
       периодично издание сочи. Човекът, който отваря архива без да е виждал
       програмата (регионалната библиотека, общината, следващата програма), е
       единственият читател на този ред — и по него решава дали файлът му
       трябва. */
    links: ['vrazki', 'Краеведски връзки (персоналия/летопис → документ, статия, персоналия, периодика)'],
