...
module.exports = function registerBackupHandlers(ipcMain, deps) {
  ...
  /* Здрав ли е даден файл с база данни. Одитът установи НУЛА попадения на
     ...
     opts.quick → PRAGMA quick_check вместо integrity_check. Ползва се САМО за
     прясно записано копие, чийто източник току-що е проверен (verifyFreshBackup
     — там е и цялото обяснение защо). Файл, дошъл отвън и предложен за
     ВЪЗСТАНОВЯВАНЕ, минава през дълбоката проверка по подразбиране. */
  ...
  /* ПОВРЕДЕНА СТРАНИЦА — НА БЪЛГАРСКИ (v2.4.65, кръг 42, находка В10).
     (а) Дотук `sqliteProblem()` превеждаше само двете СЪОБЩЕНИЯ ЗА ГРЕШКА на
     SQLite („database disk image is malformed“, „file is not a database“), но
     когато файлът се ОТВАРЯ и се проваля самата проверка, изходът на
     `integrity_check` се вграждаше дословно в българското изречение:
         „Избраният файл не може да бъде възстановен: проверката на базата
          съобщава: *** in database main *** Tree 3 page 151 cell 12: Rowid 65
          out of order … Fragmentation of 395 bytes reported as 0 on page 151.
          Базата данни на библиотеката НЕ е променяна.“
     (б) ЗАЩО Е ГРЕШНО. Това е екранът, на който библиотекарката стои, защото
     базата ѝ вече е повредена — най-лошият възможен момент да получи шест реда
     английски термини за B-дървета. От текста не личи нито какво да направи,
     нито че има още копия в списъка; а той е и дълъг колкото цял абзац.
     (в) ЗАЩО ТОЧНО ТАКА. Казва се това, което решава въпроса — файлът е
     повреден и НЕ става за възстановяване, опитайте с по-старо копие — и чак
     след това, в скоби и с етикет, суровият изход остава за човека, който
     евентуално ще помага по телефона. Изходът се свива до първия му ред: той
     назовава повредата, а следващите са същото за съседните клетки. */
  function describeIntegrityFailure(res) {
    const raw = String(res == null ? '' : res).trim();
    const first = raw.split('\n').map(x => x.trim()).filter(x => x && !/^\*\*\*/.test(x))[0] || raw;
    return 'файлът е повреден — проверката на базата намери повредена страница вътре в него. '
      + 'Такова копие не става за възстановяване: изберете по-старо копие от списъка '
      + '(техническа подробност за поддръжката: „' + first + '“)';
  }
  function sqliteProblem(filePath, opts) {
    ...
    try {
      ...
      if (String(res).toLowerCase() !== 'ok') return describeIntegrityFailure(res);
      return null;
    } catch (err) {
  ...
  /* ЗАЩО quick_check, А НЕ integrity_check (v2.4.64, измерване на стартирането).
     ===========================================================================
     ИЗМЕРЕНОТО. При базата от измерването (19,47 МБ — 15 000 документа, 3 000
     читатели, 12 000 заемания) автоматичното копие при стартиране струваше
     140 ms, разбити така (node /tmp/r41/backup-split.js, медиана от 5 повторения):
       wal_checkpoint(TRUNCATE)                 0 ms
       db.serialize() → буфер 19,5 МБ          17 ms
       fs.writeFileSync на .tmp                 5 ms
       проверка на заглавието „SQLite format 3“ 1 ms
       отваряне на копието само за четене       0 ms
       PRAGMA integrity_check ВЪРХУ КОПИЕТО   117 ms   ← ТУК
       rename .tmp → auto-ГГГГ-ММ-ДД.db         0 ms
     Тоест 117 от 140 ms — над осем десети от цялото копие — отиваха в ПОВТОРНА
     дълбока проверка. PRAGMA quick_check върху същия файл е 41 ms.

     КАКВО ХВАЩА quick_check: целия обход на страниците на базата — отрязан или
     непълен файл, нечетима страница, разпаднато B-дърво, редове, които не
     отговарят на схемата си, счупени страници с презаписан или липсващ край.
     Точно това, срещу което тази проверка съществува: ПРЕКЪСНАТ ИЛИ НЕПЪЛЕН
     ЗАПИС — спиране на тока, изваден USB насред копирането, прекъснат мрежов
     дял, антивирусна, захапала файла по средата. Копие с такава повреда не
     минава quick_check и doBackupTo() го отказва, изтрива .tmp-а и оставя
     предишното копие непокътнато — точно както досега.

     КАКВО НЕ ХВАЩА: quick_check пропуска САМО сверяването на индексите срещу
     таблиците (ред в индекс без ред в таблицата, разместена подредба, дублирана
     стойност в UNIQUE индекс) — тя е скъпата част на integrity_check. Такова
     разминаване обаче НЕ се ражда от записа на копието: db.serialize() изнася
     страниците на вече отворената база такива, каквито са. Разминат индекс в
     копието значи разминат индекс в ЖИВАТА база отпреди копирането — това е
     предварителна повреда на източника, не провал на записа.

     ЗАЩО ТОВА Е ПРАВИЛНАТА РАЗМЯНА ТОЧНО ТУК. Източникът на това копие не е
     непознат файл, а базата, която същата програма е проверила преди по-малко от
     секунда: checkDbIntegrityAtStartup() в main.js прави PRAGMA quick_check върху
     ЖИВАТА база ~200 ms преди авто-копието и при повреда изобщо не пуска
     програмата напред. Дълбоката проверка тук питаше втори път за здравето на
     току-що проверена база и плащаше 117 ms при всяко пускане. Въпросът, на който
     тази проверка ТРЯБВА да отговори, е друг — „стигна ли всичко до диска“ — и на
     него quick_check отговаря изцяло.

     КЪДЕ ДЪЛБОКАТА ПРОВЕРКА ОСТАВА: при ВЪЗСТАНОВЯВАНЕ (sqliteProblem() без opts
     — виж backup:restoreFromList/restoreBrowse по-долу). Там файлът идва отвън:
     от чужд компютър, от USB, отпреди месеци, никой не го е проверявал, и той
     предстои да ЗАМЕНИ живата база. Там 117 ms не значат нищо срещу цената на
     възстановена повредена база, затова deep остава по подразбиране. */
  function verifyFreshBackup(filePath, password) {
    if (!password) return sqliteProblem(filePath, { quick: true });
    let buf;
    try {
    ...
    try {
      ...
      /* Същата размяна като при некриптираното копие по-горе, и с още едно
         основание: целостта на САМИЯ .invbak вече е доказана — decryptBackupBuffer
         проверява етикета на AES-GCM, който не минава при нито един отрязан или
         променен байт. Тук остава да се потвърди, че разшифрованото е здрава база,
         и за това quick_check стига. */
      return sqliteProblem(tmp, { quick: true });
    } catch (err) {
  ...
  /* ЕДИНСТВЕНИЯТ вход към записа на копие. Дотук НЕкриптираното копие се
     ...
     път: пиши настрани → провери, че наистина е база и че проверката на базата
  ...
  /* ДНЕШНИТЕ МЕЖДИННИ КОПИЯ (v2.4.65, кръг 42, находка А10).
     `todayPaths()` знаеше само за ЕДИН файл на ден — `auto-ГГГГ-ММ-ДД`. Но
     програмата прави и междинни копия: таймерът на 3 часа и `backupBeforeQuit()`
     при всяко затваряне (виж intradayPath()). Точно те са най-новите файлове в
     папката — копието при затваряне носи целия последен работен ден. Щом
     „днешното копие“ означава само дневния файл, всяко правило, което се
     прилага „за днес“ (криптиране след отключване, прекриптиране след смяна на
     паролата), подминава именно най-прясното. Затова тук — вторият списък. */
  function todayIntradayBackups() {
    const today = todayStr();
    const re = new RegExp('^auto-' + today + '-\\d{4}\\.(db|invbak)$');
    try {
      return fs.readdirSync(backupsDir()).filter(f => re.test(f)).sort();
    } catch (e) {
      return []; // няма папка — няма и копия; същият избор като в newestAutoBackupAt()
    }
  }
  ...
  /* Известие към интерфейса, че състоянието на дневното копие се е променило —
     ...
     съществува (напр. копие, поискано от друг път преди старта на интерфейса) —
     тогава просто няма кого да известим, картата в „Настройки“ ще прочете
     състоянието при отваряне. От v2.4.64 дневното копие при стартиране тръгва
     СЛЕД като прозорецът е зареден (виж main.js), тоест известието обикновено
     стига дотам. */
  ...
  /* ДНЕШНИТЕ МЕЖДИННИ КОПИЯ В ЧИСТ ТЕКСТ (v2.4.65, кръг 42, А10).
     Втората половина на същата находка. Докато защитата е заключена (или още не
     е отключена след стартирането), таймерът и затварянето на програмата пишат
     междинните копия НЕкриптирани — `auto-ГГГГ-ММ-ДД-ЧЧММ.db`. Дотук нищо
     никога не ги вдигаше: `upgradeTodayAutoBackup` знаеше само за дневния файл,
     а `reencryptOldBackups` гледа само вече криптирани файлове. Тоест след
     отключването екранът казваше „Днешното резервно копие вече е криптирано“, а
     до него в същата папка лежеше пълен регистър с имената, адресите и ЕГН на
     всички читатели — понякога по-нов от дневния.
     Редът е същият като навсякъде другаде в този файл: пиши настрани → провери,
     че новият файл наистина се отваря с паролата → преименувай → и чак накрая
     махни некриптирания. Провал на един файл не спира останалите. */
  function encryptTodayIntradayBackups(password) {
    const dir = backupsDir();
    let done = 0;
    const failed = [];
    for (const f of todayIntradayBackups()) {
      if (!f.endsWith('.db')) continue; // криптираните минават през reencryptOldBackups
      const full = path.join(dir, f);
      const encDest = full.slice(0, -'.db'.length) + '.invbak';
      if (fs.existsSync(encDest) && opensWith(encDest, password)) {
        // Криптираният близнак вече е налице и е четим — остава само да падне голият.
      ...
      try {
        ...
        failed.push(f);
        try { if (fs.existsSync(staged)) fs.unlinkSync(staged); } catch (e2) { /* нищо за чистене */ }
      }
    }
    if (done || failed.length) {
      logAudit('Резервно копие', done + ' от днешните междинни копия бяха презаписани криптирани'
        + (failed.length ? '; ВНИМАНИЕ: ' + failed.length + ' останаха в ЧИСТ ТЕКСТ и съдържат личните '
          + 'данни на читателите: ' + failed.join(', ') : ''));
    }
    return { done, failed };
  }
  ...
  function upgradeTodayAutoBackup(meta) {
    ...
    /* ПЪРВО междинните — и БЕЗУСЛОВНО, преди която и да е от ранните изходни
       точки по-долу. Точно те са най-прясното копие в папката (последното
       затваряне на програмата носи целия работен ден), а всяка от ранните
       „няма какво да се прави“ проверки долу пита само за ДНЕВНИЯ файл. */
    const intraday = encryptTodayIntradayBackups(password);
    if (intraday.failed.length) {
      notifyAutoBackup('err', intraday.failed.length + ' от днешните междинни резервни копия останаха в '
        + 'ЧИСТ ТЕКСТ и съдържат личните данни на читателите — вижте одитната следа и „Настройки“ → '
        + '„Резервно копие“.');
    }
    const intradayNote = intraday.done
      ? ' Криптирани са и ' + intraday.done + (intraday.done === 1 ? ' междинно копие' : ' междинни копия') + ' за днес.'
      : '';
    /* Ранните изходи по-долу означават „с ДНЕВНИЯ файл няма какво да се прави“ —
       но ако междинните току-що са били криптирани, това е новина за екрана и
       не бива да пропадне заедно с тях. */
    const doneWithIntradayOnly = () => {
      if (intraday.done) {
        notifyAutoBackup('ok', 'Днешните междинни резервни копия вече са криптирани с паролата за защита '
          + 'на личните данни (' + intraday.done + ').');
      }
    };
    ...
    if (fs.existsSync(encDest)) {
      ...
      if (!changed && todayEncryptedWith && todayEncryptedWith.date === today && todayEncryptedWith.fp === fp) {
        doneWithIntradayOnly();
        return;
      }
      if (changed) {
    ...
    /* Съобщението назовава и междинните (А10): дотук зеленото „Днешното резервно
       ...
       парола. Числото идва от свършеното, не от намерението. */
    notifyAutoBackup('ok', (changed
      ? 'Днешното резервно копие беше прекриптирано с новата парола.'
      : 'Днешното резервно копие вече е криптирано с паролата за защита на личните данни.') + intradayNote);
  }
  ...
  function reencryptOldBackups(password, prevPassword) {
    ...
    try {
      ...
      /* И предпазното копие отпреди ИЗТРИВАНЕТО на всички данни (v2.4.64).
         Същият пропуск, същите последствия — само по-тежки: след „Започване на
         чисто“ това е ЕДИНСТВЕНОТО копие на цялата стара библиотека, защото
         базата вече е празна. Оставено извън филтъра, смяната на паролата (най-
         честият повод: напуснал служител) го заключва завинаги със старата
         парола, а backup:list продължава да го показва като наличен изход. */
      /* И МЕЖДИННИТЕ, И РЪЧНИТЕ КОПИЯ (v2.4.65, кръг 42, находка А10).
         (а) КАКВО СТАВАШЕ ДОТУК. Филтърът беше `^auto-ГГГГ-ММ-ДД\.invbak$` —
         тоест САМО дневното копие. Извън него оставаха точно двата вида, които
         на практика са най-НОВИТЕ файлове в папката:
           • МЕЖДИННИТЕ `auto-ГГГГ-ММ-ДД-ЧЧММ.invbak` — прави ги таймерът на
             всеки 3 часа И backupBeforeQuit() при всяко затваряне на програмата
             (intradayPath() по-горе). Копието при затваряне носи ЦЕЛИЯ последен
             работен ден. По AUTO_BACKUP_INTRADAY_KEEP = 4 на ден и 30 дни назад
             това са до ~120 файла.
           • РЪЧНИТЕ `Inventar-backup-….invbak` (backup:now, ред ~1081) — онова,
             което библиотекарката прави сама преди нещо страшно.
         (б) ЗАЩО Е ГРЕШНО. След смяна на паролата екранът показваше зелено
         „Днешното резервно копие беше прекриптирано с новата парола“, а
         най-прясното копие в папката се отваряше САМО със СТАРАТА. Двете
         последици са различни и двете са тежки: ако паролата се сменя, защото
         старата е компрометирана (най-честият повод — напуснал служител), тези
         файлове остават четими с компрометираната; а ако старата просто бъде
         забравена, последният работен ден става невъзстановим — докато екранът
         твърди обратното.
         (в) ЗАЩО ТОЧНО ТАКА. Приемат се и четирите семейства имена, които
         САМАТА програма пише криптирани, и нищо друго: чужд .invbak, донесен в
         папката отвън, нарочно не се пипа — за него няма стара парола и опитът
         би дал само излишен доклад за провал. Обработката по-долу не се мени:
         всеки файл се проверява поотделно (opensWith) и се прекриптира само
         ако наистина не се отваря с текущата парола. */
      files = fs.readdirSync(dir)
        .filter(f => /^auto-\d{4}-\d{2}-\d{2}(-\d{4})?\.invbak$/.test(f)
    ...
    for (const f of files) {
      ...
      try {
        ...
        /* Прясно записан файл — значи и тук проверка, не само „отваря ли се“:
           verifyFreshBackup() хваща повредата, дошла от самото прекриптиране
           (прекъснат запис по мрежов дял), която иначе би се видяла чак в деня,
           в който точно това копие потрябва. Проверката е quick_check (виж
           обяснението при verifyFreshBackup): тя е точно срещу непълния запис, а
           съдържанието идва от decryptBackupBuffer, тоест е вече потвърдено
           байт по байт от етикета на AES-GCM. */
        const problem = verifyFreshBackup(staged, password);
        if (problem) throw new Error('новото копие не мина проверката — ' + problem);
  ...
  ipcMain.handle('backup:autoStatus', () =>
    run(() => {
      ...
      /* Броят НЕкриптирани дневни копия се смята ПРЕДИ решението за състоянието
         ...
         екрана да го каже. */
      /* И КОЛКО ОТ ТЯХ СА МЕЖДИННИ (v2.4.65, кръг 42, находка В11).
         `plainDailyCount` брои ВСИЧКИ `auto-*.db`, тоест и междинните
         `auto-ГГГГ-ММ-ДД-ЧЧММ.db` — до 4 на ден. Предупреждението обаче ги
         описваше като „некриптирани ДНЕВНИ копия ОТПРЕДИ ВКЛЮЧВАНЕТО на
         защитата“, а те са друго и често са отпреди минути: правят се от
         таймера на 3 часа и при всяко затваряне на програмата и са в чист
         текст винаги, когато защитата е заключена в момента на копието.
         Библиотекарка, включила защитата днес, четеше „копия отпреди
         включването“, брояше файловете с днешна дата и решаваше, че екранът
      ...
      try {
        ...
        plainIntradayCount = names.filter(f => /^auto-\d{4}-\d{2}-\d{2}-\d{4}\.db$/.test(f)).length;
        /* Предпазните копия отпреди необратимо действие са различен вид файл и НЕ
           бива да се съветва изтриването им: те са единственият изход от сгрешено
           възстановяване или от изтриване на всички данни. Броят се отделно точно
           за да не се слеят в един съвет.
           v2.4.64: тук влиза и `before-reset-…` — копието, което „Изтриване на
           всички данни“ прави ЗАДЪЛЖИТЕЛНО преди да изпразни базата
           (handlers/reset.js). То съдържа целия стар регистър с имената, адресите
           и ЕГН на читателите; ако защитата на личните данни е изключена или
           заключена, то лежи в чист текст като всяко друго. Дотук се броеше само
      ...
      if (encrypted) {
        ...
        /* Надписът брои и двата вида поотделно (В11). „Дневни“ са
           `auto-ГГГГ-ММ-ДД.db` — по едно на ден; „междинни“ са
           `auto-ГГГГ-ММ-ДД-ЧЧММ.db` — от таймера на 3 часа и при затваряне на
           програмата. Причината за оставането им също е различна: дневното е
           отпреди включването на защитата, междинното — отпреди отключването ѝ
           в конкретната сесия. */
        const plainOnlyDaily = plainDailyCount - plainIntradayCount;
        warning = plainDailyCount
          ? 'В папката с резервните копия все още стоят некриптирани автоматични копия: '
            + (plainOnlyDaily ? plainOnlyDaily + (plainOnlyDaily === 1 ? ' дневно' : ' дневни') : '')
            + (plainOnlyDaily && plainIntradayCount ? ' и ' : '')
  ...
  /* УСПЕШНОТО ВЪЗСТАНОВЯВАНЕ ОСТАВЯ РЕД В СЛЕДАТА (v2.4.65, кръг 42, Б21)
     ==========================================================================
     (а) КАКВО СТАВАШЕ ДОТУК. `performRestore()` вписваше ред САМО при провал
     („предпазното копие не можа да бъде направено по обичайния начин…“).
     Успешното възстановяване — тоест действието, което ПОДМЕНЯ цялата база на
     библиотеката — не оставяше нито ред никъде. Библиотекарката отваря
     програмата след рестарта и намира базата от миналия вторник: липсват
     двайсет вписани документа и трийсет заемания, а нищо в програмата не казва
     нито че е било възстановявано, нито от кой файл, нито на коя дата.
     (б) ЗАЩО Е ГРЕШНО. По чл. 17, ал. 2 от Наредба № 3 одитната следа отговаря
     на въпроса „кой, кога, какво“ по вписванията в регистрите. Подмяната на
     ЦЕЛИЯ регистър е най-голямото възможно вписване и е единственото, за което
     следата мълчеше. Освен това точно този ред обяснява по-късно „изчезналите“
     документи — без него разликата изглежда като загуба на данни.
     (в) ЗАЩО ТОЧНО ТАКА — И КЪДЕ. Ред, вписан в ЖИВАТА база преди подмяната, е
     безсмислен: след секунда тази база е презаписана от копието и редът си
     отива заедно с нея. Затова редът се вписва в САМОТО КОПИЕ — във файла
     `*.restore-tmp`, докато той още е настрани, преди преименуването. След
     рестарта програмата отваря именно него и следата отваря с реда „на тази
     дата базата беше заменена с копие от …“. Записът е в отделна връзка към
     файла и се затваря веднага; провал тук НЕ отменя възстановяването —
     по-добре възстановена база без ред, отколкото отказано възстановяване
     заради невъзможно вписване (затова грешката се съобщава, а не се поглъща).
     Редът се пише като реда на logAudit() в main.js — същите колони, същият
     формат — но с изрична `ts`, защото връзката е нова. */
  function noteRestoreInRestoredDb(stagedPath, sourcePath, safetyPath, safetyEncrypted) {
    /* КОЙ. `getCurrentUser` НЕ е сред зависимостите, които main.js подава на
       ТОЗИ модул (подава го на handlers/reset.js — виж main.js:2153), а main.js
       е забранен за промяна в този кръг. Затова: ако някой ден бъде подаден, се
       ползва; дотогава името се взема от последния ред на СТАРАТА следа — това е
       служителят, който току-що е работил, тоест този, който възстановява. По-
       добре вероятно вярно име, отколкото празна колона „кой“ на най-голямото
       действие в програмата. (Нужната промяна в чужд файл е докладвана.) */
    let who = '';
    try {
      if (typeof deps.getCurrentUser === 'function') who = deps.getCurrentUser() || '';
      if (!who) {
        const live = getDb();
        const last = live && live.prepare('SELECT user FROM audit_log WHERE COALESCE(user, \'\') != \'\' '
          + 'ORDER BY id DESC LIMIT 1').get();
        who = (last && last.user) || '';
      }
    } catch (e) {
      who = ''; // следата ще каже „(без записан служител)“ — не е повод да се спира
    }
    let out = null;
    try {
      out = new Database(stagedPath);
      const has = out.prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'audit_log'").get();
      if (!has) return; // копие от толкова стара схема, че таблицата я няма — няма къде да се впише
      out.prepare('INSERT INTO audit_log (ts, user, action, detail, diff) VALUES (?, ?, ?, ?, NULL)').run(
      ...
      /* Без празен catch: възстановяването продължава, но човекът трябва да
         знае, че следата няма да го обясни — иначе липсата на ред по-късно се
         чете като „не е било възстановявано“. */
      console.error('Редът за възстановяването не можа да се впише в копието:', err.message);
      try {
        logAudit('Възстановяване от резервно копие', 'ВНИМАНИЕ: редът за възстановяването не можа да бъде '
          + 'вписан във възстановената база (' + err.message + '). Възстановяването продължава. След '
          + 'рестарта в одитната следа няма да има запис, че на тази дата базата е заменена с копието „'
          + sourcePath + '“.');
      } catch (e2) { /* следата е в базата, която точно сега се подменя */ }
    } finally {
      if (out) { try { out.close(); } catch (e) { /* връзката и без това си отива */ } }
    }
  }
  ...
  /* doBackupTo СЕ ИЗНАСЯ (v2.4.64).
     =====================================================================
     Редът „запиши настрани (.tmp) → провери копието → преименувай“ е една от
     малкото последователности в програмата, които НЕ бива да съществуват на две
     места: при прекъсване по средата разликата между правилния и „почти
     правилния“ ред е файл с име на редовно копие и съдържание на половин база.
     „Изтриване на всички данни“ (handlers/reset.js) прави задължително копие,
