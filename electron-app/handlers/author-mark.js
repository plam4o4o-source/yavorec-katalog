...
/* ============================================================================
   ...
   ТАБЛИЦАТА ИДВА С ПРОГРАМАТА (v2.4.63). Знакът е буква + двузначно число, а
   числата стоят в db/author-table.js — вграден файл, зареден при първото пускане
   (миграция 17). Дотук програмата не носеше таблица и всяка библиотека трябваше
   да посочи свой файл; докато това не се случеше, копчето „Предложи“ до полето
   „Авторски знак“ нямаше откъде да предлага и сигнатурата се пишеше на ръка,
   книга по книга.

   ВНЕСЕНОТО ИМА ПРЕДИМСТВО. Библиотека със СВОЕ издание на таблиците внася своя
   файл по всяко време и той застава върху вграденото — вграденото се зарежда
   САМО в празна таблица и никога не презаписва внесено (виж миграция 17 в
   main.js и authorMark:loadBuiltin по-долу). Разчитането е през един и същ код
   (parseTable), тоест вграденото и внесеното не могат да се разминат по формат.
   ...
   показва ОТКЪДЕ идва („фамилия «Вазов» → ред «ВАЗ» → В-14“), а вписването
...
/* ВГРАДЕНАТА ТАБЛИЦА (v2.4.63).
   =====================================================================
   db/author-table.js носи редовете в ЧОВЕКОЧЕТИМ вид („ДИМИТРОВ, Г“), а
   вътрешният ключ се получава тук — през rowKeyOf(), същата функция, която
   обработва и внесен файл. Така вграденото и внесеното минават по един път:
   ако утре се промени нормализацията, тя се променя на едно място и двете я
   получават заедно.
   require() чете и през app.asar в пакетираното приложение (както
   fs.readFileSync чете db/schema.sql), а данните са статични — кешът на
   модула е точно каквото трябва. */
let BUILTIN_CACHE = null;
function builtinRows() {
  if (!BUILTIN_CACHE) {
    /* ЛИПСВАЩ ИЛИ ПОВРЕДЕН ФАЙЛ НЕ СЪБАРЯ ЕКРАНА. authorMark:status се вика при
       всяко отваряне на „Настройки“ → „Фонд“; ако require() хвърли оттам,
       целият раздел се заменя със „състоянието не се зареди“ и изчезва копчето
       за внасяне на файл — тоест повреда във вграденото отнема и единствения
       път, който работеше преди то да съществува. Затова провалът се записва
       като ПРАЗНА вградена таблица: екранът просто не предлага вграденото. */
    try {
      BUILTIN_CACHE = require('../db/author-table.js')
        .map(([prefix, mark]) => ({ prefix: rowKeyOf(prefix), mark: String(mark) }))
        .filter(r => r.prefix && r.mark);
    } catch (err) {
      console.error('Вградената таблица за авторски знак не се чете:', err.message);
      BUILTIN_CACHE = [];
    }
  }
...
module.exports = function registerAuthorMarkHandlers(ipcMain, deps) {
  ...
  ipcMain.handle('authorMark:status', () => run(() => {
    ...
    /* Дали в базата стои ТОЧНО вградената таблица. Екранът казва това с думи:
       библиотекарката трябва да може да различи „ползвам таблицата на
       програмата“ от „внесох своята“, иначе не знае какво ще се промени, ако
       натисне „Внеси друг файл“. Сравнява се по СЪДЪРЖАНИЕ, а не по брой — две
       различни издания спокойно имат равен брой редове — и не по РЕДА на
       редовете: от базата те идват по `ORDER BY prefix` (побайтова подредба на
       SQLite), а вграденият файл е подреден по буква и число, както се чете
       печатната таблица. Сравнение по позиция даваше „внесена“ за таблица,
       която е буквално вградената. */
    const bi = builtinRows();
    const biMap = new Map(bi.map(r => [r.prefix, r.mark]));
    const isBuiltin = rows.length === bi.length && rows.every(r => biMap.get(r.prefix) === r.mark);
    return {
      ...
      builtinRows: bi.length, isBuiltin
    };
  }));
  ...
  /* Зареждане на ВГРАДЕНАТА таблица по изрично искане — за библиотека, която е
     изтрила своята или е внесла негоден файл и иска да се върне към таблицата
     на програмата. Автоматичното зареждане в ПРАЗНА таблица е в миграция 17
  ...
  ipcMain.handle('authorMark:loadBuiltin', () => run(() => {
    ...
    db.transaction(() => { db.prepare('DELETE FROM author_table').run(); rows.forEach(r => ins.run(r.prefix, r.mark)); }).immediate();
    pending = null;
    logAudit('Таблица за авторски знак', 'заредена е вградената таблица — ' + rows.length + ' реда');
    return { rows: rows.length };
  }));
...
module.exports.pure = { basisOf, keyOf, rowKeyOf, prefixLabel, refinements, isNamed, parseTable, lookup, formatMark,
  ...
  foldJot, searchKey, tableHasJot, builtinRows };
/* Вградената таблица — и за засяването в initDb() (main.js), което я зарежда в
   празна база при стартиране. Стои и тук, извън `pure`, за да не зависи
   викащият от това дали функцията се брои за чиста: тя чете файл
   (db/author-table.js), макар и статичен и кеширан. */
module.exports.builtinRows = builtinRows;
