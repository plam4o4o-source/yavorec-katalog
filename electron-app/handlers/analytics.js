  ...
  ipcMain.handle('analytics:list', (e, { q, year, onlyLocal } = {}) =>
    run(() => {
      ...
      /* Търсенето обхваща и ПОДЗАГЛАВИЕТО, УДК и БРОЯ (v2.4.61): подзаглавието
         ...
         ТЪРСИ СЕ ПО ТОВА, КОЕТО САМИЯТ УКАЗАТЕЛ ПОКАЗВА (v2.4.65, находка Б19).
         =====================================================================
         ДОТУК две неща от колона „Източник“ не се намираха по никакъв начин:
           • АВТОРЪТ НА КНИГАТА-НОСИТЕЛ. Редът гласи „Колектив. Сборник Яворец
             (инв. № 900)“ — авторът е първото, което се чете, — а търсене на
             „Колектив“ връщаше 0. Търсеше се `a.author` (авторът на СТАТИЯТА) и
             `b.title` (заглавието на книгата), но не и `b.author`. Краеведът,
             който преписва от показания ред, получаваше празен отговор от поле,
             чиято подсказка обещава дословно „…заглавие, автор, ключови думи,
             източник…“.
           • САМАТА ДУМА „БР.“. Коментарът отгоре твърди, че „«бр. 21» е начинът,
             по който човек пита за конкретен вестник“ — а в `a.issue` стои голо
             „21“ и LIKE '%бр. 21%' не съвпадаше с нищо. Тоест намираше се само
             това, което НЕ се показва („21“), а показаното („бр. 21 от
             27.05.2025“) не се намираше.
         ЗАЩО ПОПРАВКАТА Е ИМЕННО ТАКАВА: представката се отрязва в КАНАЛА, по
         същия образец, по който links:search разпознава „инв. № 5“
         (handlers/links.js) — там доводът е дословно, че изрично посоченият
         номер трябва да търси САМО по номер, иначе точното питане дава
         приблизителен отговор. Тук е същото: „бр. 21“ е питане за брой, не за
         думата „21“ някъде в текста — иначе отговорът щеше да включва и 1921 г.,
         и стр. 21, и УДК 21. Затова при изрична представка се търси само в
         `a.issue`: точно съвпадение на свитата стойност, плюс съдържане, за да
         се намери и брой, изписан като „21-22“ или „21/22“. */
      const raw = String(q == null ? '' : q).trim();
      /* Представката се иска С РАЗДЕЛИТЕЛ — точка или интервал. Без това условие
