...
module.exports = function registerHoldsHandlers(ipcMain, deps) {
  ...
  function activateHoldOnReturn(bookId) {
    ...
    if (!next) {
      /* НЯМА ЧАКАЩ — ТАЗИ БРОЙКА НУЖНА ЛИ Е ЗА ЗАДЕЛЕНА РЕЗЕРВАЦИЯ? (v2.4.67)
         Дотук тук стоеше `return queue[0]` безусловно. Стар неразделен запис с 2
         бройки, и двете заети, един чакащ A: първото връщане заделя за A, второто
         не намира чакащ и връщаше ПАК резервацията на A — екранът казваше „заделете
         за A“ и втората бройка отиваше на рафта за заделени, докато базата я
         водеше свободна.
         Връщането вече е записано, тоест freeCopies() брои и тази бройка. Ако
         свободните са ПОВЕЧЕ от заделените резервации, бройката е излишна и отива
         на отворения рафт. Иначе е нужна — законният случай е, когато заделената
         бройка все пак е била заета от друг и сега се връща: тя трябва да се
         върне при заделените, и тогава се връща резервацията. */
      const setAside = queue.filter(h => h.status === 'заделена').length;
      return freeCopies(bookId) > setAside ? null : queue[0];
    }
