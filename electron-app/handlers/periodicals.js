...
module.exports = function registerPeriodicalsHandlers(ipcMain, deps) {
  ...
  /* ЕДНО ИЗДАНИЕ — ЕДИН КАРТОН (одит v2.4.61, находка 2).
     ...
     COLLATE NOCASE в SQLite сгъва само латиницата, затова заглавието се сверява
     и след свиване на интервалите — „Труд “ и „Труд“ са едно и също заглавие за
     библиотекаря, а за базата не са.
  ...
  function assertNoDuplicate(db, row, selfId) {
    ...
    /* trim() и от двете страни: входът вече е изчистен от periodicalPayload(), но
       в базата може да има заварено заглавие с интервал отпред или отзад (внос от
       стар файл) — за библиотекаря „Труд “ и „Труд“ са едно и също издание. */
    const same = db.prepare('SELECT id, title, issn FROM periodicals WHERE id <> ? AND trim(title) = ? COLLATE NOCASE')
      .get(id, row.title);
    if (same) {
