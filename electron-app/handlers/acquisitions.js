...
module.exports = function registerAcquisitionsHandlers(ipcMain, deps) {
  ...
  function assertAcqDate(a) {
    ...
    /* БЪДЕЩА ДАТА НА ПАРТИДА ВЕЧЕ НЕ МИНАВА (v2.4.65, кръг 42, находка Б10).
       (а) КАКВО СТАВАШЕ ДОТУК. Проверката гледаше само ФОРМАТА. Разместени
           цифри в годината — „2062-09-22“ вместо „2026-09-22“ — минаваха, и
           програмата отговаряше „Партидата е заведена в КДБФ част 1“.
       (б) ЗАЩО Е ГРЕШНО ЗА БИБЛИОТЕКАТА И ЗА НАРЕДБА № 3. Годината 2062 я няма
           в падащото меню на КДБФ (то е текущата година и назад), Част № 1 за
           тази година казва „няма постъпления“, а партидата е вписана в
           официален регистър по чл. 14 — тоест съществува и я няма едновременно.
           И е в капан: щом по нея бъде инвентиран първият документ, партидата
           не може и да се изтрие, защото изтриването би оставило документите
           без постъпление.
       (в) ЗАЩО ПОПРАВКАТА Е ТОЧНО ТАЗИ. Същото правило и същото обяснение като
           при акта за отчисляване (handlers/deaccession-acts.js:475 – 480):
           документът носи датата на СЪСТАВЯНЕТО си, а такъв ден още не е
           настъпил. Отказът е в обработчика, защото и двата пътя (завеждане и
           поправка) минават оттук, и назовава изхода — коя дата да се въведе. */
    const nowStr = todayStr();
    if (String(a.date) > nowStr) {
      throw new Error('Датата на партидата (' + bgDate(a.date) + ' г.) е в бъдещето — днес е '
        + bgDate(nowStr) + ' г. Партидата носи датата, на която документите са постъпили по първичния '
        + 'документ (чл. 14, ал. 2); ден, който още не е настъпил, не може да е ден на постъпване. '
        + 'Бъдеща година освен това изважда партидата от Книгата за движение на фонда — Част № 1 се печата '
        + 'по години до текущата и редът не се появява в нито една от тях, а щом по партидата бъде '
        + 'инвентиран документ, тя вече не може и да се изтрие. Най-честата причина са разместени цифри в '
        + 'годината (2062 вместо 2026). Въведете датата от първичния документ.');
    }
  }
  /* НАЧИНЪТ НА ПОСТЪПВАНЕ ВЕЧЕ СЕ ПРОВЕРЯВА ТУК, ПРЕДИ ТРИГЕРА (v2.4.65, кръг 42,
     находка В8).
     (а) КАКВО СТАВАШЕ ДОТУК. `acquisitions.how` има enum тригер в
         db/enum-triggers.js (закупуване / депозит / обмен / дарение). Стойност
         извън него — от внесена стара таблица, от ръчно редактирана база или от
         печатна грешка — стигаше до самия тригер и библиотекарката получаваше
         суровото му съобщение „Непозната стойност за acquisitions.how.“: без
         кое поле е това на екрана, без кои са позволените стойности и без какво
         да направи.
     (б) ЗАЩО Е ГРЕШНО. Начинът на постъпване е реквизит на партидата по чл. 14,
         ал. 2 и определя кой първичен документ се съставя (акт за дарение по
         чл. 6, ал. 5 при дарение). Партидата просто „не се записва“, а
         библиотекарката няма как да разбере защо — най-честата ѝ реакция е да
         реши, че програмата е повредена.
     (в) ЗАЩО ПОПРАВКАТА Е ТОЧНО ТАЗИ. Дословно по образеца на assertValidStatus
         в handlers/books.js:176 – 186: проверката се прави ПРЕДИ да се стигне до
         тригера, съобщението е на български, назовава полето така, както е
         изписано на екрана, изрежда позволените стойности и казва изхода.
         Списъкът се чете от СЪЩИЯ източник, който създава тригера, за да не
         могат двата да се разминат. Празно поле остава позволено (партидата може
         да се заведе, преди начинът да е изяснен) — точно както досега. */
  const { ENUM_COLUMNS } = require('../db/enum-triggers');
  const ACQ_HOW_VALUES = (ENUM_COLUMNS.find(c => c.table === 'acquisitions' && c.col === 'how') || {}).values || [];
  function assertAcqHow(a) {
    const how = a.how == null ? '' : String(a.how).trim();
    if (how && !ACQ_HOW_VALUES.includes(how)) {
      throw new Error('Начинът на постъпване „' + how + '“ не е познат на програмата (най-вероятно от стара база, '
        + 'редактирана на ръка, или от внесени данни отпреди въвеждането на списъка с позволени стойности). '
        + 'Партидата не може да бъде записана с тази стойност. Изберете стойност от падащото меню „Начин на '
        + 'постъпване“ — ' + ACQ_HOW_VALUES.join(' / ') + ' — и запазете отново. Начинът решава кой първичен '
        + 'документ се съставя по партидата (при дарение — акт за приемане на дарение по чл. 6, ал. 5).');
    }
  }
