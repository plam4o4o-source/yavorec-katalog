...
/* ЕДИН БРОЙ — ЕДИН РЕД (v2.4.61, находка 4 от одита).
   ...
   За вече работещите бази индексът се създава от миграция 16 в main.js — там с
   изрична проверка за заварени дубликати, защото върху тях CREATE UNIQUE INDEX
   се проваля и не бива да спира стартирането на програмата. */
CREATE UNIQUE INDEX IF NOT EXISTS idx_per_issue_unique ON periodical_issues(periodical_id, issue_no, date);
