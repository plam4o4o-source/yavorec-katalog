...
/* Таблица за авторски знак — буквосъчетание и числото срещу него („ВАЗ“ → „14“).
   ИДВА С ПРОГРАМАТА от v2.4.63: db/author-table.js се зарежда тук при стартиране,
   но САМО ако таблицата е празна (виж initDb() в main.js). Библиотека със свое
   издание внася своя файл и той застава върху вграденото — внесеното никога не
   се презаписва само (handlers/author-mark.js). Празна таблица означава, че
   вграденият файл липсва или е бил изтрит нарочно: тогава копчето „Предложи“ до
   полето „Авторски знак“ просто няма откъде да предлага. */
...
/* `code` (v2.4.61) — НЕПРОМЕНЛИВ ключ на началните видове документи.
   ===========================================================================
   ДОТУК видът документ се разпознаваше от програмата по БУКВАЛНОТО си име:
   handlers/dnevnik.js преобразуваше „продължаващо издание“ в реда „Периодични
   издания“ на Раздел Б, а handlers/periodicals.js търсеше същия низ, за да
   впише годишния комплект във фонда. Името обаче е на библиотекаря — екранът
   „Категории“ позволява преименуване, а Наредба № 3 не предписва етикетите.
   Едно преименуване чупеше и двете: Дневникът започваше да брои заетия годишен
   комплект на вестник в „Книги“ (резервната стойност), а инвентирането не
   намираше вида и СЪЗДАВАШЕ ВТОРИ със старото име — фондът излизаше с два вида
   за една и съща периодика в КДБФ Приложение № 2 и в годишния отчет, а
   събирането им оставаше ръчна работа на човека, който чете справката.
   Затова: името остава за хората, кодът — за програмата. Код имат само
   началните видове (виж INSERT-а най-долу и миграция 16 в main.js за вече
   работещите бази); вид, създаден от библиотекаря, няма код и нищо в програмата
   не зависи от него. UNIQUE допуска много NULL-а в SQLite — точно каквото
   трябва тук. */
...
CREATE TABLE IF NOT EXISTS deaccession_acts (
  ...
  committee2   TEXT,
  committee3   TEXT,
  -- Анулиране (v2.4.56). Актът е документ по чл. 39 и НЕ се трие: анулирането
  -- само го отбелязва. Редът остава, номерът остава зает завинаги и не се дава
  -- на друг акт, а КДБФ Приложение № 3 го показва зачертан, с основанието и с
  -- нула в сборовете. Всяко броене на отчислени минава през revoked_at IS NULL.
  revoked_at    TEXT,
  revoke_reason TEXT,
  revoked_by    TEXT,
  -- Препратка към документа, от който е роден актът (v2.4.61). Най-често това е
  -- протоколът от инвентаризация по чл. 40: „Съставен от протокол за
  -- инвентаризация № 3 от 12.05.2026 г.“. Пренася се от проекта
  -- (deaccession_drafts.note) при утвърждаването и се ПЕЧАТА в самия акт — иначе
  -- актът по чл. 30, т. 6 и протоколът по чл. 40 стоят като два несвързани
  -- документа и проверяващият няма как да мине от единия към другия.
  note          TEXT,
  -- Кой и кога е СЪСТАВИЛ акта (v2.4.61). Анулирането отдавна се подписва
  -- (revoked_at/revoked_by), а съставянето — не: за отпадналия акт се знаеше
  -- повече, отколкото за действащия. „Съставил“ е името на библиотекаря от
  -- настройките — програмата няма вход с парола и не се преструва, че има.
  created_at    TEXT,
  created_by    TEXT
);
CREATE INDEX IF NOT EXISTS idx_acts_year ON deaccession_acts(year);

-- ПРОЕКТ за акт (v2.4.56). Не е документ: няма номер, нищо не е отчислено и
-- документите стоят във фонда. Съществува, защото утвърденият акт е вечен —
-- грешките трябва да имат къде да се случат преди него. Отделна таблица, за да
-- не се налага никое от шестте места, които четат „акт“, да знае за проектите.
...
CREATE TABLE IF NOT EXISTS deaccession_items (
  ...
  category    TEXT,
  language    TEXT,
  -- Брой екземпляри КЪМ МОМЕНТА НА АКТА. Редът е снимка (чл. 35, ал. 2) и точно
  -- затова носи и собствена цена — бройките трябва да са снимани по същата
  -- причина. Ако се четяха живо от inventory, редакция на „Налични бройки" през
  -- 2026 г. би променила отпечатания КДБФ за 2024 г. със задна дата, а изтриване
  -- на документа (ON DELETE CASCADE върху inventory) би свило стар акт.
  quantity    INTEGER
);
...
CREATE TABLE IF NOT EXISTS loans (
  ...
  -- Номерът на акта, закрил принудително това заемане при отчисляване на документа.
  -- Позволява анулирането на акта да отвори заеманията обратно, а на статистиката —
  -- да ги изключи от „върнати със забава“ (книгата никога не се е връщала).
  -- Колоната се създаваше САМО от миграция в handlers/deaccession-acts.js, затова на
  -- чиста база не съществуваше, докато онзи модул не се регистрира — и всеки друг
  -- модул, който я ползва, гърмеше според реда на зареждане. Мястото ѝ е тук;
  -- миграцията остава заради вече създадените бази.
  deaccession_act_id INTEGER,
  -- ИЗГУБЕН ИЛИ НЕВЪРНАТ ОТ ЧИТАТЕЛЯ ДОКУМЕНТ (v2.4.56). Заемането се ЗАТВАРЯ с
  -- date_in (иначе документът виси зает завинаги), но книгата никога не се е
  -- върнала — затова е нужна изрична отметка, а не досещане по дати. Тя пази
  -- статистиката („спазени срокове“ не бива да брои като върната книга, която я
  -- няма) и свързва трите неща, които дотук се правеха поотделно и без връзка:
  -- уреждането с читателя, начислението в сметката му и актът по чл. 30, т. 5.
  lost                     INTEGER,   -- 1 = приключено като изгубено
  lost_date                TEXT,
  lost_resolution          TEXT,      -- обезщетение | замяна с идентичен | замяна с равностоен
  lost_amount              REAL,
  lost_account_line_id     INTEGER,   -- редът в читателската сметка, ако има начисление
  lost_replacement_book_id INTEGER,
  lost_replacement_note    TEXT,
  lost_note                TEXT,
  -- Колко от loans.fine е начислено от САМИЯ акт за отчисляване (v2.4.61).
  -- Актът по чл. 30, т. 5 затваря заемането на невърнат документ и начислява
  -- забавата до деня на акта. Анулирането отваря заемането обратно и трябва да
  -- върне ТОЧНО толкова: инак „Просрочени“ добавя дните от падежа до днес към
  -- вече записаното и една и съща забава се начислява два пъти. Пресмятане
  -- наново не става — тарифата и календарът може да са се променили междувременно.
  deaccession_fine         REAL,
  -- Редът в account_lines, с който актът е начислил тази забава на читателя
  -- (v2.4.61). Забавата се води на ДВЕ места, защото двете отговарят на различни
  -- въпроси: loans.fine казва „колко дължи по ТОВА заемане“, а account_lines е
  -- сметката на читателя — единственото, което гишето показва като „Дължи по
  -- сметка“ и което годишният отчет разнася като „Събрани обезщетения“. Номерът
  -- на реда се пази, за да може анулирането на акта да го махне, ако по него още
  -- не е платено нищо; платеното не се пипа никога.
  deaccession_fine_line_id INTEGER
);

-- Желязна гаранция срещу двойно заемане на ниво база данни.
-- ЕДИН ИНВЕНТАРЕН НОМЕР = ЕДИН ЕКЗЕМПЛЯР: екземплярите на едно заглавие са
-- ОТДЕЛНИ редове в books, всеки със свой инвентарен номер, точно както са в
-- инвентарната книга. `inventory.quantity` е 1 за всеки жив ред; програмата
-- не създава друга стойност (виж normalizeQuantity в handlers/books.js), а
-- стари редове с друга бройка се поправят от „Настройки“ → „Проверка на данните“.
-- Правилото остава изразено като "активните заемания не надвишават quantity" и
-- се пази с тригер, а не с UNIQUE индекс, защото индекс би забранил и
-- завършените заемания на същия екземпляр (историята е в същата таблица).
-- (До v2.4.20 полето беше свободно число и коментарът тук говореше за
-- „легитимни втори бройки“ — сгрешено.)
...
CREATE TABLE IF NOT EXISTS inventory_sessions (
  ...
  at_binder       INTEGER,
  -- ПРОВЕРЕНИТЕ към приключването — броят сканирания, които СА ОЩЕ В ОБХВАТА.
  -- Сканиранията са всичко, което комисията е минала, а обхватът се смята наново
  -- при приключване: документ, отчислен или преместен в друг отдел, докато
  -- проверката тече, вече не е в него. Дотук се печаташе суровият брой сканирания
  -- и четирите числа надхвърляха обхвата („в обхвата 9 · проверени 6 · липсващи 4“).
  scanned_final   INTEGER,
  -- ПРОЦЕНТЪТ НА СВОБОДЕН ДОСТЪП КЪМ ПРИКЛЮЧВАНЕТО (v2.4.67) — от него зависи
  -- допустимият естествен отпад по чл. 41 (0,5 % или 1 % от обхвата). Дотук се
  -- четеше от текущите настройки при ВСЯКО отпечатване: смяна на процента по-късно
...
CREATE TABLE IF NOT EXISTS inventory_session_missing (
  ...
  author      TEXT,
  price       REAL DEFAULT 0,
  -- СНИМКА НА БРОЙКАТА КЪМ ДЕНЯ НА ПРИКЛЮЧВАНЕТО (v2.4.61).
  -- Протоколът по чл. 40 – 41 брои библиотечни ДОКУМЕНТИ, а не редове: стар
  -- неразделен запис с inventory.quantity = 3 е три липсващи документа. Дотук
  -- бройката се четеше наживо от inventory при всяко отваряне на протокола —
  -- тоест поправка на „Налични бройки“ на вече липсващ документ променяше
  -- ВЕЧЕ ОТПЕЧАТАН и подписан протокол със задна дата. Тук се пази снимката
  -- към деня на приключването, по същата причина и по същия начин, по който
  -- deaccession_items.quantity пази снимката на акта (чл. 35, ал. 2).
  quantity    INTEGER
);
...
/* ЕДИН БРОЙ — ЕДИН РЕД (v2.4.61, находка 4 от одита).
   ===========================================================================
   ДОТУК нищо не пазеше кардекса от повторение: второ натискане на „Добави брой“,
   два пъти прочетен код или второто работно място вписваха същия брой отново.
   Цената му влиза в сбора за годината, а ТОЗИ СБОР се предлага като цена на
   годишния комплект при инвентирането — тоест дубликатът стига до инвентарната
   книга (чл. 16) и до КДБФ като стойност на документ, по-висока от платената по
   фактурата. При проверка това е разминаване между регистъра и първичния
   документ, което никой не може да обясни, защото причината е на две екрана
   разстояние.
   Проверката стои и в handlers/periodicals.js — там, за да каже на човешки език
   какво е станало; индексът е ТУК, защото двете работни места към обща мрежова
   база не се виждат едно друго и проверката в JavaScript между тях не струва
   нищо. Тройката (издание, № на брой, дата) е точната: същият номер с ДРУГА
   дата е друг физически брой (притурка, повторно издание) и минава.
   ИНДЕКСЪТ СЕ СЪЗДАВА ОТ МИГРАЦИЯ 16 в main.js, НЕ ТУК — по същата причина, по
   която и уникалният индекс на сканиранията стои в миграция 8 (виж бележката при
   inventory_session_scans по-горе). Първият вариант на тази поправка сложи
   CREATE UNIQUE INDEX тъкмо тук и това беше грешка: schema.sql минава в началото
   на initDb(), ПРЕДИ миграциите, а db.exec() спира на първата грешка. Върху
   заварена база с повтарящи се броеве — единствената, заради която проверката
   изобщо съществува — изявлението се проваля, целият файл пада и програмата
   минава през резервната пътека „изявление по изявление“ при ВСЯКО стартиране,
   с грешка в дневника, която никой не може да свърже с кардекса. Миграцията прави
   същото, но веднъж, с try/catch и с обяснение в одитната следа на човешки език.
   Нови бази минават през същата миграция (user_version 0 → 16). */

/* ПЕРИОДИКАТА Е ЧАСТ ОТ ФОНДА (v2.4.56).
   ===========================================================================
   ДОТУК: двете таблици по-горе бяха ЗАТВОРЕН СВЯТ. Нито `periodicals`, нито
   `periodical_issues` имаха инвентарен номер, нито `acquisition_id`, и нито
   един техен ред не влизаше в `inventory`. Следствието: handlers/kdbf.js и
   handlers/stats.js четат ФОНДА през `books`, тоест абонаментът за вестници и
   списания просто не съществуваше нито за КДБФ, нито за годишния отчет.

   ЗАЩО Е ГРЕШНО: самата програма твърди обратното на три места, които се
   отпечатват и подписват:
     • заглавието на КДБФ Част № 1 гласи „Регистриране на постъпили книги,
       ПЕРИОДИЧНИ ИЗДАНИЯ и други материали“ — а в разпечатката им няма нито ред;
     • Дневникът, Раздел Б, има готов ред „Периодични издания“, който винаги
       излизаше нула;
     • Наредба № 3 брои периодичните издания за библиотечни документи (чл. 13,
       ал. 3, т. 1; чл. 14; чл. 16) — тоест подлежат на регистрация точно както
       книгите.
   При проверка това не е „липсваща функция“, а системно ЗАНИЖЕНИ постъпления и
   занижена стойност на фонда: библиотека, която получава пет заглавия периодика
   годишно, отчита пет библиотечни документа по-малко всяка година, завинаги.

   ЗАЩО ПОПРАВКАТА Е ИМЕННО ТАКАВА: библиотечната практика (и начинът, по който
   статистиката брои) е броевете на ЕДНО издание за ЕДНА година да се подвързват
   и да се водят като ЕДИН библиотечен документ с ЕДИН инвентарен номер — не по
   един номер на всеки брой. Затова инвентираният обект не е нито изданието
   (то живее десетилетия), нито броят (той е част от комплекта), а ГОДИШНИЯТ
   КОМПЛЕКТ. Един ред тук = един ред в инвентарната книга.

   ЗАЩО ОТДЕЛНА ТАБЛИЦА, А НЕ КОЛОНА В `periodicals`: едно издание има МНОГО
   годишни комплекта (в-к „Труд“ 2023, 2024, 2025 — три отделни инвентарни
   номера). Колона `book_id` в `periodicals` би побрала само последния и би
   изтрила предишните при всяко ново инвентиране — точно обратното на това,
   което инвентарната книга изисква. UNIQUE(periodical_id, year) е самата
   защита срещу повторно инвентиране на една и съща година: тя пази правилото
   в БАЗАТА, а не само в проверката на обработчика, тоест държи и когато два
   компютъра пишат в обща мрежова база едновременно.

   `book_id` е ON DELETE SET NULL по вече установения в схемата образец
   (books.acquisition_id, deaccession_items.book_id): изтрит по погрешка картон
   не бива да отнася със себе си следата, че годината изобщо е била инвентирана.
   Ред с book_id IS NULL се чете като „не е инвентирана“ и позволява повторно
   инвентиране (виж handlers/periodicals.js) — иначе една грешка би заключила
   годината завинаги.

   `issue_count`/`issue_sum` са СНИМКА към момента на инвентирането, по същата
   причина, по която deaccession_items носи собствена цена: комплектът е
   подвързан и предаден; по-късно изтрит или добавен брой в кардекса не бива да
   променя със задна дата какво е било вписано в инвентарната книга. */
...
CREATE TABLE IF NOT EXISTS settings (
  ...
  -- Обезщетение за ИЗГУБЕН документ (v2.4.56). Нарочно БЕЗ DEFAULT: NULL значи
  -- „библиотеката не е пипала правилото“ и програмата пада към подразбиращото се
  -- (3 × цената, 10 € при документ без цена). При DEFAULT 3 нямаше да се различава
  -- библиотека, изрично решила тройния размер, от такава, която просто не е
  -- отваряла настройката — а разликата има значение в деня, в който
  -- подразбиращото се число се промени. Чл. 43, ал. 2 задължава обезщетяването,
  -- но размерът е решение на настоятелството, не на наредбата.
  lost_price_multiplier REAL,
  lost_fallback_amount  REAL,
...
-- Кодът е вътрешният, непроменлив ключ (виж коментара при CREATE TABLE categories):
-- по него Дневникът намира реда на Раздел Б и инвентирането на периодика намира
-- вида си, независимо как библиотекарят е преименувал самите етикети.
INSERT OR IGNORE INTO categories (name, code) VALUES
  ...
  ('видеодокумент', 'video'), ('електронен документ', 'electronic'), ('патент/стандарт', 'patent'), ('друго', 'other');
