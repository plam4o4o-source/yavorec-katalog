...
прозорец срещу истинска база (13 349 документа, 400 заети, 240 просрочени, 160
предстоящи връщания), а не с четене на кода.

**Таблото ставаше 9 830 px високо при екран от 768 — над девет екрана
превъртане.** Причината е една: списъкът с предстоящите връщания нямаше лимит, за
разлика от просрочените точно до него (`LIMIT 7`). Всяко предстоящо връщане
получаваше свой ред, при това с най-едрия шрифт на таблото за собствената си
дата — при положение че всички дати са в рамките на три дни, тоест най-силният
акцент отиваше за най-малко важното. Картата разтягаше и съседките си в същия ред.

Сега денят е заглавие на групата („Днес · 40“, „Утре · 40“), заглавието на
документа е главното на реда, показват се шест реда и бутон за останалите.
Обявяват се ВСИЧКИ дни с действителния си брой, дори когато редовете им не се
побират — така се вижда формата на следващите три дни, а не само днешният. Броят
идва от базата (`upcomingCount` и `upcomingByDay`), а не от преброяване на
показаните редове: иначе „Днес · 40“ би било вярно само докато списъкът не е
отрязан. **Измерено: 9 830 → 1 179 px.**

**„240 просрочени“ не казва какво да се направи.** Разликата между три дни и три
месеца е разликата между напомняне и акт по чл. 30. Показателят вече носи
разбивката по дни забава — до 7 дни, от 8 до 30, над 30 — като лента и като
числа. Под „Заети в момента“ застана микрографика на заеманията за последните
дванайсет седмици. И двете са ДОПЪЛНЕНИЕ към числата, не заместител: самите
стойности стоят и като текст, и в описанието за екранен четец, така че работят и
при монохромен екран, и при далтонизъм.

**Изискването по чл. 40, т. 2 е единственото на таблото с краен срок (31
декември), а стоеше на 2 500 px надолу** — последният ред в дъното на „Годината“,
като „0 / 1465“ и празна лента. Тоест виждаше се чак когато вече е късно. Сега е
собствен ред веднага под показателите, с пръстен, състояние спрямо календара
(„изостава“ / „в график“ / „изпълнена“) и, най-важното, темпото: **„Остават 112
дни до 31 декември — по 367 документа на месец, за да бъде изпълнено.“** Прагът е
изминалата част от годината, а не кръгло число: 8% през януари е в график, 60%
през декември — не. От „Годината“ отпадна, за да не стои едно число на две места.

**Покрай това — три неща, които се видяха чак при измерването:**

- **Колоните на таблото можеха да се изяждат една друга.** `1fr` значи
  `minmax(auto, 1fr)`: колона с дълго съдържание расте над своя дял. Дотук нищо на
  таблото не го задействаше; новият списък с групи по ден го задейства веднага —
  измерено при първата редакция на този кръг долният ред излезе 116 / 254 / 253 px
  вместо три по 338, а „Бързи действия“ се смачка до две колони от по 34 px с
  настъпващи един върху друг надписи. Поправено само за редовете на таблото
  (`.grid.g3.dashGrid`) — общият `.grid.g3` се ползва и във формулярите, където
  полетата имат собствена най-малка ширина и заковаването ѝ до нула би ги
  накарало да преливат. Тежестта има значение: правило само с `.dashGrid` е с един
  клас срещу двата на `.grid.g3` и не влиза в сила — точно този капан беше
  заложен, докато мутационната проверка не го извади.
- **Пръстенът се дели със „Справки“, вместо да се рисува втори.** `ringSvg()`
  приема свой цвят, когато „добре“ не значи „високо“, и вече не рисува дъга при
  0% — заоблена шапка с нулева дължина оставяше точка, която прилича на повреда.
  Повикването от „Спазване на сроковете“ е непроменено.
- **Кехлибареното в лентите беше втори, немерен цвят.** `#C98A2E` стоеше зашито в
  `.bar-fill.near`, докато проектът вече има проверен `--amber`. Обединени.

**Махнато, защото не се доказа:** втора проверка за обхвата на седмиците в
JavaScript, при вече отрязани от SQL редове. Мутационната проверка показа, че
премахването ѝ не променя нищо — два предпазителя за едно и също правят и двата
непроверими. Остана този, който освен това пази и от пълно сканиране на таблицата.

Проверки: 14 нови теста, 28 мутации (всички уловени). Пълната поредица: 1454
успешни, 0 неуспешни (UTC и Europe/Sofia); сайтът: 8 сценария + всички проверки
при 15 002 записа.

**EN:** The dashboard, all three changes found by MEASURING the screen in a real
window against a real database (13,349 documents, 400 on loan, 240 overdue, 160
upcoming returns).

- **The dashboard grew to 9,830 px on a 768 px screen** — over nine screens of
  scrolling — because the upcoming-returns list had no limit, unlike the overdue
  list right beside it. Every return got a row, with the largest type on the
  dashboard given to its date, though all dates fall within three days. Now the
  day is the group heading, the title is the row, six rows show, and every day is
  announced with its true count (from the database, not from counting the rows
  shown). **9,830 → 1,179 px.**
- **"240 overdue" does not say what to do.** The figure now carries a breakdown by
  days late — up to 7, 8–30, over 30 — plus a twelve-week sparkline under "on
  loan". Both supplement the numbers rather than replacing them: the values stay
  as text and in the screen-reader description.
- **The Article 40 requirement — the only thing on the dashboard with a deadline —
  sat 2,500 px down** as "0 / 1465" and an empty bar. It now has its own row under
  the metrics, with a ring, a calendar-relative state, and the pace needed:
  "112 days left until 31 December — 367 documents a month."
- Also found while measuring: the dashboard columns could eat each other (`1fr` is
  `minmax(auto, 1fr)`; the new grouped list triggered it — measured 116 / 254 / 253
  instead of three 338s);
  the ring is now shared with Reports instead of a second one being drawn, and no
  longer paints a dot at 0%; and a second, unmeasured amber was folded into the
  project's existing token.

Checks: 14 new tests, 28 mutations (all caught). Full suite: 1,454 passing, 0
failing (UTC and Europe/Sofia).

## v2.4.51

**BG:** Еврото става основната валута на програмата, а таблото се връща на
по-компактната си подредба.

### Еврото е основната валута

От 01.01.2026 г. лицевата валута на страната е еврото и официалните документи по
Наредба № 3 — инвентарната книга, КДБФ, актовете за отчисляване, протоколите от
инвентаризация — се водят в него. Дотук програмата пазеше сумите в **лева** и
смяташе еврото при показване. Разликата не е козметична: **сборът от закръглени
преобразувания не е равен на преобразувания сбор**, тоест инвентарната книга
показваше обща стойност, която не съвпада със сбора на собствените си редове —
точно това проверява регионалната библиотека. Сега всяка сметка се прави във
валутата на записа.

- **Стойностите в базата данни се преобразуват веднъж** при обновяването
  (миграция 15) по фиксирания и необратим курс **1 EUR = 1.95583 BGN** от
  Регламент (ЕС) 2025/1409, със закръгляне до цял евроцент. Преобразуват се
  точно девет колони: цените на документите, обявените стойности на партидите,
  цените в актовете за отчисляване и в протоколите от инвентаризация,
  обезщетенията по заеманията, редовете по сметките на читателите, цените на
  периодичните броеве, тарифата за забава и годишната такса.
- **Три колони НЕ се пипат**, макар да са числа с плаваща запетая: дните
  преустановено заемане (в правилата по категория и в Настройки) и двете
  разстояния в милиметри за листа с етикети. Преобразуването им би счупило
  наказанията и печата на етикети. Списъкът се съставя от самата схема на
  базата, а не по памет — нова парична колона утре ще провали проверката,
  вместо да остане в лева мълчаливо.
- **Показването е с еврото отпред, левът остава в скоби**: „12.00 € (23.47 лв.)“.
  Заварените документи и фактурите отпреди 2026 г. са в лева и хората още смятат
  в тях.
- **Въвеждането приема и двете валути.** Навсякъде, където се вписва сума — цена
  на документ, обща стойност на партида, плащане и начисление по сметка на
  читател, цена на периодичен брой, двете тарифи в Настройки — вече има **две
  полета: в евро и в лева**. Пишете в което ви е удобно, другото се пресмята
  веднага. При стара фактура или опис пишете сумата в дясното поле точно както
  стои на документа. Записва се винаги стойността в евро; полето в лева няма име
  и не стига до базата, тоест нито един канал не се променя заради него.
- Официалните бланки, изнасянето в CSV и одитната следа минаха в евро. В CSV
  общата стойност се смята от записаната валута, не от преобразуваната — иначе
  сборът в Excel не съвпада със сбора в програмата.

**Прегледайте двете тарифи.** Закръгляването до цял евроцент се вижда при
дребните суми: **0.05 лв./ден забава става 0.03 €/ден** (≈ 0.06 лв.), а годишна
такса от 6.00 лв. става 3.07 €. Настройки → „Обслужване“ вече го казва наяве —
задайте стойностите както е решило настоятелството, защото програмата няма как
да знае каква е новата тарифа. Резервното копие отпреди обновяването остава в
папката с копията.

Онлайн каталогът не е засегнат: той никога не е съдържал цени.

### Таблото се върна на по-компактната подредба

„Бързи действия“ отново е карта в долния ред заедно с „Предстоящи връщания“ и
„За днес“, вместо лента на цяла ширина със собствен ред (както беше преди
v2.4.29). Лентата вдигаше таблото със 145 px, без да показва нищо повече —
измерено в истински браузър при 1366×768: **1115 → 970 px височина, а
превъртането 436 → 291 px**. Шестте копчета се редят 2×3 в своята колона;
всичките шест остават.

### Намерени при прегледа на кръга и поправени в него

Шест неща, всяко в код, до който първите проверки не стигаха:

- **Напомнителните писма деляха повторно по курса.** Обезщетението вече е
  записано в евро, а писмото, имейлът и SMS-ът продължаваха да го делят —
  задължение от 2.05 € излизаше като „2.05 лв. (1.05 €)“ в имейла и „2.05 € /
  4.01 лв.“ на хартиеното напомняне. Три различни числа за едно задължение, при
  това в документа, който стига до читателя.
- **Фантомно салдо по погасена сметка.** Закръгляването ред по ред се натрупва:
  +0.05, +0.05, −0.10 лв. (баланс нула) даваше +0.03, +0.03, −0.05 = **+0.01 €**,
  тоест читател, който не дължи нищо, светваше в червено и не можеше да бъде
  изтрит. Сметките вече се преобразуват **кумулативно по читател** — всеки ред се
  смята така, че текущият баланс след него да е точното закръгляне на стария.
- **Обявената стойност се разминаваше със сбора.** Партида от 5 × 2.99 лв. с
  обявени 14.95 лв. (съвпадащи) даваше 7.65 € срещу сбор от преобразуваните цени,
  и печатът добавяше абзаца „обявената стойност се различава от сбора“ върху
  **вече подписан** акт за дарение. Сега партида, в която двете са съвпадали,
  продължава да съвпада; разминаване от самия документ се пази като разминаване.
- Отказът да се изтрие читател с дълг, двете съобщения при въвеждане на сума по
  сметка и грешката за цена на периодичен брой все още казваха „лв.“ за сума,
  която вече е в евро.

### Проверка

Миграцията е проверена срещу **истинска база**, пусната през самия `main.js`:
всяка от деветте колони поотделно, празната цена остава празна (не става 0),
отрицателните редове по сметките се преобразуват вярно и балансът на читателя
пада точно на нула, а не на стотинка остатък; трите непарични колони са
непокътнати. Всяка нова проверка е доказана с мутация: върната поправка →
съответната проверка пада.

Пълната поредица: 1440 успешни, 0 неуспешни (UTC и Europe/Sofia); сайтът: 8
сценария + всички проверки при 15 002 записа. Мутационна проверка: 33 върнати
поправки → 33 паднали проверки, 0 непокрити.

**EN:** The euro becomes the program's primary currency, and the dashboard
returns to its more compact layout. Since 1 January 2026 the country's currency
is the euro and the official records under Ordinance No 3 are kept in it, while
the program still stored amounts in **leva** and converted for display. That is
not a cosmetic difference: the sum of rounded conversions does not equal the
converted sum, so the inventory book showed a total that did not match the sum
of its own rows. Migration 15 converts the stored values once, at the fixed and
irreversible rate of 1 EUR = 1.95583 BGN, rounded to the euro cent — exactly
nine money columns, while three further floating-point columns (suspension days
and two label-sheet measurements in millimetres) are deliberately left alone;
the list is derived from the schema itself rather than from memory. Amounts now
read euro-first with leva in brackets, and every place that takes a sum has
**two fields, euro and leva**, so an old invoice can be typed exactly as
written — only the euro value is stored. Printed forms, the CSV export and the
audit trail moved to euro as well, with CSV totals computed from the stored
currency so the spreadsheet agrees with the program. **Review the two tariffs:**
rounding to the cent is visible on small amounts — 0.05 BGN/day late fee becomes
0.03 EUR/day — and Settings now says so. On the dashboard, "Quick actions" is
once again a card in the lower row rather than a full-width strip on its own row
(the pre-v2.4.29 arrangement): measured in a real browser at 1366×768, the page
went from 1115 px to 970 px, and scrolling from 436 px to 291 px. The migration
was verified against a real database driven through `main.js` itself, including
that empty prices stay empty and a reader's balance settles to exactly zero.
Full suite: 1436 passing, 0 failing (UTC and Europe/Sofia).
...
поправките от тридесет и третия кръг (v2.4.44).

**Телефонът: камерата мълчеше и никой не казваше защо.**

- **„Пусни камерата“ не питаше за разрешение и не правеше нищо.** Причината не е
  в програмата, а в правилата на браузъра: Chrome пази разрешението за камера
  **по адрес на страницата**. Страницата за сканиране се прехвърля на телефона и
  се отваря оттам като файл (`file://`, а от Вайбър — `content://`) — такъв
  адрес няма къде да запише разрешение, затова заявката се отказва, **без
  телефонът изобщо да попита**. Наръчникът пък казваше „разрешете достъп до
  камерата“ — разрешение, което няма как да се даде. Сега страницата го **казва
  предварително**, а не след натискането, и то с думи, а не с текста на браузъра
  („Permission denied“, а при друг случай — „Cannot read properties of
  undefined“).
- **Добавено е „Снимай баркод“ — и то работи от файл.** Снимката минава през
  самото приложение „Камера“ на телефона, което си има собствено разрешение и не
  зависи от адреса на страницата. Един кадър с два етикета добавя двата, а
  повторенията в него се броят веднъж. Живата камера остава за случаите, в които
  наистина работи: на компютър и когато страницата е отворена по `https://`.
- Проверено в истински браузър (Chromium, Playwright): на `file://` страницата е
  „сигурен контекст“ и `navigator.mediaDevices` СЪЩЕСТВУВА — тоест проверка „има
  ли камера“ не лови случая; решаващ е адресът. Затова проверката е по адрес.

**Телефонът: името на библиотеката вече идва от настройките навсякъде.**

- Дотук то стоеше само в лентата вътре в страницата. Сега влиза и в **заглавието
  на раздела** (оттам го взима иконата, ако страницата се добави на началния
  екран), и в **предложеното име на файла**
  (`inventarizaciya-skener-<библиотека>.html`), и в името на изнесения списък.
  На телефона на библиотекаря стоят и други файлове, а две библиотеки изглеждаха
  еднакво.
- Името се **екранира**, а не се реже: „Читалище Х&Y“ оставаше „Читалище ХY“.
  Заместването вече е с функция — име, съдържащо `$&` или `` $` ``, вкарваше
  части от самата страница в заглавието ѝ. Непопълнени настройки не оставят
  празна лента.

**Преглед на v2.4.44 — три реални дефекта, всеки възпроизведен:**

- **Пазачът срещу двойно вписване пропускаше истинското двойно щракване.** Той
  спираше само две повиквания в един и същ синхронен тик; между две щраквания на
  човек минават 60–150 ms и дотогава записът вече е приключил, а прозорецът се
  маха чак 140 ms след затварянето — тоест бутонът оживяваше **върху още видимия
  прозорец**. Измерено с истинска форма: „Плати“ с 4,50 лв. пращаше `account.pay`
  два пъти при всяка пауза до 140 ms. Собственият тест на кръга щракваше два пъти
  в един тик и затова минаваше и с дупката. Поправено на две места: затварящ се
  прозорец вече не приема натискания (`pointer-events:none`), а пазачът не
  връща към живот бутон в затварящ се прозорец и не пуска ново вписване от
  форма, която си отива (задържан Enter се повтаря на ~30 ms).
- **Прегледът преди печат стана недостъпен с клавиатура.** Новият капан за Tab
  познаваше трите завеси, но не и прегледа, който се отваря ВЪРХУ отворен
  прозорец („Квитанция“ в сметката на читателя): Tab дърпаше фокуса в прозореца
  отдолу и „Печат…“, „Запази PDF…“ и „Отказ“ не се стигаха — в кръга, който беше
  за достъпност.
- **Списъкът с проверките показваше друго число, различно от протокола.**
  Приключването вече пази снимка на проверените срещу обхвата (`scanned_final`),
  но списъкът четеше суровия брой сканирания — до самия бутон „Протокол“, който
  печата поправеното. Текущите проверки и сесиите отпреди снимката се броят
  както досега.
- Махнати са и две функции, останали без нито едно повикване след
  пренаписването на търсенето на дублети — бележката за симетрията стоеше върху
  тях.

Проверено и НЕ прието за дефект: списъкът, копиран от телефона, съдържа само
номера (програмата чете всяка дума като номер) — затова името на библиотеката е
в името на файла, не в съдържанието; и запомнянето на сканираното от `file://`
работи (проверено в Chromium: преживява презареждане).

Проверки: 22 нови теста (общо 1380), всеки проверен с мутация в двете посоки.
Пълната поредица: 1380 успешни, 0 неуспешни (UTC и Europe/Sofia); сайтът: 8
сценария + всички проверки при 15 002 записа.

**EN:** Phone scanning finally explains itself, plus a review of round
thirty-three (v2.4.44). **The camera never asked for permission** — and could
not: Chrome keys camera permission to the page's address, and the scanner page
is transferred to the phone and opened as a local file, which has no address to
grant permission to, so the request is refused without any prompt (the manual
meanwhile said "allow camera access"). The page now says so upfront, in plain
words, and offers **"Снимай баркод"** — a photo taken through the phone's own
Camera app, which carries its own permission and therefore works from a file;
live camera remains for `https://` and desktop. Verified in real Chromium: on
`file://` the page IS a secure context and `navigator.mediaDevices` exists, so a
feature check does not catch this — the address does. **The library name from
settings** now also reaches the page `<title>` (and thus the home-screen icon)
and the suggested file name, is HTML-escaped instead of stripped, and is
substituted with a function so a name containing `$&` cannot inject the page
into its own title. Review findings, each reproduced: the double-submit guard
still allowed a real double click (it only blocked same-tick calls, while the
dialog stays on screen for 140 ms after closing — measured: 4.50 lv. paid
twice); the new Tab trap made the print preview unreachable by keyboard when it
opens over a dialog; and the sessions list showed the raw scan count next to the
button printing the corrected protocol. 22 new tests (1 380 total), each
mutation-checked.

## v2.4.44

**BG:** Пълна проверка на работата — не по код, а по употреба: истински бази при
реален мащаб (15 000 книги, 15 208 различни автора) и истински прозорец в
браузър. Нищо тук не е „намерено при четене“: всяко от изброените е
възпроизведено, преди да бъде поправено, и всеки тест е проверен с мутация.

**Пари и официална статистика: едно щракване значеше две вписвания.** Прозорецът
се маха 140 ms след натискането — през това време бутонът стоеше жив. Измерено:
„Плати“ с 4,50 лв. пращаше `account.pay` ДВА пъти и читателят се кредитираше с
9,00 лв.; „Впиши посещения“ пращаше `visits.add` два пъти, а сборът е сумарен —
50 посещения ставаха 100 в статистиката за общината, без грешка и без второ
съобщение. Същото важеше за всяко записване във фонда, КДБФ, актовете,
краезнанието, МЗС и периодиката. Сложен е пазач около 20-те действия за
записване: второто повикване се пренебрегва, а натиснатият бутон се заключва,
докато първото приключи. Пазачът е и по-долен от бутона: при Enter в поле или
повикване от код фокусът не е върху бутон и работи само той.

**Протоколът по чл. 40 не се събираше.** Обхватът се смята наново при
приключване (документ, отчислен или преместен в друг отдел, докато проверката
тече, вече не е в него), а „проверени“ се печаташе като суров брой сканирания.
Резултат: „в обхвата 9 · проверени 6 · липсващи 4“ — десет от девет, в подписан
документ. Проверените вече се броят СРЕЩУ обхвата и се снимат в базата
(`scanned_final`, миграция 14), както отдавна се прави с `pool_final`; излезлите
от обхвата се съобщават отделно при приключване, вместо да се скриват. Старите
приключени сесии остават с NULL и печатат както досега — подписаният протокол е
това, което е било отпечатано тогава.

**Вносът от файл си измисляше баркодове.** При файл без колона „Баркод“ вносът
слагаше инвентарния номер като баркод. Точно това е сблъсъкът, който проверката
във формата пази: заварен документ с етикет „700“ и внесен документ с инв. № 700
стават неразличими при сканиране и програмата отказва И ДВАТА — седмици по-късно
и далеч от причината. Празното поле вече си остава празно, а вносът минава през
същата проверка, която пази ръчното въвеждане.

**Редакция на изтрит от другаде запис минаваше мълчаливо.** Работата на две
работни места към обща мрежова база е изрично поддържана, тоест изтрит от
другаде ред е нормален случай. Дотук UPDATE-ът просто не намираше какво да
промени, повикването връщаше „записано“, а в одитната следа влизаше редакция,
каквато не се е случвала. Сега отказва с обяснение — при читателите и при трите
раздела на краезнанието.

**„Проверка на данните“ стоеше залепнала при истински фонд.** Хлабавото търсене
на дублирани имена сравняваше всяко име с всяко и смяташе думите наново при
всяка двойка; коментарът в кода твърдеше, че „стойностите са няколкостотин“. При
15 000 книги различните автори са ~15 000, а не няколкостотин. Измерено: 2 мин.
21 сек., през които прозорецът не се прерисува и програмата изглежда увиснала.
Сега се сравняват само имената, които делят поне една пълна дума (а имената само
от инициали — с всички): 706 ms при същите данни, при непроменен резултат.
Покрай това се оказа, че сравнението е било НЕСИМЕТРИЧНО — „Иванов“ ≈
„Г. Иванов“ излизаше вярно, а обратното невярно, тоест дали двата записа изобщо
ще бъдат предложени за сливане, зависеше от това кой от двата има повече книги.

**Бележката при заемане не се виждаше никъде.** Вложеният низ в списъка с
читатели беше с единични кавички: подсказката на значката гласеше буквално
„${esc(r.alert_note)}“. Значката казва само „бележка“ — тоест текстът, заради
който тя съществува, не се четеше нито в списъка, нито от подсказката.

**Кардексът не се опресняваше при ✕ и Esc.** Опресняването висеше на един бутон,
а прозорецът има три изхода: току-що вписан брой продължаваше да се води
закъснял, докато разделът не се отвори наново. Закачено е на самото затваряне.

**Още:** „Баркод етикети“ беше единственият раздел, който гърми при отказ от
базата и оставя заглавието си върху предишния екран; вече въведен автор не се
предлагаше за автодовършване до следващото пускане на програмата (списъкът се
чете веднъж и не се забравяше след запис на книга, внос или сливане).

**Графично и достъпно — измерено, не на око:**

- **Официалните документи излизаха цветни.** `body` носи цвета на темата, а
  „Запази PDF…“ печата фоновете: инвентарната книга, която се прошнурова и
  заверява по чл. 26, ал. 2, излизаше кремава, синкава или розова според това коя
  тема е избрана на екрана. Хартията вече е бяла при всяка тема.
- **Отпечатаната читателска карта не приличаше на прегледа.** Картата носи
  класовете `lbl` и `rcard`, а правилото за `lbl` (центриране — вярното за баркод
  етикет) влиза в `<style>` СЛЕД този файл и при еднаква тежест печели.
  Измерено: цветната лента със заглавието излизаше 64 px на място x=138 вместо
  338 px през цялата карта. Тоест на екрана се одобряваше една карта, а от
  принтера излизаше друга.
- **Пръстенът на фокуса практически го нямаше.** Беше в светло бронзово: между
  1,63:1 и 2,42:1 върху хартията в шест от седемте теми при изискване 3:1 — видим
  само в тъмната лента. Сменен е с тъмно бронзово (най-малко 5,69:1), а в самата
  лента остава светъл, където е обратното.
- **Приглушеният текст в лентата** беше зашит `#8A8168` — 2,67:1 върху темата по
  подразбиране при изискване 4,5:1, а лентата стои на всичките 24 екрана. Вече е
  променлива, еднаква за всички теми, най-лошото 4,71:1; тест я мери за всяка
  тема, за да не се счупи при добавяне на нова.
- **Обходът с Tab излизаше от прозореца в менюто ЗАД затъмнението**, където Enter
  отваря друг раздел, а попълнената форма остава отворена и невидима зад него.
  Обходът се затваря в кръг и в трите слоя (форма, помощен прозорец, въпрос), а
  прозорците се обявяват като диалози и за екранния четец.
- **В Дневника току-що записана клетка нямаше признак на фокус** — „записано“
  идваше след „:focus“ при еднаква тежест и го надвиваше, точно когато човек се
  връща да я поправи. Освен това „записано“ се предаваше единствено със зелен цвят;
  сега носи и отметка, видима при монохромен екран и при далтонизъм.
- **„Намалено движение“ пропускаше три движения** (бързите бутони, картоните в
  „Персоналии“ и „Летопис“, пълзящата лента на заетостта) — настройката иска
  движението да е изключено, а не намалено.
- **Полето „Екземпляр“ в картона на книгата** беше изключено поле, в което
  надписът се режеше с 93 px и се четеше „1 — един инвентарен номер, ед“, без да
  може да се превърти или маркира. Показва се като изречение.

**Махнато, защото не се доказа.** Пропускането на вече слетите двойки при
търсенето на дублети (`find(i) === find(j)`) не издържа мутационната проверка:
измерено при 15 000 автора и 208 имена само от инициали, повикването отнема
706 ms с него и 643 ms без него. Правилото на този проект е да няма непроверена
находчивост в кода.

**Намерено и СЪЗНАТЕЛНО непоправено в този кръг** (изисква решение, не поправка):
(1) страничната лента не се събира на екран 768 px височина при мащаб 125 % —
нужно е решение как да се свива менюто, а не козметика; (2) картонът на книгата
се превърта вътрешно — 31 полета в един прозорец не се побират по друг начин без
да се раздели на страници; (3) бутоните на реда в „Читатели“ излизат извън екрана
при 1366 px — вложеното превъртане работи, но е неудобно; (4) повтарящи се
селектори за `.chk` и `.badge` в стиловете — подредба без разлика на екрана.

Проверки: 24 нови теста, 34 мутации (всички уловени). Пълната поредица:
1358 успешни, 0 неуспешни; сайтът: 8 сценария + всички проверки на изгледа при
15 002 записа.

**EN:** A full working audit — by use, not by reading code: real databases at
real scale (15,000 books, 15,208 distinct authors) and a real browser window.
Nothing here was "spotted while reading": every item was reproduced before it was
fixed, and every test is mutation-checked.

- **Money and official statistics: one click meant two entries.** The dialog is
  removed 140 ms after the press and the button stayed live. Measured: a 4.50 BGN
  payment sent `account.pay` twice (the reader was credited 9.00); "record visits"
  sent `visits.add` twice and the handler sums — 50 visits became 100 in the
  municipal statistics, with no error and no second message. A guard now wraps all
  20 saving actions.
- **The Article 40 inventory protocol did not add up.** The scope is recomputed at
  closing while "checked" printed the raw scan count: "in scope 9 · checked 6 ·
  missing 4" — ten out of nine, in a signed document. Checked is now counted
  against the scope and snapshotted (`scanned_final`, migration 14); items that
  left the scope during the check are reported separately instead of hidden.
- **File import invented barcodes** equal to the inventory number when the file had
  no barcode column — exactly the collision that makes scanning refuse both
  documents, weeks later and far from the cause.
- **Editing a record deleted from another workstation passed silently**, returning
  "saved" and writing an audit-trail entry for an edit that never happened. Two
  workstations against one shared database is an explicitly supported mode.
- **"Data check" froze on a real fund**: loose duplicate search compared every name
  with every other — 2 min 21 s at 15,000 distinct authors. Now 706 ms, same
  result. It was also asymmetric: whether two records were offered for merging at
  all depended on which of them had more books.
- **The lending note was invisible**: single quotes around a nested template made
  the tooltip read `${esc(r.alert_note)}` literally.
- Graphics and accessibility, all measured: official documents printed in the
  theme's colour instead of white; the printed reader card did not match its
  preview (64 px title bar at x=138 instead of 338 px across); the focus ring was
  1.63:1–2.42:1 on paper in six of seven themes; dimmed rail text was 2.67:1;
  Tab escaped the dialog into the menu behind the veil; a just-saved journal cell
  showed no focus and used colour alone; "reduced motion" missed three
  transitions; the "copy" field clipped its own sentence by 93 px.
- **Removed because it did not prove itself**: the already-merged pair skip in
  duplicate search survived mutation testing — 706 ms with it, 643 ms without.
- **Found and deliberately NOT fixed** (needs a decision, not a patch): the rail
  does not fit a 768 px screen at 125 % zoom; the book form scrolls internally
  (31 fields); row buttons in Readers go off-screen at 1366 px; duplicate `.chk`
  and `.badge` selectors (tidying with no visual difference).

Checks: 24 new tests, 34 mutations (all caught). Full suite: 1358 passing, 0
failing; site: 8 scenarios plus all view checks at 15,002 records.
...
пълния съкратен указател, извлечен от официалните страници на UDC Summary
(българска версия) и хармонизиран с националното издание от 2017 г. Досегашните
164 кода стават **1920**, плюс **739 общи определителя** по шестте спомагателни
таблици.

Прозорецът за избор е пренаписан, защото 1920 реда наведнъж са непрегледни:

- **Дърво по самата класификация.** Разгръща се „8 → 82 → 821 → 821.163.2“,
  свито в началото, с брой подраздели срещу всеки клон. Родителят на един код е
  **най-дългият съществуващ код, който стои в началото му** — така дървото следва
  схемата, вместо да се гади по точки или по дължина; отделен тест минава по
  всичките 1911 връзки.
- **Търсенето минава и през списъците „Включва:“.** Това се оказа решаващо:
  думата, която библиотекарката ще напише, често изобщо не е в заглавието на реда.
  „Пчеларство“ го няма в заглавието на `638` („Отглеждане и развъждане на насекоми
  и други членестоноги“) — стои в „Включва“. Само по заглавията половината
  търсения биха връщали „няма намерено“. Показва се и пътят до намереното
  (`6 → 63 → 638`), а самото „Включва“ се разкрива на намерения ред.
- **Съставният код се СГЛОБЯВА.** „94“ + определителя за място „(497.2)“ =
  `94(497.2)`. Изданието нарочно не носи такива кодове наготово — те се правят по
  правилата за комбиниране, и сега програмата го прави наяве, с изписано от какво
  е сглобен кодът и с „Назад“ за последната стъпка. Така се получава и
  `821.163.2-053.2` (българска литература за деца).
- **Бързи препратки** към 17-те кода, които изданието не носи като готови редове,
  а този фонд ползва: `82-93` детска литература, `821.163.2-31` български роман,
  `94(497.2)` история на България, `271.2` Православие, `793` танци и др. Те са
  отбелязани отделно в данните — изданието е едно, добавките на проекта са друго.
  Отделен тест изброява всичките 109 стари кода и пада, ако някой изчезне.

Три неща, поправени покрай новото — и трите намерени, като се погледна
резултатът, а не кодът:

- **Празната лента наследяваше чужд вид.** Класът `.empty` вече е зает от общия
  празен изглед („тук няма записи“) с икона на отворена книга и 36 px отстояние —
  лентата със сглобения код показваше книжка над надписа. Преименуван на
  `.udkBlank`.
- **Залепената лента оставяше ивица, през която минаваше съдържанието.**
  `position:sticky; top:0` в прозорец с `padding:16px` залепва **под** отстоянието,
  не в горния ръб; измерено — лентата стоеше на 100 px при ръб 84 px. Оправено с
  `top:-16px` заедно с отрицателните полета (и двойка стойности за ниските екрани).
- **Кодът в `onclick` минава през `jsq()`.** 69 определителя за време са в кавички
  („19“ = ХХ век) и 41 езикови носят обратен апостроф. Собственият тест на проекта
  срещу обърнатото екраниране `esc(x).replace(/'/g,…)` хвана моя ред — точно
  затова съществува.
...
коментар на `src/udk.js` по средата и би оставило суров, изпълним код в готовия
файл. Проверката е пренесена в пренаписания генератор и вече покрива и новото
поле `license_check`.

Данните са в `src/udk.json`, а `src/udk.js` се прави с `node tools/build-udk.js` —
следващото издание се подменя, без да се пипа JavaScript.

Проверки: 18 нови теста (общо 1332), всеки проверен с мутация (29 мутации, всички
уловени). Прозорецът се сглобява за ~30 ms с всичките 1920 кода.

**EN:** Round thirty-one — **the full UDC table**: 1 920 codes and 739 common
auxiliaries, replacing the 164-code selection. The picker is rebuilt as a
collapsible tree following the scheme itself (a code's parent is the longest
existing code that prefixes it), with search that also covers the "Включва:"
(includes) lists — decisive in practice, since the word a librarian types is
usually there and not in the heading ("пчеларство" is absent from 638's heading).
Composite codes are now assembled explicitly ("94" + "(497.2)" = 94(497.2)),
which is how the edition expresses what used to be canned rows; 17 such canned
codes this fund uses are kept as marked shortcuts. Three defects found by looking
at the result: a class-name collision with the global `.empty` state, a sticky bar
that stuck below the dialog's padding and let content show through, and an
unescaped code in an `onclick` (69 auxiliaries contain quotes) caught by the
project's own escaping regression test. On licensing: the supplied file claimed
CC BY-NC 4.0 as of 2026; the UDC Consortium's own pages still say CC BY-SA 3.0, so
that is what is recorded — NonCommercial would be incompatible with GPL-3.0 and
would force the table out of the repository. 18 new tests (1 332 total).
...
(„В-15“ в сигнатурата) се набираше на ръка от печатната авторска таблица за всеки
документ поотделно. Сега програмата може да го **предлага**.

Как работи:
- **Таблицата не идва с програмата.** Авторската таблица е чуждо печатно издание
  и не се разпространява заедно с програмата (тя е под GPL-3.0). Затова числата
  ги няма никъде в кода: библиотеката **внася веднъж своя файл** от
  „Настройки → Фонд → Авторски знак“. Четат се `.docx`, `.odt`, `.xlsx`, `.csv`,
  `.tsv`, `.txt` и записана страница; кирилицата в стари файлове (Windows-1251)
  се разпознава сама.
- **Внасянето минава през преглед.** Първо се казва колко реда и колко букви са
  разчетени, по коя подредба („буквосъчетание, после число“ или обратната) и се
  показва мостра от три различни букви. Чак след изричното „Запиши таблицата“
  нещо влиза в базата. Файл, от който излизат под 50 реда, се отказва с обяснение
  вместо да запише боклук.
- **Правилото е това на таблиците:** взима се редът с **най-голямото
  буквосъчетание, което не надминава фамилията** („Вазов“ → редът „Ваз“ → В-14),
  и се търси само вътре в буквата на фамилията. Без автор се подписва по първата
  дума на заглавието.
- **Копчето „Предложи“ казва ЗАЩО.** До полето „Авторски знак“ във формата на
  документа: „фамилия «Вазов» → ред «Ваз» → **В-14**“. Ако в полето вече има друг
  знак, той не се сменя без изричното „да“.
- **Редовете за конкретен автор се показват, но не се налагат.** Таблицата има и
  редове като „Вазов, И. 15“ — за точно този човек. Те **не** участват в
  търсенето (иначе всеки Вазов би получил знака на Иван Вазов), а се предлагат
  отделно до предложението и се взимат с натискане. Същото важи и за псевдонимите
  („Елин Пелин“): когато името е без запетая, фамилията е предположение и това си
  личи в обяснението.
- **Разделителят се извежда, не се налага.** Една библиотека пише „В-15“, друга
  „В 15“, трета „В15“ — програмата гледа вече въведените знаци (а ако ги няма —
  края на сигнатурата) и следва тях.
- **Групово попълване** — само на **празните** знаци, само като отделно действие
  и само след преглед „ще бъдат попълнени N документа, ето примери“. Вече
  попълнен знак не се пипа никога: заварените знаци са решение на библиотекар.
- **Нова проверка в „Настройки → Проверка на данните“:** знаци, чиято буква не
  отговаря на фамилията. Такъв ред е или грешка при въвеждане, или подписване по
  друго (сборник по съставител, книга *за* някого) — програмата не гадае кое от
  двете, само посочва.

Три неща, които се счупиха при разчитането на истинска таблица и затова имат
свои тестове:
- **Дългите записи отиваха в чужда буква.** Първият разчитач търсеше
  буквосъчетание с най-много 8 букви и режеше „Христович“ до „ристович“ — редът
  се озоваваше под буква Р. Сега текстът се взима **между две числа**, без
  предварителна дължина.
- **Тихо разместване с един ред.** Същият файл, прочетен като „число, после
  буквосъчетание“, събираше всяко число с буквосъчетанието от СЛЕДВАЩИЯ ред:
  цялата таблица излизаше сгрешена, без нищо да изглежда счупено. Сега
  буквосъчетанието и числото трябва да са на **един и същи ред**, а при равен
  резултат печели обичайната подредба.
- **Редовете за конкретен автор отпадаха като шум.** Те стоят там, където е
  човекът, а не по азбучен ред на целия низ („Димитров, Г. 58“ между „Диме 57“
  и „Дими 59“), и проверката за растящи числа ги изхвърляше. Сега минават покрай
  нея — но се приемат само ако числото им е в обхвата на буквата, иначе всяко
  изречение от страницата („Авторски таблици“ с номера на страницата до него) би
  влязло като ред.

Проверки: 21 нови теста (общо 1310). Числата в тестовете са **измислени** —
проверява се механизмът: разчитане на CSV, записана страница, обърнати колони,
`.docx` с разцепени от Word парчета и Windows-1251; правилото за търсене
(точно съвпадение, най-близкото преди, преди първото, буква без редове, без
прескачане в чужда буква); водеща нула („05“ не е 5); изведеният разделител;
внасяне с преглед и отказ; групово попълване при 15 000 документа; и четирите
екрана. Разчитачът е сверен и с истинска авторска таблица: 2 398 реда, 0
отпаднали, а два от записите, които са в програмата, излизат точно както са
въведени от библиотекар — Габе → Г 13 и Славейков → С 42.

**EN:** Round twenty-five — **author sign (Cutter number) from the surname**.
The Bulgarian author table is a third-party print edition, so it ships with the
app nowhere: the library **imports its own file once** (Settings → Fund →
Author sign; `.docx`, `.odt`, `.xlsx`, `.csv`, `.tsv`, `.txt` or a saved page,
Windows-1251 detected automatically), behind a preview that states the row and
letter counts and shows a sample before anything is written. A "Suggest" button
next to the Author sign field then applies the table's own rule — the row with
the greatest prefix not exceeding the surname, within that surname's letter —
and shows **why** ("surname Vazov → row Vaz → V-14"). Rows for one named author
("Vazov, I. 15") are offered beside the suggestion but never applied
automatically. The separator is inferred from the marks already in the fund.
Bulk fill touches only empty marks, behind its own preview. A new data check
lists marks whose letter does not match the surname. 21 new tests (1 310 total);
the parser was validated against a real table (2 398 rows, none dropped).

## v2.4.35

**BG:** Двадесет и четвърти кръг — нова публична страница на онлайн каталога
(`site/page-katalog.html`, страницата в chyavorec.org). Настолната програма
НЕ е променяна в този кръг; сменен е само екранът, който вижда читателят.

Какво не беше наред:
- **Страницата нямаше начало.** Първото на екрана бяха „2143 намерени записа“
  и плосък списък, започващ от произволен запис в реда на постъпване. Читател,
  който не знае какво точно търси — най-честият случай на сайт на читалище —
  нямаше откъде да тръгне: нито нови книги, нито детски, нито краезнание на
  видно място.
- **Всеки ЕКЗЕМПЛЯР беше отделен ред.** Една книга в седем бройки заемаше седем
  еднакви реда с различни инвентарни номера — между които читателят няма как да
  избира. За него това е една книга; важното е „7 екземпляра, 6 налични“.
- **Кориците-заместители бяха шум.** Еднакви бежови правоъгълници с първата
  буква на заглавието заемаха най-силното място на реда, без да носят нищо.
- **Филтрите бяха три падащи менюта без броеве** — не се виждаше колко има в
  „за деца“ или на френски, преди да се избере.
- **Липсваше „и сега какво“** — какво е сигнатурата, която показваме, и какво
  прави читателят с намереното.

Новата страница:
- **Начален изглед** — търсене с примери, числа за фонда (заглавия, екземпляри,
  налични сега, нови), рафт „Нови постъпления“, десет раздела с броеве, витрините
  на библиотекаря от `katalog.json`, азбучник на авторите (буквите без автор са
  изключени) и три стъпки „Как да заемете книга“.
- **Екземплярите се групират по заглавие и автор** — един ред на заглавие с
  „7 екземпляра · 6 налични“; инвентарните номера са в картата на записа, където
  по тях се намира бройката. Търсене по инв. № продължава да намира заглавието.
- **Типографски корици** вместо сиви правоъгълници: корицата се сглобява от
  заглавието и автора, а цветът идва от раздела на УДК — един поглед по рафта
  отличава историята от литературата. Деветте цвята са проверени по WCAG AA
  (бял текст върху цвета: 4,59:1 – 11,32:1). Сканирана корица (поле `cv`) ляга
  върху типографската; мъртъв адрес или еднопикселна картинка от Open Library
  не оставя счупена иконка, а показва цветната отдолу.
- **Филтри с броеве** (наличност, отдел, вид) вместо падащи менюта, подредба и
  „Покажи още“ по 50 заглавия с автоматично дозареждане.
- **Карта на записа** в собствен слой: сигнатурата изрично („по нея се намира на
  рафта“), таблица с всеки екземпляр и състоянието му, „Копирай връзка“, „Още от
  автора“, „Подобни книги“, а при изцяло заета книга — какво да направи читателят.

Запазено непроменено: начинът, по който каталогът СТИГА до страницата — GitHub
raw с jsDelivr като резервен път, двата отделни срока (6 сек. за заглавките,
25 сек. за тялото), кешът в браузъра с изрично предупреждение „кеширани данни“,
вградените демо записи при пълен отказ, скритата диагностика „Защо?“ и връзката
„Опитай отново сега“. Старите споделени връзки (`#q=`, `#vid=`, `#otdel=`,
`#podredba=`, `#vitrina=`, `#zapis=`) работят непроменени; новите изгледи
добавят `#razdel=`, `#avtor=` и `#samo=nalichni`.

Поправено покрай новото:
- **Заглавие на изгледа от самите филтри.** Споделена връзка `#razdel=8` или
  `#avtor=Б` се отваря в чужд прозорец, където името на натиснатата плочка го
  няма — дотук такъв адрес щеше да показва „Целият каталог“ над резултат, който
  не е целият каталог.
- **Променливите на цветовете излязоха на `:root`.** Картата на записа, слоят и
  плаващият бутон се закачат за `document.body`, тоест ИЗВЪН `.kat-page`: там
  `var(--kat-wine)` не се решаваше и бутонът „Копирай връзка“ излизаше бял текст
  върху бял фон.
- **Лентата с търсенето се залепва под навигацията на сайта, не под нея.**
  Навигацията на шаблона е `position:fixed`; височината ѝ се МЕРИ при зареждане
  (`setNavH()`), защото е различна на телефон и се променя при промени в
  шаблона. Признакът „залепнала“ се смята от текущото място на лентата, а не от
  запомнен `offsetTop` — при `position:sticky` той връща вече залепналото място.
- **Страницата се плъзгаше настрани при 320 px** (стар телефон): полето за
  търсене не можеше да се свие под вътрешната си широчина от 20 знака
  (`min-width:0`), а заглавията на разделите не се пренасяха на нов ред.
- Плаващият бутон „към началото“ вече стои над долната навигация на телефона.

Проверки: досегашният `site/test-page-katalog.js` минава НЕПРОМЕНЕН — и осемте
сценария за източници, срокове, кеш и диагностика. Нов `site/test-page-katalog-view.js`
проверява изгледа при 15 000 екземпляра (реален фонд): групиране, начален изглед,
търсене, филтри, „Покажи още“, картата на записа, празен резултат, повреден ред в
`items` — плюс времената (зареждане 0,35 s, филтър 50 ms, „Покажи още“ 41 ms,
отваряне на запис 20 ms в jsdom). Отделно, в истински Chromium: клавиатура
(Enter търси, Enter отваря запис, Esc затваря и връща фокуса), ARIA, „намалено
движение“, липса на хоризонтално плъзгане при 320 px и залепването под
симулирана навигация на сайта. Размер на файла: 71 740 знака при лимит 75 000
за страница в Page Editor.

**EN:** Round twenty-four — a rebuilt public catalog page (`site/page-katalog.html`
on chyavorec.org). The desktop app is unchanged. The old page opened on a flat
list of 2 143 rows in accession order, showed every COPY as its own row (one book
in seven copies filled seven identical rows), used identical grey placeholder
covers, and offered three count-less dropdowns. The new page opens on a landing
view (search with examples, fund figures, new arrivals, ten sections with counts,
the librarian's shelves, an author index, and how to borrow), groups copies by
title ("7 copies · 6 available"), draws typographic covers coloured by UDC class
(all nine WCAG AA verified; a scanned cover overlays it and degrades to it when
the URL is dead), offers count-bearing filter chips, and opens each record in its
own layer with the shelf mark, a per-copy availability table and share/related
actions. The data path is untouched — GitHub raw plus jsDelivr, the two separate
timeouts, the browser cache with its explicit stale warning, the embedded demo
records, and the hidden diagnostics — and every old share link still works. The
existing 8-scenario test passes unchanged; a new view test covers the rendering
at 15 000 copies, alongside keyboard, ARIA, reduced-motion and 320 px checks.
...
Измервано върху голяма реалистична база (15 000 книги, 3 000 читатели,
100 000 заемания, 33 000 събития, 20 000 записа в дневника на действията)
през истинския `main.js` (време и размер на отговора на всеки IPC канал,
медиана от 5) и през истински Chromium (време до нарисуван екран, DOM,
пренесени байтове). Първо анализ — проследяване на всяка SQL заявка по канал,
профил на процесора в главния процес и в екрана — после само промените, които
измерванията оправдават.

Какво беше бавно и защо:
- **Книги, Инвентарна книга, Читатели теглеха ЦЕЛИЯ регистър при всяко
  отваряне** (5,3 MB / 5,2 MB / 1,9 MB по IPC за 15 000 записа), а екранът
  показваше 300 реда. Всяка буква в търсенето, всеки филтър, всеки запис
  презареждаше всичко. Отгоре на това `readers:list` изпълняваше по две
  корелирани подзаявки на читател (отворени и просрочени), а `invBook:list`
  теглеше и всички инвентаризационни проверки.
- **Годишната статистика** правеше 20+ отделни заявки, повечето без индекс:
  `substr(register_date,1,4) = ?` и `substr(date_out,1,4) = ?` спират
  използването на индексите; „върнати навреме / със закъснение“ бяха два
  отделни прохода през 100 000 заемания; корелираната подзаявка за бройката се
  изчисляваше два пъти на ред (веднъж за броя, веднъж за стойността).
- **Просрочени и напомняния** броят работните дни между две дати —
  `closedDaysBetween()` четеше работните дни и почивните дати от базата и
  обхождаше ден по ден ЗА ВСЯКО просрочено заемане (250 просрочени × до 200 дни).
- **Публичният каталог** се строеше от `SELECT b.*` (всички 40+ колони,
  включително служебните) с корелирана подзаявка за наличността на всеки ред.
- **Преходът между раздели** принуждаваше подредба на СТАРОТО съдържание
  (`void v.offsetWidth`, за да се рестартира CSS анимацията) — при 5 000
  реда в таблицата това е десетки милисекунди чист блокаж преди всеки преход.

Какво е направено:
- **Прозоречен режим в базата** за `books:list`, `invBook:list` и
  `readers:list` — по избор на екрана (`page: {offset, limit, …}`) обработчикът
  връща само порцията + общия брой (+ отделите / показателите за целия
  регистър, изчислени в SQL). Търсене, филтър по отдел/категория/статус и
  „Избери всички“ (`idsOnly`) вървят през базата. Без `page` каналите връщат
  досегашния пълен масив — всички стари повиквания (печат, износ, другите
  екрани, тестовете) работят непроменени. Търсенето в инвентарната книга
  минава през FTS5 (`books_fts`) — `LIKE` сгъва само латиницата, „вазов“ не
  намираше „Вазов“ през базата.
- **Екраните Книги / Инвентарна книга / Читатели** зареждат първата порция,
  „Покажи още“ тегли следващата с `offset`; поколение на списъка (`*_GEN`)
  изхвърля закъсняла порция, ако междувременно търсене или филтър е подменил
  списъка; пълен рендер след запис/изтриване запазва разгърнатия прозорец;
  изтритият документ отпада от избора за групови действия.
- **Статистика, табло, КДБФ, отчет „Фонд по групи“:** агрегиране в SQL
  (`SUM(CASE …)`, `GROUP BY`, `LEFT JOIN inventory` вместо корелирана
  подзаявка), `BETWEEN 'Г-01-01' AND 'Г-12-31'` вместо `substr(…)`, един проход
  за върнати навреме / със закъснение / начислени глоби, „най-търсени“ по
  документ, после по заглавие. Нови индекси `idx_loans_date_out`,
  `idx_loans_reader_open`, `idx_books_register_date`; `+b.register_date` там,
  където предикатът не е селективен (при 15 000 книги индексът правеше пълното
  сканиране ПО-БАВНО — измерено, затова е изключен точно там).
- **Календар:** снимка на работните дни и почивните дати с 3-секунден живот
  (обезсилвана при всяка промяна на календара), а `closedDaysBetween()` е
  аритметика по дни от седмицата + точните почивни дати в интервала, вместо
  обхождане ден по ден.
- **Публичен каталог:** списък от 18-те публични полета +
  `LEFT JOIN (отворени заемания GROUP BY book_id)` за наличността.
- **Читатели:** отворени/просрочени заемания през едно `LEFT JOIN (… GROUP BY
  reader_id)`; `loans:list` подрежда по `+l.date_out` (без излишен индексен
  обход при пълен списък).
- **Гише:** правило, заемания и заявки на читателя се четат успоредно
  (`Promise.all`) — три обиколки по IPC стават една.
- **Преход между раздели:** Web Animations API (`v.animate(...)`) вместо
  `void v.offsetWidth` + CSS анимация; зачита `prefers-reduced-motion`.

Измерено (медиана от 5, същата база; „преди“ = v2.4.30 в същата среда):

| Канал / екран | преди | след |
|---|---|---|
| `books:list` (прозорец 300) | 70 ms · 5,3 MB | 4 ms · 104 KB |
| `books:list` „Избери всички“ | 80 ms · 5,3 MB | 0,6 ms · 77 KB |
| `invBook:list` (прозорец 300) | 49 ms · 5,2 MB | 5,6 ms · 107 KB |
| `invBook:list` търсене | 49 ms · 5,2 MB | 12 ms · 106 KB |
| `readers:list` | 98 ms | 14 ms |
| `loans:overdue` | 26 ms | 1,8 ms |
| `loans:reminders` | 33 ms | 6,3 ms |
| `stats:report` (текуща / минала година) | 178 / 170 ms | 37 / 42 ms |
| `dashboard:full` | 21 ms | 7,7 ms |
| `kdbf:report` | 15 ms | 7,3 ms |
| `catalog:export` (пълен запис на katalog.json) | 227 ms | 110 ms |
| Екран „Книги“ (Chromium, до нарисуван екран) | 7 771 ms | 267 ms |
| Екран „Инвентарна книга“ | 6 488 ms | 311 ms |
| Екран „Читатели“ | 1 811 ms | 396 ms |
| Екран „Статистика“ | 247 ms | 79 ms |
| Екран „Табло“ | 99 ms | 61 ms |

Отбелязано и оставено: `books:list` с търсене + филтър по отдел в прозоречен
режим е ~4 ms по-бавно от стария пълен отговор (три заявки — редове, общ брой,
отдели — срещу една), при 4× по-малък отговор; пълният `loans:list` (33 MB за
100 000 заемания) не се ползва от екраните и е оставен.

Проверки: нов `test/perf-v2431.test.js` (16 теста) — равностойност на
прозоречните отговори с пълните (редове, общ брой, отдели, филтри, `idsOnly`,
показатели), на SQL агрегатите в статистиката/таблото/КДБФ с независимо
JS изчисление за две години, на `closedDaysBetween()` с бавния обход ден по
ден (600+ интервала, с обезсилване на кеша), на публичния каталог с
`SELECT b.*`, на екраните в jsdom (порции, offset, филтри през базата,
задържан фокус, печат с пълния списък, надпревара „Покажи още“ ↔ търсене в
двете посоки). Мутационна проверка: 41 мутации (изхвърлен индекс, махнат
филтър, махнато поколение, сгрешена граница, …) — всички уловени, 1
равностойна контрола минава. Пълната поредица: 1278 успешни, 0 неуспешни.

**EN:** Round twenty — an in-depth performance analysis and optimisation pass,
measured on a large realistic database (15k books, 3k readers, 100k loans)
through the real `main.js` (per-channel IPC latency and payload, median of 5)
and real Chromium (time to painted view). Books / inventory book / readers
views fetched the whole register on every open (5.3 / 5.2 / 1.9 MB) to show
300 rows; the yearly statistics ran 20+ unindexed queries (`substr(date,1,4)`
defeats indexes, correlated subqueries evaluated twice per row); overdue
reminders walked the calendar day by day per loan; the public catalog was
built from `SELECT b.*`; view transitions forced layout of the old content.
Now: opt-in server-side windowing (`page: {offset, limit, …}` → rows + total +
SQL-computed facets, FTS5 search for Cyrillic case folding; the legacy
array response is unchanged when `page` is absent), SQL aggregation with
`BETWEEN` and new indexes (`+col` where an index made a non-selective scan
slower), a cached calendar snapshot with arithmetic `closedDaysBetween()`,
a lean catalog payload with a grouped-join availability, `Promise.all` at the
desk, and a WAAPI view transition. Measured: books window 70 → 4 ms and
5.3 MB → 104 KB, readers 98 → 14 ms, overdue 26 → 1.8 ms, stats 178 → 37 ms,
Books view painted 7.8 s → 0.27 s, Readers view 1.8 s → 0.4 s. 16 new
equivalence/race tests, 41 mutations all caught, full suite 1278 passing.
