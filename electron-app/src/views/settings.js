...
async function renderSetup() {
  ...
  $('#view').innerHTML = `
    ...
    ${setupMore('Календар на библиотеката', 'работни дни и затворени дни — празници и отпуск', `
    ...
    ${/* „ЗАПОЧВАНЕ НА ЧИСТО“ (v2.4.64) — най-долу в раздела и нарочно свито.
          Дотук единственият начин беше ръчен: затвори програмата, намери
          папката, преименувай library.db. Това е действие, което библиотекарят
          прави веднъж грешно и губи годината си работа. Стои ДО резервните
          копия, защото първото, което прави, е резервно копие, и защото
          изходът от сгрешено изтриване е точно списъкът с копия отгоре.
          Самият бутон само ОТВАРЯ прозореца с обяснението — нищо не се изтрива,
          докато думата „ИЗТРИЙ“ не бъде написана на ръка. */''}
    ${setupMore('Изтриване на всички данни (започване на чисто)',
      'необратимо · прави резервно копие · за нова библиотека или след обучение', `
      ${setupHow('Изтрива <b>всички данни за фонда, читателите, заеманията и краезнанието</b> и връща програмата в състоянието на нова инсталация, <b>без</b> да губи настройките на библиотеката. Служи за три неща: читалище, което е въвеждало пробни записи, докато свикне с програмата, и започва истинското описване на фонда; компютър, който минава към друга библиотека; и обучение или демонстрация, след която демонстрационните данни трябва да изчезнат. <b>Преди изтриването се прави пълно резервно копие</b> на цялата база — ако копието не успее, не се изтрива нищо. Копието остава в папката с резервните копия и от него всичко се връща обратно („Възстанови от файл…“ по-горе). Ако просто искате да се отървете от няколко сгрешени записа, не ползвайте това — поправете или изтрийте самите записи.')}
      <div class="note d" style="margin-top:0"><b>Необратимо.</b> Единственият път назад е резервното копие,
        което програмата прави точно преди изтриването. Прозорецът по-долу показва <b>какво изчезва</b> и
        <b>какво остава</b>, с точните числа, преди да потвърдите.</div>
      <div class="toolbar">
        <button type="button" class="btn dgr" onclick="resetAllForm()">Изтриване на всички данни…</button>
      </div>`)}
    </section>
    ...
    ${setupCard('Помощ и обратна връзка', `
      ...
      <div class="hint" style="margin-bottom:4px">Имейл за връзка и поддръжка:
...
async function loadAutoBackupBox() {
  ...
  /* ДНЕВНИ И МЕЖДИННИ СЕ НАЗОВАВАТ ПООТДЕЛНО (v2.4.65, кръг 42, находка В11).
     `plainDailyCount` брои ВСИЧКИ `auto-*.db`, тоест и междинните
     `auto-ГГГГ-ММ-ДД-ЧЧММ.db` — те се правят от таймера на всеки 3 часа и при
     всяко затваряне на програмата, до 4 на ден. Надписът обаче ги описваше като
     „некриптирани ДНЕВНИ копия“, а обяснението до него ги обявяваше за копия
     „отпреди включването на защитата“ — и двете неверни за файл, направен
     преди час. Библиотекарка, включила защитата днес, броеше файловете в
     папката, виждаше днешни дати и решаваше, че числото на екрана е грешно.
     Общото число не се променя (експозицията е същата), но двата вида се
     назовават с истинските си имена. По-стар main процес не праща
     `plainIntradayCount` — тогава надписът остава както досега. */
  const plainIntraday = Number(st.plainIntradayCount) || 0;
  const plainOnlyDaily = Math.max(0, plainDaily - plainIntraday);
  const plainKinds = [
    plainOnlyDaily ? `<b>${plainOnlyDaily}</b> ${plainOnlyDaily === 1 ? 'дневно' : 'дневни'}` : '',
    plainIntraday ? `<b>${plainIntraday}</b> ${plainIntraday === 1 ? 'междинно' : 'междинни'}
  ...
  const plainNote = plainDaily > 0
    ? ` В папката с резервните копия в момента има <b>${plainDaily}</b>
        ${plainDaily === 1 ? 'некриптирано автоматично копие' : 'некриптирани автоматични копия'}
        ${plainKinds ? '— ' + plainKinds + ' — ' : ''}(всяко е пълен списък с личните данни на читателите).`
    : '';
  ...
  const restoreNote = plainRestore > 0
    /* Числото брои И двата префикса (backup.js: „before-restore-“ и от v2.4.64
       „before-reset-“), затова и надписът трябва да ги назове и двата — иначе
       библиотекарката търси в папката файлове „before-restore-…“, брои по-малко
       от обявеното и решава, че екранът лъже. Копието отпреди изтриването е и
       по-важното от двете: след „Започване на чисто“ то е единственото, което
       още съдържа старата библиотека. */
    ? `<div class="hint" style="margin-top:6px">В папката има и <b>${plainRestore}</b>
        ...
        или отпреди изтриване на всички данни (<code>before-restore-…</code>,
        <code>before-reset-…</code>) в чист текст. <b>Не ги изтривайте</b> — те са изходът,
        ако възстановяване или започване на чисто се окаже сгрешено. Ако личните данни в тях
        са проблем, преместете ги на място, което не е споделено в мрежата.</div>`
    : '';
...
/* ---------------- „Изтриване на всички данни“ (започване на чисто, v2.4.64) ----------------
   ЗАЩО ПРОЗОРЕЦ С ЧИСЛА, А НЕ ВЪПРОС „сигурни ли сте?“. Въпросът с „Да/Не“ се
   затваря с Enter, а отметката се цъка с мишката по инерция — и двете са
   обичайни за действие, което се прави по десет пъти на ден. Това се прави
   веднъж и е необратимо, затова екранът показва ТОЧНИТЕ числа на онова, което
   изчезва, поименния списък на онова, което остава (и защо), къде ще легне
   резервното копие — и иска думата „ИЗТРИЙ“, написана на ръка.

   Думата НЕ е преписана тук: идва от reset:plan, тоест от самия обработчик,
   който я и проверява. Проверката на екрана е само любезност — същинският
   отказ без дума е в handlers/reset.js, защото екранът е един от възможните
   входове към канала, а не пазачът му. */
let RESET_PLAN = null;
async function resetAllForm() {
  const p = await call(window.api.reset.plan());
  if (!p) return;
  RESET_PLAN = p;
  ...
  /* ИЗТИЧАНЕТО, което изтриването НЕ поправя, се казва ТУК, преди потвърждението,
     а не се открива после: публикуваният katalog.json е в папка на библиотеката
     (git хранилище) и е вече качен в интернет — изтриването на базата не го
     маха нито от папката, нито от сайта. И НЯМА да се изчисти само: проверено в
     writeCatalogIfConfigured() (main.js) — автоматичният запис нарочно отказва
     да презапише непразен публикуван каталог с празен, за да не изтрие прясна
     инсталация реално публикувани данни. Тоест старите записи остават онлайн,
     докато човек не се разпореди с файла. */
  /* И ВТОРАТА ПОЛОВИНА НА СЪЩОТО (v2.4.65, кръг 42, А13): досега екранът
     казваше само че ПУБЛИКУВАНИЯТ файл остава — и премълчаваше далеч по-лошото,
     че остава и ВРЪЗКАТА към чуждата папка и чуждия профил в GitHub, тоест че
     новата библиотека ще започне да публикува фонда си там. От v2.4.65
  ...
  const catalogWarn = (p.catalogFolder || cr.any)
    ? `<div class="note w"><b>Онлайн каталогът: връзката се прекъсва, публикуваният файл остава.</b>
        ...
        <b style="font-family:var(--mono)">${esc(p.catalogFolder || '(няма)')}</b>
        ${p.catalogRepo ? ' и с хранилището <b style="font-family:var(--mono)">' + esc(p.catalogRepo) + '</b>' : ''}.
        Така новият фонд няма да бъде записан в работното копие на предишната библиотека и няма да бъде
        качен в нейния профил в GitHub. За да публикувате каталог наново, свържете папка от
        „Онлайн каталог“.
        <br><b>Остава</b> — самият публикуван файл <code>katalog.json</code>. Записите на СТАРАТА
        библиотека стоят в него и остават достъпни в интернет: изтриването тук не пипа чужди папки, а
        автоматичният запис на каталога <b>нарочно отказва</b> да презапише непразен публикуван каталог
        с празен (предпазна мярка срещу прясна инсталация, която би изтрила реално публикувани данни).
        Ако старите записи не бива да остават онлайн, премахнете или заменете файла ръчно.</div>`
    : '';
  /* ОТМЕТКАТА „това е нова библиотека“ (А13). Нарочно НЕ е сложена по
     подразбиране: най-честият повод за изтриване е СЪЩАТА библиотека, която
  ...
  const identityBox = ir.any
    ? `<div class="note" style="margin-top:10px">
        <label style="display:flex;gap:8px;align-items:flex-start;cursor:pointer">
          <input type="checkbox" id="rsNewLib" style="margin-top:3px">
          <span><b>Това е НОВА библиотека — изчисти и самоличността.</b>
          <span class="hint" style="display:block;margin-top:3px">Ще бъдат изтрити и наименованието
          (<b>${esc(ir.libName || ir.org || '(празно)')}</b>), организацията и населеното място
          (<b>${esc(ir.place || '(празно)')}</b>). Оставете отметката празна, ако това е същата
          библиотека, която просто започва отначало: тези три полета излизат върху всеки акт, протокол
          и регистър за печат.</span></span>
        </label></div>`
    : '';
  modal('Изтриване на всички данни — започване на чисто', `
    <div class="note d" style="margin-top:0"><b>Това действие е необратимо.</b>
      Преди изтриването програмата прави <b>пълно резервно копие</b> на цялата база в папката
      <b style="font-family:var(--mono)">${esc(p.backupFolder)}</b>
      (файл <code>before-reset-…</code>). Ако копието не успее, <b>не се изтрива нищо</b>.
      ${p.backupEncrypted
        ? 'Копието ще бъде <b>криптирано</b> с паролата за защита на личните данни — пазете я, без нея копието не се отваря.'
        : (p.pdpConfigured
          ? 'Защитата на личните данни е включена, но <b>заключена</b>, затова копието ще бъде <b>некриптирано</b> и ще съдържа ЕГН и адресите на читателите. Отключете я и повторете, ако това е проблем.'
          : 'Копието ще бъде <b>некриптирано</b> и ще съдържа личните данни на читателите (имена, адреси, телефони, ЕГН) — пазете файла на място, което не е споделено.')}
    </div>
    <h3 style="margin:14px 0 4px">Изчезва (${(p.totalRows || 0).toLocaleString('bg-BG')} записа общо)</h3>
    ${goes}
    <h3 style="margin:14px 0 4px">Остава</h3>
    ${stays}
    <div class="hint">Следващият инвентарен номер се връща на <b>1</b> и новите записи пак започват от
      номер 1, точно както при нова инсталация. Резервните копия в папката
      <b style="font-family:var(--mono)">${esc(p.backupFolder)}</b> <b>не се изтриват</b> — в тях остават
      всички данни на старата библиотека.</div>
    ${catalogWarn}
    ${identityBox}
    <div class="note d"><b>След изтриването програмата ще се стартира наново.</b> Това е нарочно: под
      краката ѝ изчезва цялото съдържание на базата и всеки отворен екран, недовършена форма или започната
      инвентаризация биха сочили към изтрити записи.</div>
    <form id="rsAllF" onsubmit="return false">
      ${fld('Напишете думата ' + p.word + ', за да потвърдите', 'word',
        { hint: 'с главни букви, на ръка — отметка не стига за действие, което не може да се върне' })}
    </form>`,
    `<button class="btn" onclick="closeModal()">Отказ</button>
     <button class="btn dgr" onclick="resetAllRun()">Изтрий всички данни</button>`);
}
window.resetAllForm = resetAllForm;
async function resetAllRun() {
  const word = (RESET_PLAN && RESET_PLAN.word) || 'ИЗТРИЙ';
  const d = formData('#rsAllF');
  if (String(d.word || '').trim().toUpperCase() !== word) {
    return toast('Напишете думата „' + word + '“ в полето за потвърждение. Нищо не е изтрито.', 'err');
  }
  /* Отметката пътува до обработчика (А13). Той е този, който решава — екранът
     само носи избора; при липсваща отметка (по-стар екран) обработчикът приема
     „не е нова библиотека“ и самоличността остава. */
  const newLibrary = !!($('#rsNewLib') && $('#rsNewLib').checked);
  const res = await window.api.reset.wipe({ word: d.word, newLibrary });
  if (!res.ok) return toast(res.error, 'err');
  closeModal();
  /* В работеща програма дотук почти не се стига — обработчикът вика
     app.relaunch()+app.exit(0) и прозорецът си отива. Съобщението остава за
     случая, в който рестартът се забави: то носи ПЪЛНИЯ път на копието, защото
     точно него човек търси, ако е изтрил не каквото е искал. */
  toast((res.data && res.data.message) || 'Всички данни са изтрити.', 'ok');
}
window.resetAllRun = resetAllRun;
...
/* ---------------- Помощ и обратна връзка (имейл и уебсайт за поддръжка) ----------------
   Статичен, некриптиран контакт за ПОДДРЪЖКА на самата програма — не е данни на
   конкретната библиотека, затова нарочно не е поле в базата данни/Настройки, а
   фиксиран текст тук (както в LICENSE/README). От v2.4.60 е пощенска кутия на
   домейна на програмата (support@invlib.com), а не личен адрес: библиотеката пише
   на един и същ адрес независимо кой отговаря от другата страна. Имейлът се изпраща през същия
...
const DEV_CONTACT_EMAIL = 'support@invlib.com';
...
/* ---------------- Проверка на данните (v2.4.21) ----------------
   ...
   И ТРИТЕ БУТОНА ЗА ОТВАРЯНЕ МИНАВАТ ПРЕЗ `invBookEdit`, НЕ ПРЕЗ `bookForm`
   (v2.4.65, кръг 42, находка Б11).
   (а) КАКВО СТАВАШЕ ДОТУК. „Поправи датата“, „Отвори“ при дублиран баркод и
   „Отвори“ при несъответстващ авторски знак викаха `bookForm(id)` направо —
   тоест отваряха формата за редакция на запис в инвентарната книга, прескачайки
   потвърждението. А надписът в „Книги“ твърди на библиотекарката точно
   обратното: че запис в инвентарната книга се редактира само от „Инвентарна
   книга“ и само с изрично потвърждение.
   (б) ЗАЩО Е ГРЕШНО. Инвентарната книга е официалният регистър по Наредба № 3;
   по чл. 17, ал. 2 всяко вписване в нея трябва да може да бъде обяснено, а
   потвърждението е точно това — моментът, в който човекът вижда КОЙ запис ще
   промени („«Вазов, Иван. Под игото» (инв. № 101)“) и че промяната важи веднага
   навсякъде, включително в онлайн каталога. Обиколен път, който го прескача,
   прави надписа в „Книги“ неверен и превръща правилото в украса.
   Отделно: „Поправи датата“ мести документа между две години на КДБФ — тоест е
   от най-тежките редакции, които изобщо се предлагат оттук.
   (в) ЗАЩО ТОЧНО ТАКА. `invBookEdit(id)` пита и чак после вика `bookForm(id)`;
   от v2.4.65 той изтегля картона от базата, ако не е в паметта (екранът
   „Инвентарна книга“ може изобщо да не е отварян в тази сесия), тоест въпросът
   назовава документа и оттук. Екранът не е мястото на правилото — то е в
   обработчика; тук се затваря обиколният път. */
    ...
    ${fund.findings.length ? fund.findings.map(f => `
      ...
      ${(f.list && f.list.length) ? `<div class="wrap" style="max-height:220px"><table class="ledger">
        ...
        ${f.list.map(r => `<tr><td class="num">${r.inv_number ?? '—'}</td><td>${esc(r.title || '')}</td>
          ...
          <td><button class="btn sm" onclick="invBookEdit(${r.id})">Поправи датата</button></td></tr>`).join('')}
    ...
    ${dups.length ? `
      ...
      ${dups.map(g => `<div style="margin-bottom:10px"><b>Баркод ${esc(g.barcode)}</b>
        ...
        ${g.books.map(b => `<tr><td class="num">${b.inv_number ?? '—'}</td><td>${nameOf(b)}</td><td>${esc(b.status || '')}</td>
          <td><button class="btn sm" onclick="invBookEdit(${b.id})">Отвори</button></td></tr>`).join('')}
