...
async function resetAllForm() {
  ...
  const catalogWarn = p.catalogFolder
    ? `<div class="note w"><b>Онлайн каталогът остава публикуван — и няма да се изчисти сам.</b>
        Записите на СТАРАТА библиотека стоят във файла <code>katalog.json</code> в папката
        <b style="font-family:var(--mono)">${esc(p.catalogFolder)}</b>
        ${p.catalogRepo ? ' и в хранилището <b style="font-family:var(--mono)">' + esc(p.catalogRepo) + '</b>' : ''}
        и остават достъпни в интернет. Изтриването тук не пипа чужди папки, а автоматичният запис на
        каталога <b>нарочно отказва</b> да презапише непразен публикуван каталог с празен (предпазна мярка
        срещу прясна инсталация, която би изтрила реално публикувани данни). Ако старите записи не бива
        да остават онлайн, премахнете или заменете файла ръчно.</div>`
    : '';
...
async function resetAllRun() {
  ...
  const res = await window.api.reset.wipe({ word: d.word });
  if (!res.ok) return toast(res.error, 'err');
  closeModal();
