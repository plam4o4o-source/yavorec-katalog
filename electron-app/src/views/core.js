...
/* ---------------- Прозоречен рендер на таблици (обща помощна функция, v2.3.0) ----------------
   ...
   тоест изчертаваха наново и вече показаните редове. Добавянето само на новата
   порция прави всяко натискане еднакво евтино.

   ИЗМЕРЕНОТО (v2.4.64) — и КОЙ ДВИГАТЕЛ КОЛКО КАЗВА. Дотук на това място стоеше
   число от jsdom: „49 натискания от 300 до 15 000 реда = 112 462 ms“. То вече не
   описва действителността и подвеждаше по два начина наведнъж. jsdom има HTML
   парсер, написан на JavaScript, и е 17–41 пъти по-бавен от Chromium при
   вписване в DOM (медиана 24,8×) — тоест числото беше около двайсет пъти
   по-голямо от истинското; а от друга страна jsdom изобщо НЕ прави оформление
   (layout) и не държи изчертана страница, тоест не мереше точно онова, което в
   истинския прозорец боли.

   В ИСТИНСКИ CHROMIUM 141 (node /tmp/r41/bench-chromium.js, 5 повторения, същата
   разметка на „Книги“, същият src/style.css) вписването на порция от 300 реда е
   ПЛОСКО и евтино на всяка дълбочина — около 7 ms, независимо дали в таблицата
   вече стоят 300 или 14 700 реда. Расте друго — ОФОРМЛЕНИЕТО И ПАМЕТТА НА
   РЕНДЕРА (node /tmp/r41/mem-chromium.js):
        300 реда →    5 400 възела ·    65 ms до нарисуваното · +73 МБ RSS
      1 200 реда →   21 675 възела ·   137 ms                  · +132 МБ
      3 000 реда →   54 075 възела ·   283 ms                  · +248 МБ
     15 000 реда →  270 375 възела · 1 115 ms                  · +1 025 МБ,
                    таблица с височина 751 835 px
   тоест около 70 КБ памет на ред и над секунда замръзнал прозорец на дъното.
   (За сравнение, същите стъпки в jsdom: ~175 ms на порция — число, което не
   отговаря на нищо, което библиотекарят изживява.)

   ЗАТОВА ТАВАН НА ОБЩИЯ БРОЙ РЕДОВЕ, А НЕ ПО-МАЛКА ПОРЦИЯ. Понеже самото
   вписване е плоско, смаляването на порцията не печели нищо — цената идва от
   това КОЛКО реда стоят на екрана общо. RENDER_MAX_ROWS слага таван на точно
   това число.

   ЗАЩО 3 000. Това е последната стъпка от измерването, при която прозорецът
   остава използваем: 283 ms и +248 МБ. Следващата (6 000) вече е 517 ms и
   +~500 МБ, а 15 000 е над секунда и над гигабайт. 3 000 реда са и повече,
   отколкото човек може да прегледа с очи — отвъд тях не се ЧЕТЕ списък, а се
   ТЪРСИ, и правилният отговор е търсене или филтър, не още редове.

   ПРИ ТАВАНА БУТОНЪТ „Покажи още“ СЕ МАХА и на негово място застава изречение,
   което казва колко се виждат от колко и какво да се направи вместо това. Мълком
   отрязан списък е по-лошо от отрязан списък с надпис: библиотекарят решава, че
   книгите ги няма.

   RENDER_PAGE_SIZE е общият размер на порцията (300 реда — толкова, колкото
   вече ползват трите екрана).

   o = { body, bar, rows, limit, painted, total, rowsHtml, emptyHtml, moreHtml }
     ...
     total     — общият брой редове в резултата, когато rows са само заредените
                 порции от базата (прозоречен режим); по подразбиране rows.length
     rowsHtml(part) — HTML за подадените редове; emptyHtml — при нула редове
     moreHtml(more, total) — HTML на лентата отдолу (при достигнат таван на нейно
                 място застава renderCapHtml — един и същ надпис за всички екрани)
   Връща новия брой изчертани редове — извикващият го пази за следващия път. */
const RENDER_PAGE_SIZE = 300;
/* Таванът на ОБЩИЯ брой редове в едно тяло на таблица — виж измерването горе. */
const RENDER_MAX_ROWS = 3000;
window.RENDER_PAGE_SIZE = RENDER_PAGE_SIZE;
window.RENDER_MAX_ROWS = RENDER_MAX_ROWS;
/* Честният надпис на мястото на „Покажи още“, когато таванът е достигнат.
   Нарочно казва три неща: колко се виждат от колко (за да е ясно, че липсващите
   ги ИМА), защо са спрени (прозорецът) и кои са трите изхода, с имената, с които
   се виждат на екрана. */
function renderCapHtml(shown, total) {
  return `<span class="hint">Показани са първите ${shown.toLocaleString('bg-BG')} реда от
    ${total.toLocaleString('bg-BG')}. Останалите ги има — просто не се изчертават наведнъж:
    при повече от ${RENDER_MAX_ROWS.toLocaleString('bg-BG')} реда прозорецът замръзва за секунди и
    заема стотици мегабайта. Стеснете търсенето, използвайте филтрите над таблицата или
    отпечатайте диапазон.</span>`;
}
window.renderCapHtml = renderCapHtml;
function paintRowWindow(o) {
  ...
  /* Таванът се прилага ТУК, в общата машинка, а не във всеки изглед поотделно:
     иначе всеки следващ прозоречен списък трябва да си спомни за него сам, а
     точно това забравяне е поводът тази функция да съществува. */
  const shown = Math.min(limit, rows.length, RENDER_MAX_ROWS);
  /* Общият брой на резултата. В прозоречен режим (порции от базата) rows са само
     заредените дотук, затова истинският общ брой идва отвън; иначе е дължината
     на подадения списък. */
  const total = Math.max(o.total == null ? rows.length : Math.max(0, o.total), rows.length);
  const capped = shown < total && shown >= RENDER_MAX_ROWS;
  ...
  /* Стои ли в тялото ТОЧНО това, което трябва да се види (таванът е достигнат и
     порцията не е пораснала) — тялото не се пипа. Без този ред достигнатият
     таван би карал всяко следващо извикване да пречертава 3 000 реда наново
     (283 ms в Chromium), за да получи същото тяло. */
  const bodyUpToDate = shown > 0 && painted === shown && body && body.children.length === painted;
  if (body && !append && !bodyUpToDate) {
    body.innerHTML = shown ? o.rowsHtml(rows.slice(0, shown)) : (o.emptyHtml != null ? o.emptyHtml : o.rowsHtml([]));
  } else if (body && append) {
    body.insertAdjacentHTML('beforeend', o.rowsHtml(rows.slice(painted, shown)));
  }
  if (bar) {
    bar.innerHTML = capped ? renderCapHtml(shown, total)
      : (o.moreHtml ? o.moreHtml(rows.length - shown, rows.length) : '');
  }
  return shown;
}
...
/* „ежедневно“ (v2.4.61, находка 1 от одита на периодиката): вестникът — най-често
   срещаната периодика в читалищната библиотека — нямаше своя стойност. Изборът
   започваше от „седмично“ и библиотекарката трябваше да сложи или „нередовно“
   (тогава таблото никога не казва „липсва брой“ — точно при ежедневника, където
   рекламацията има смисъл само в първите дни), или „седмично“ (тогава всеки
   редовно доставен вестник изглежда закъснял с шест дни). Стойността трябва да
   съвпада дословно с FREQ_INTERVAL в handlers/periodicals.js и със списъка в
   db/enum-triggers.js — трите заедно решават предвиждането, а базата отказва
   всичко извън списъка. */
const PER_FREQ = ['ежедневно', 'седмично', 'двуседмично', 'месечно', 'тримесечно', 'полугодишно', 'годишно', 'нередовно'];
...
/* ИМЕТО НА ФАЙЛА НЕ ЗАВЪРШВА С ТОЧКА (v2.4.61).
   ===========================================================================
   Имената на документите в програмата са написани както се пишат на български
   и затова свършват със съкращение: „КДБФ 2026 г.“, „Инвентарна книга —
   18.09.2026 г.“, „Периодични издания 2026 г.“. handlers/print.js долепя
   разширението (name + '.pdf'), тоест диалогът „Запази PDF…“ предлагаше
   „КДБФ 2026 г..pdf“ — с две точки. Освен че в папката с документите за
   проверката това изглежда като печатна грешка, Windows МАХА крайните точки и
   интервали от имената на файлове: предложеното и записаното име се
   разминават, а ръчно въведено такова име Explorer отказва.
   Правилото стои ТУК, а не в шаблоните: името се съставя на двайсетина места
   (по едно setPrintPage на документ) и всяко от тях щеше да трябва да го помни.
   Точките ВЪТРЕ в името се пазят — „18.09.2026 г.“ и „инв. № 1000“ трябва да
   останат четими. Водещите точки също падат: файл, чието име започва с точка,
   е скрит в Linux и macOS. */
