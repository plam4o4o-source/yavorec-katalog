...
async function route() {
  ...
  if (v) {
    ...
    /* Старият раздел се маха ВЕДНАГА (v2.4.50). Дотук route() не пипаше #view:
       заглавието горе се сменяше мигом, а съдържанието оставаше старото, докато
       новото се изтегли — и ако изтеглянето се провалеше, рендерът излизаше тихо
       (`if (!r) return;` в почти всеки раздел) и под новото заглавие оставаше
       ЧУЖДИЯТ екран. Библиотекарката гледаше „Читатели“ с таблицата на книгите. */
    v.innerHTML = '<div class="viewLoading" role="status">Зарежда се…</div>';
  }
  /* finally, а не просто ред след await-а: рендерът може и да ХВЪРЛИ (тогава
     известието идва от мрежата за отхвърлени обещания по-долу). И в двата случая
     мястото не бива да остане с надпис „Зарежда се…“ завинаги. */
  const мой = VIEW;
  try {
    await RENDERERS[VIEW]();
  } finally {
    /* Рендерът не е сложил нищо върху мястото: значи е излязъл по тихия път.
       Тогава се казва наяве. Самата причина вече е показана като известие.
       Сравнението с „мой“ е задължително (открито при прегледа на кръга): при
       бързо превключване, докато базата се бави, ЗАКЪСНЕЛИЯТ рендер иначе
       щамповаше червената кутия върху „Зарежда се…“ на СЛЕДВАЩИЯ раздел, който
       още се зарежда съвсем нормално. */
    if (VIEW === мой && v && v.firstElementChild && v.firstElementChild.classList.contains('viewLoading')) {
      v.innerHTML = '<div class="note err viewFailed" role="alert"><b>Разделът не се зареди.</b> '
        + 'Данните не бяха получени — вижте съобщението за грешката. '
        + '<button type="button" class="btn sm" onclick="route()">Опитай пак</button></div>';
    }
  }
