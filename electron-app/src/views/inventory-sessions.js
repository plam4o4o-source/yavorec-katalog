...
async function draftFromMissing(sessionId) {
  ...
  const legacy = miss.filter(m => invQty(m) > 1);
  const ids = miss.map(m => m.book_id);
  if (legacy.length) {
    ...
    for (const m of legacy) {
      const r = await window.api.books.splitCopies(m.book_id);
      if (r && r.ok) { ids.push(...(r.data.createdIds || [])); continue; }
      ...
      if (r && /вече е за един екземпляр/.test(r.error || '')) {
        toast('Инв. № ' + m.inv_number + ' вече е разделен на отделни записи — в проекта влиза само той. '
          + 'Добавете в проекта и останалите липсващи екземпляри по новите им номера.', 'warn');
        continue;
      }
      return toast((r && r.error) || ('Записът с инв. № ' + m.inv_number + ' не можа да бъде разделен.'), 'err');
    }
  }
