  ...
  if (b) {
    ...
    box.innerHTML += `<div class="card scanHit">
      <div class="scanHit-head"><b>Документ</b> · инв. № ${b.inv_number ?? '—'}
        ${/* „ОТВОРИ КАРТА“ ВЕЧЕ МИНАВА ПРЕЗ ПОТВЪРЖДЕНИЕТО ПО ЧЛ. 17, АЛ. 2
              (v2.4.65, кръг 42, находка Б11).
              (а) КАКВО СТАВАШЕ ДОТУК. Бутонът викаше bookForm(id) — направо
                  формата за редакция на документа.
              (б) ЗАЩО Е ГРЕШНО. Редакцията на вписан документ нарочно минава
                  само през „Инвентарна книга“ и само след изричен въпрос
                  (invBookEdit, от v1.71.0): инвентарната книга е официалният
                  регистър на фонда по Наредба № 3, съхранява се безсрочно, а
                  проверката по чл. 17, ал. 2 иска отговор за всеки вписан ред.
                  Надписът в „Книги“ обещава на библиотекарката точно това — че
                  редакция става само оттам. Таблото обаче е екранът с баркод
                  четеца, тоест най-бързият и най-често ползваният път до един
                  документ, и той заобикаляше обещанието: сканира се книга,
                  натиска се „Отвори карта“ и формата се отваря без нито дума.
              (в) ЗАЩО ПОПРАВКАТА Е ТОЧНО ТАЗИ. Не се прави втора проверка тук —
                  вика се СЪЩАТА invBookEdit, за да има едно-единствено място,
                  на което се пита, и един-единствен текст. invBookEdit вече
                  изтегля картона от базата, когато списъкът на инвентарната
                  книга не е зареден (какъвто е случаят на Таблото), за да
                  назове документа по автор, заглавие и инвентарен номер. */''}
        <button class="btn sm" style="float:right" onclick="invBookEdit(${b.id})"
          title="Отваря картона за редакция — инвентарната книга е официален регистър, затова пита преди това">Отвори карта</button></div>
