...
async function renderCatalog() {
  ...
  $('#view').innerHTML = `
    ...
    <div class="card"><h3 style="margin-top:0">Работна папка (git clone на хранилището)</h3>
      ${status.folder ? `
        ${/* ТЕКСТЪТ КАЗВА КОГА, А НЕ САМО ЧЕ (v2.4.64). Записът вече има две скорости:
              промените по фонда влизат във файла веднага (4 секунди), а заемането и
              връщането — до около минута, защото при 15 000 документа всеки запис е
              4,82 МБ и 180 ms, а гишето сканира през 10 – 20 секунди. Това е видима
              за библиотекаря разлика: натиска „Генерирай katalog.json“ веднага след
              връщане и трябва да знае, че бутонът записва и него, без да чака. */''}
        ...
        отчисляване) — веднага; а заемането и връщането на гишето — до около минута, за да не се пренаписва
        целият файл при всяко сканиране. Бутонът „Генерирай katalog.json“ по-долу записва всичко натрупано веднага.
        ${status.isGitRepo
...
/* ВЪПРОСЪТ КАЗВА КАКВО СЛЕДВА, А НЕ САМО КАКВО СПИРА (v2.4.65, кръг 42, Б21).
   ...
   „Изключено.“ — а последицата е, че от този миг ПУБЛИЧНИЯТ каталог на сайта
   на читалището замръзва: файлът остава качен и достъпен, но вече не следва
   инвентарната книга. Читателят продължава да вижда наличности, които може и
   да не съществуват. Самият ред в одитната следа се вписва в обработчика
   (handlers/catalog.js) — екранът само предупреждава по-рано. */
async function catalogDisconnect() {
  if (!await askConfirm('СПИРАНЕ НА ОНЛАЙН КАТАЛОГА\n\n'
    + 'Програмата ще спре да записва katalog.json в свързаната папка.\n\n'
    + 'Вече публикуваният файл НЕ се изтрива: онлайн каталогът остава достъпен в интернет, но '
    + 'замръзва в сегашния си вид и от днес нататък няма да отговаря на инвентарната книга — '
    + 'читателите ще виждат стар фонд и стари наличности.\n\n'
    + 'Ако искате каталогът да изчезне от сайта, премахнете файла ръчно от папката или от '
    + 'хранилището в GitHub.', { kind: 'danger', okLabel: 'Спри записа' })) return;
  await call(window.api.catalog.disconnectFolder(),
    'Онлайн каталогът е спрян. Публикуваният файл остава в интернет непроменен.');
  renderCatalog();
}
