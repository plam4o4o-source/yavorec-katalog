...
/* ЕДИН ТАВАН, А НЕ ДВА (v2.4.65, кръг 42, находка В9).
   (а) КАКВО СТАВАШЕ ДОТУК. Тук заявеният прозорец се режеше на 2 000 реда, а
       таванът на изчертаните редове (RENDER_MAX_ROWS в src/views/core.js) е
       3 000. Двете числа не си говореха: библиотекарка с 3 500 документа
       натискаше „Покажи още“ по 300 реда, стигаше до 3 000 и виждаше честния
       надпис „Показани са първите 3 000 реда от 3 500…“. След това записваше
       или изтриваше една книга — списъкът се презарежда с booksFetch(0,
       BOOKS_RENDER_LIMIT), тоест иска 3 000 — и получаваше 2 000. Таблицата се
       свиваше с 1 000 реда, а надписът изчезваше, защото таванът вече не беше
       достигнат: на негово място оставаше обикновеното „Покажи още“.
   (б) ЗАЩО Е ГРЕШНО ЗА БИБЛИОТЕКАТА. Екранът мълком показва различен брой редове
       за един и същ резултат, и то СЛЕД запис — точно в мига, в който
       библиотекарката проверява дали промяната ѝ е влязла. А изречението, заради
       което отрязаният списък изобщо е честен („Останалите ги има — просто не се
       изчертават наведнъж“), изчезва. Мълком отрязан списък е точно положението,
       което core.js описва като по-лошото от двете.
   (в) ЗАЩО ПОПРАВКАТА Е ТОЧНО ТАЗИ. Числото не се преписва, а се взима от самия
...
function booksRowsHtml(shown) {
  return shown.length ? shown.map(b => `
    <tr data-id="${b.id}">
      ...
      ${/* НАЛИЧНОСТ НЕ СЕ РИСУВА ЗА ОТЧИСЛЕН ДОКУМЕНТ (v2.4.57).
            ==========================================================
            Дотук редът на отчислен документ носеше ДВЕ значки една до друга:
            червената „отчислен“ и зелената „1/1“. Втората не е просто излишна —
            тя е невярна, и то СЛЕДСТВИЕ на самото отчисляване: `available` се
            смята като бройки минус отворени заемания (BOOK_SELECT в
            handlers/books.js), а актът затваря заеманията (closeLoans в
            handlers/deaccession-acts.js). Тоест книга, заета от читател и
            отчислена по чл. 30, т. 6 („невърнати от ползватели“), СКАЧА от
            „0/1“ на „1/1“ точно в мига, в който излиза от фонда — екранът
            обявява за налично това, което библиотеката току-що е отписала.

            Не се нулира стойността, а се спира рисуването ѝ. Нулирането
            изглежда по-честно и е по-опасно: inventory.quantity е ОТЧЕТНАТА
            бройка и по нея books:multiCopyRecords/splitCopies разпознават
            старите многоекземплярни записи; освен това анулирането на акта
            трябва да върне документа такъв, какъвто е бил. Числото се чете
            погрешно на екрана — там му е и поправката. */''}
      <td>${b.status === 'отчислен'
        ? `<span class="badge" title="Отчисленият документ не е част от фонда — наличност не се брои за него.">—</span>`
        : `<span class="badge ${b.available > 0 ? 'ok' : 'warn'}">${b.available}/${b.quantity}</span>`}</td>
...
async function bookAftermath(res, bookId) {
  ...
  /* Читателят, поискал книгата, е единственият човек, за когото със сигурност
     се знае, че я иска — и дотук беше последният, който научаваше. Не се
     затваря автоматично: съвпадението е по заглавие и автор, а библиотекарят е
...
window.bookAftermath = bookAftermath;
