...
function acqCountNote(a) {
  ...
  if (!(a.items || []).length) {
    return `<b>Забележка:</b> към момента на отпечатване по партидата няма нито един инвентиран документ.
      Настоящият документ удостоверява само общата регистрация по чл. 14 и НЕ съдържа опис на конкретни документи.<br>`;
  }
  return `<b>Забележка:</b> обявеният общ брой (${declared}) не съвпада със сбора на изброените по-долу инвентирани документи (${listed}). `
    + (listed < declared
...
async function printDonationDoc(id) {
  ...
  doPrint(`<div class="pdoc">${shead()}
    ...
    <b>${declared != null ? 'Обща стойност по документа' : 'Обща стойност (изчислена по инвентираните документи, не е обявена в документа)'}:</b>
    ${mny(declared != null ? declared : acqValue(a.items))}<br>
    ${(declared != null && acqDiffers(declared, a.items)) ? `<b>Забележка:</b> обявената стойност (${mny(declared)}) се различава от сбора
...
async function printAcqNoDocDoc(id) {
  ...
  doPrint(`<div class="pdoc">${shead()}
    ...
    <b>${declared != null ? 'Обща оценена стойност' : 'Обща стойност по описа (сбор на оценките на изброените документи)'}:</b>
    ${mny(declared != null ? declared : acqValue(a.items))}
    ${(declared != null && acqDiffers(declared, a.items)) ? `<br><b>Забележка:</b> обявената при завеждането стойност (${mny(declared)}) се различава от
    сбора на инвентираните до момента документи (${mny(acqValue(a.items))}).` : ''}
    <br>${acqCountNote(a)}
    ${a.note ? '<b>Забележка:</b> ' + esc(a.note) : ''}</div>
