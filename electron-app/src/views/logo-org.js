...
async function printReaderCard(id) {
  ...
  const piiWhy = {
    ...
    /* Отделено от 'unreadable' (одит v2.4.18): при сменена от друго работно място
       парола данните са здрави и се връщат с едно отключване. Обединени, двете
       състояния караха картона да праща библиотекаря да въвежда наново ЕГН, което
       не е загубено — обратната грешка на поправяната.
       Одит v2.4.20: без обещанието „Данните са запазени“. Същото състояние настъпва
       и когато единствените криптирани редове в базата са повредени — тогава
       отключването отказва (виж бележката при maskOne в handlers/pdp.js) и
       обещанието на подписан документ би било невярно. Стъпките са верни и за
       двата случая: първо отключване, чак при отказ — въвеждане наново. */
    stale: 'ключът от тази сесия вече не разчита записаните стойности — най-често защото паролата е сменена '
      + 'от друго работно място. Заключете и отключете защитата наново с актуалната парола и отпечатайте пак; '
      + 'въвеждайте стойността наново само ако и отключването откаже.',
    unreadable: 'записаната стойност не се разчита с текущата парола за защита на личните данни. Отключването '
      + 'не помага — стойността трябва да бъде въведена наново в картотеката.'
