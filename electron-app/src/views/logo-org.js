...
async function printReaderCard(id) {
  ...
  const piiWhy = {
    ...
    /* Отделено от 'unreadable' (одит v2.4.18): при сменена от друго работно място
       парола данните са НАПЪЛНО ЗДРАВИ и се връщат с едно отключване. Обединени,
       двете състояния караха картона да праща библиотекаря да въвежда наново ЕГН,
       което не е загубено — обратната грешка на поправяната. */
    stale: 'паролата за защита на личните данни е сменена от друго работно място, докато програмата е била '
      + 'отворена тук. Данните са запазени — отключете наново с новата парола и отпечатайте пак.',
