...
/* КОЛОНАТА „ВЪРНАТ НА“ НЕ ЛЪЖЕ ЗА НЕВЪРНАТИТЕ ДОКУМЕНТИ (v2.4.61).
   =====================================================================
   КАКВО СТАВАШЕ ДОТУК. Колоната печаташе loans.date_in, каквото и да означава
   той. А от v2.4.56 date_in се попълва и когато заемането е приключено като
   ИЗГУБЕНО („Документът е изгубен“ — loans:markLost слага date_in, за да не виси
   бройката заета завинаги), а от v2.4.61 и когато акт по чл. 30, т. 5 закрива
   заемането на невърнат документ (handlers/deaccession-acts.js). И в двата
   случая книгата е у читателя или изобщо я няма — а неговият подписан картон,
   който се подрежда в картотеката и се вади при спор, твърдеше „Върнат на
   18.09.2026 г.“. Това е най-лошото място, на което програмата може да сгреши:
   картонът е хартиеният документ, с който библиотеката доказва какво е дала и
   какво ѝ е върнато.
   КАКВО ПИШЕ СЕГА. „изгубен“ за заемане, приключено от гишето (lost = 1), и
   „невърнат (акт за отчисляване)“ за заемане, закрито от акт — двата случая
   се различават, защото първият още може да се уреди с читателя (обезщетение или
   замяна), а вторият вече е минал през комисия. Датата остава само там, където
   документът наистина се е върнал. */
function loanEndCell(l) {
  if (l && l.deaccession_act_id) return 'невърнат (акт за отчисляване)';
  if (l && l.lost) return 'изгубен';
  return l && l.date_in ? bg(l.date_in) : '';
}
...
async function printReaderCard(id) {
  ...
  doPrint(`<div class="pdoc">${shead()}
    ...
    ${loans.slice(0, CARD_LOAN_ROWS).map(l => `<tr><td>${bg(l.date_out)}</td><td>${l.inv_number ?? ''}</td><td>${esc(l.title)}</td>
      <td>${bg(l.date_due) || ''}</td><td>${loanEndCell(l)}</td></tr>`).join('')}
    </tbody></table>
    ...
    ${/* Одит v2.4.56: картонът свършваше направо с „Подпис на читателя: ………“, а в
          целия документ нямаше НИТО ЕДНО изречение, под което този подпис да стои.
          Двете неща, които подписът на читателя трябва да удостоверява, липсваха
          изобщо: (1) че е запознат с Правилата за обслужване на читателите
          (чл. 47, ал. 2 от Наредба № 3 — библиотеката приема правила и ползвателят
          се задължава да ги спазва) и (2) съгласието за обработване на личните му
          данни, макар самата програма да го пази в readers.gdpr_consent /
          .gdpr_consent_date и да го показва на екрана. Датата на съгласието също
          не се печаташе никъде.
          Защо това има значение: при проверка (както на библиотечната документация,
          така и при въпрос по защитата на личните данни) ДОКАЗАТЕЛСТВОТО е хартията
          с подписа — записът в програмата е само вътрешно твърдение на самата
          библиотека. Подпис под нищо не доказва нищо, а картонът е единственият
          документ, който гражданинът подписва.
          Датата се печата САМО когато я има. Когато я няма, това се казва с думи,
    ...
    <div class="pmeta" style="margin-top:4mm">
      ...
      1. Запознат/а съм с Правилата за обслужване на читателите на библиотеката и се задължавам да ги спазвам
      (чл. 47, ал. 2 от Наредба № 3 от 18.11.2014 г. за съхраняването, ползването и разпореждането с документи
      от библиотечния фонд).<br>
      2. Давам съгласието си библиотеката да обработва личните ми данни, вписани в този картон, за целите на
      библиотечното обслужване и на отчетността по Наредба № 3, при условията на Регламент (ЕС) 2016/679 (ЗЗЛД).
      Известно ми е, че имам право на достъп, поправка и заличаване на данните си и че мога да оттегля
      съгласието си по всяко време.<br>
      ${r.gdpr_consent_date
        ? `Съгласие за обработване на лични данни — отбелязано в библиотечната документация на
           <b>${bg(r.gdpr_consent_date)}</b> г.`
        : (r.gdpr_consent
          ? 'Съгласие за обработване на лични данни е отбелязано в библиотечната документация, но без дата — '
            + 'подписът и датата по-долу са документът за него.'
          : 'В библиотечната документация няма отбелязано съгласие за обработване на лични данни — '
            + 'подписът и датата по-долу са документът за него.')}
      ${r.guarantor_name ? `<br>За читател под 14 години картонът се подписва от родителя/настойника
...
async function printOverdueNotices() {
  ...
  /* ДАТА И АДРЕСАТ НА ПИСМОТО (одит на печатните документи, v2.4.61).
     ...
     предупреждаваше за преустановяване на достъпа — и никъде по листа не пишеше
     кой е този ден. Получателят няма как да разбере кога е писано, библиотеката
     няма как да докаже, че е напомняла преди да наложи наказание, а самата сума
     расте с всеки ден и без дата е непроверима. Разписката за заемане
     (printLoanSlip в src/views/loans.js) отдавна носи „Дата:“ — тук липсваше.
     АДРЕСАТЪТ. За читател под 14 години писмото се адресира до
     родителя/настойника, „чрез“ когото се води детето — данните идват готови от
     loans:overdueByReader (виж дългата бележка там). Името на детето остава в
     обръщението, иначе родителят няма да разбере за кого е писмото. */
  doPrint(rows.map(r => `<div class="pdoc">${shead()}
    <h2>НАПОМНИТЕЛНО ПИСМО</h2>
    <div class="pmeta">Дата: <b>${bg(today())}</b> г.<br>
    До: <b>${esc(r.notice_to || r.name)}</b>${r.notice_via_guarantor
      ? ` <span style="font-size:9pt">(${esc(r.notice_via_guarantor)} на ${esc(r.name)} — читател под 14 г.)</span>` : ''}<br>
    Адрес: ${esc(r.address2 || r.address || '…………………')}<br><br>
    ${r.notice_via_guarantor ? 'Уважаеми/а родителю/настойнико,' : 'Уважаеми/а читателю,'}<br><br>
    Съгласно чл. 43, ал. 1 от Наредба № 3 от 18.11.2014 г. всеки ползвател е длъжен да върне заетите библиотечни документи
    в определения срок. Според нашата документация срокът на изброените по-долу документи${r.notice_via_guarantor
      ? ', заети от ' + esc(r.name) + ',' : ''} е изтекъл.</div>
