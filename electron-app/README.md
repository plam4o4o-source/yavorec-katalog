...
инсталатора: `Инвентар Setup 1.2.3.exe`):
...
2. Build:
   ...
   - `Инвентар Setup X.Y.Z.exe` (самият инсталатор)
   - `Инвентар Setup X.Y.Z.exe.blockmap`
...
| Баркод етикети | автоматичен печат на етикети Code 39: за фонда (име на библиотеката + баркод + инв. номер на един етикет), отделни етикети за сигнатура (само УДК + авторски знак), читателски карти; формат A4 лист или ролков лейбъл принтер; проверка на баркод четец |
