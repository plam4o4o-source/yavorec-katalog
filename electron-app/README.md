# InvLib — десктоп приложение (Electron + SQLite)
...
извеждане в библиотечни формати (UNIMARC/MARCXML, Dublin Core), въвеждане на данни
...
на Windows (`%APPDATA%/inventar-desktop/library.db` — по полето `name` в
`package.json`, не по `productName`/визуалното име на програмата; виж
`docs/ARCHITECTURE.md`), а не вътре в инсталационната папка на програмата — така
потребителските данни оцеляват при преинсталиране/ъпдейт.
...
Резултатът (`InvLib-Setup-<версия>.exe`) се появява в папка `dist/`, заедно
с `InvLib-Setup-<версия>.exe.blockmap` и `latest.yml` (нужни за автоматичното
...
не открива файла (проверено и поправено). `productName` в конфигурацията е
„InvLib“ (до v2.4.10 включително беше „Инвентар“) — определя изпълнимия файл,
преките пътища и записа в „Приложения“ на Windows. Смяната му НЕ пипа папката
с данните (тя се определя от полето `name`, не от `productName` — виж
`docs/ARCHITECTURE.md`) и НЕ преименува инсталационната папка на вече
монтирани библиотеки: electron-builder/NSIS засича съществуваща инсталация по
`appId` (непроменен) и преизползва записания в регистъра път, а преките
пътища (Старт меню/Работен плот) се преименуват на място вместо да се
дублират — проверено чрез разчитане на действителната логика в
...
инсталатора: `InvLib-Setup-1.2.3.exe`):
...
2. Build:
   ...
   - `InvLib-Setup-X.Y.Z.exe` (самият инсталатор)
   - `InvLib-Setup-X.Y.Z.exe.blockmap`
