...
/* ОТКРИВАНЕ НА ЕДНОВРЕМЕННА РЕДАКЦИЯ (v2.4.56).
   =====================================================================
   Програмата изрично поддържа няколко работни места към една база на мрежов
   диск. Транзакциите са коректни и техническа повреда няма — но записът беше
   ...
   отворена отпреди малко форма, записва адрес — и телефонът изчезва, без нищо
   да се случи на екрана. Открива се месеци по-късно, ако изобщо. Одитната следа
   пази какво е било, но никой не гледа там, докато не заподозре нещо.

   Вместо нова колона `updated_at` (която иска миграция във всяка съществуваща
   база и тригер при всеки запис) се ползва ОТПЕЧАТЪК на самия ред: формата
   получава отпечатъка при отварянето и го връща при записа. Ако междувременно
   някой е пипнал ред, отпечатъкът не съвпада и записът се отказва — с имената
   на полетата, които ДРУГИЯТ е променил, за да реши човекът какво да прави.
   Отпечатъкът е върху същите полета, които формата редактира: промяна, направена
   от самата програма по друг повод (например дата на статус), не бива да отказва
   редакция, която не я засяга. */
function rowFingerprint(row, fields) {
  const src = fields.map(f => {
    const v = row ? row[f] : null;
    return f + '=' + (v === null || v === undefined ? '' : String(v));
  }).join('\u0001');
  // FNV-1a, 32 бита: достатъчно за „същият ли е редът“ и без зависимости.
  let h = 0x811c9dc5;
  for (let i = 0; i < src.length; i++) {
    h ^= src.charCodeAt(i) & 0xff;
    h = (h + ((h << 1) + (h << 4) + (h << 7) + (h << 8) + (h << 24))) >>> 0;
    h ^= (src.charCodeAt(i) >> 8) & 0xff;
    h = (h + ((h << 1) + (h << 4) + (h << 7) + (h << 8) + (h << 24))) >>> 0;
  }
  return h.toString(36);
}
/* Хвърля разбираем отказ, ако редът е сменен между отварянето и записа.
   `rev` липсва при по-стар изглед или при повикване от тест — тогава проверката
   се прескача нарочно: поправката не бива да счупи път, който не я подава. */
function assertUnchanged(prev, rev, fields, what) {
  // `what` е цяло изречение до запетаята („Документът е променен“) — на български
  // родът се сменя с думата и слепването „променен(а)“ чете зле точно на екрана,
  // на който човекът тепърва разбира, че работата му е спряна.
  if (!rev) return;
  const now = rowFingerprint(prev, fields);
  if (now === rev) return;
  throw new Error(what + ' от друго работно място, откакто отворихте формата. '
