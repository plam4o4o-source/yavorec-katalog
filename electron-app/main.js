...
/* Пазач НАПРЕД по версия на схемата (одит v2.4.18, преглед на поправките от
   ...
   ЗАЩО ТОЧНО ТУК, а не в runMigrations(): първият вариант на този пазач (v2.4.18)
   стоеше именно там — тоест СЛЕД `journal_mode`, след schema.sql, след всички
   ensureColumns() и след старите backfill-и в initDb(). Измерено с истинския
   main.js срещу здрава база с user_version = 99: файлът сменяше контролната си
   сума, `readers.gdpr_consent_date` се пренаписваше от NULL на дата от стария
   backfill, базата се преобразуваше в WAL и оставаше с -wal/-shm файлове до нея.
   Тоест пазачът позволяваше точно това, срещу което съществува (по-стар код
   пренаписва данни, които по-новата версия чете другояче), а диалогът твърдеше
   „не ѝ е направено нищо“ — неверен текст пред библиотекаря. Затова проверката е
   ПЪРВОТО нещо след отварянето на файла: `PRAGMA user_version` само чете.
...
function assertSchemaNotNewer() {
  ...
  /* Пътят до настройката за папката на базата — единственият изход, ако това
     работно място сочи към чужда (мрежова) папка: прозорецът не се отваря и
     „Настройки“ → „Работа в мрежа“ е недостижимо. */
