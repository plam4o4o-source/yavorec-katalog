...
function initDb() {
  ...
  /* Анулиране на акт (v2.4.56) — колоните се добавят ТУК, а не само лениво в
     handlers/deaccession-acts.js, защото шест други места ги четат: КДБФ,
     таблото, статистиката (два пъти) и справките броят отчислените ПРЕЗ
     revoked_at IS NULL. Ако колоната се появяваше едва при първото пипане на
  ...
  /* Изгубен/невърнат документ (v2.4.56) — по същата причина като по-горе: тези
     ...
     срокове“ през COALESCE(lost,0) = 0, актът за отчисляване чете кой читател е
     изгубил документа, а settings:update пише двете нови настройки. Ако се
     появяваха едва при първото приключване на изгубено заемане, всяко от трите
     щеше да гърми на база, в която такова заемане още няма. */
  ensureColumns('loans', {
  ...
  /* ВГРАДЕНАТА ТАБЛИЦА ЗА АВТОРСКИ ЗНАК (v2.4.63).
     =====================================================================
     Дотук програмата не носеше таблица: докато библиотекарката не посочи свой
     файл, копчето „Предложи“ до полето „Авторски знак“ нямаше откъде да
     предлага и сигнатурата (реквизит по чл. 16) се пишеше на ръка за всяка
     книга. Сега числата идват с програмата (db/author-table.js).

     ЗАЩО НЕ Е МИГРАЦИЯ, макар да е „еднократно попълване на данни“. Миграцията
     вдига PRAGMA user_version, а пазачът assertSchemaNotNewer() СПИРА всяка
     станция, чиято програма познава по-ниска версия — и това е правилно, когато
     схемата наистина се е променила. Тук не се променя НИЩО по схемата: това са
     само редове в таблица, с които станция на предишната версия работи без
     никакъв проблем. Вдигане на версията заради тях би заключило второто
     работно място в деня на обновяването (виж дългата бележка при
     CURRENT_SCHEMA_VERSION — разминаването е НОРМАЛНОТО състояние в този ден)
     за нещо, което не го засяга. Затова засяването стои тук, до празниците:
     проверява се условие, а не версия, и е безопасно при всяко пускане.

     САМО В ПРАЗНА ТАБЛИЦА. Библиотека, която вече е внесла СВОЕТО издание, не
     бива да го загуби: нейните знаци са по нейните числа и подреждат нейния
     рафт, а две различни издания дават различни числа за една и съща фамилия.
     Ръчният път назад е „Настройки“ → „Фонд“ → „Авторски знак“ → „Върни
     вградената“, където презаписването е изричен избор на човека.

     В ЕДНА ТРАНЗАКЦИЯ и с хваната грешка: при прекъсване по средата НЕ бива да
     остане наполовина пълна таблица — търсенето взима най-голямото
     буквосъчетание, което не надминава фамилията, тоест липсващите редове не
     личат никъде, а дават МЪЛЧАЛИВО други числа. Или цялата, или никаква;
     следващото пускане опитва пак. Провалът не спира програмата — таблицата е
     удобство, не условие за работа с фонда. */
  try {
    if (!db.prepare('SELECT COUNT(*) AS n FROM author_table').get().n) {
      const rows = require('./handlers/author-mark').builtinRows();
      if (rows.length) {
        const ins = db.prepare('INSERT OR REPLACE INTO author_table (prefix, mark) VALUES (?, ?)');
        db.transaction(() => { for (const r of rows) ins.run(r.prefix, r.mark); }).immediate();
        /* Следата се пише ИЗВЪН транзакцията на данните и със своя защита: тя е
           също запис в базата и грешка в нея не бива да отменя вече заредената
           таблица, нито да спира стартирането. */
        try {
          logAudit('Таблица за авторски знак', 'вградената таблица е заредена — ' + rows.length
            + ' реда. Копчето „Предложи“ до полето „Авторски знак“ вече предлага знак по фамилията. '
            + 'Ако библиотеката работи по СВОЕ издание на авторските таблици, внесете го от '
            + '„Настройки“ → „Фонд“ → „Авторски знак“ — внесеното застава върху вграденото, а вече '
            + 'записаните знаци по документите не се променят от нищо тук.');
        } catch (e) { console.error('Следа за вградената таблица:', e.message); }
      }
    }
  } catch (err) {
    console.error('Вградената таблица за авторски знак не се зареди:', err.message);
  }
...
const MIGRATIONS = [
  ...
  /* ============================ ЛЕВА → ЕВРО (v2.4.51) ============================
     От 01.01.2026 г. лицевата валута на страната е еврото и официалните
     документи по Наредба № 3 (инвентарна книга, КДБФ, актовете за отчисляване)
     се водят в него. Дотук програмата пазеше сумите в ЛЕВА и смяташе еврото при
     показване — което е грешно не само козметично: сборът от закръглени
     преобразувания НЕ е равен на преобразувания сбор, тоест инвентарната книга
     показваше обща стойност, която не съвпада със сбора на собствените си
     редове. Затова стойностите се преобразуват ВЕДНЪЖ, тук, и оттук нататък
     всяка сметка се прави в записаната валута.

     Курсът е фиксираният и необратим по Регламент (ЕС) 2025/1409 — 1.95583,
     същият, който стои в EUR_RATE горе в този файл и в src/views/core.js.

     Закръгляването е до цял ЕВРОцент, както изисква преобразуването: без него
     цената остава с плаваща опашка и всеки сбор я влачи. `ROUND(x, 2)` на SQLite
     е точно това.

     Кои колони НЕ се пипат, макар да са REAL — проверено в schema.sql, не по име:
       • circulation_rules.suspend_per_day и settings.suspend_per_day — ДНИ
         преустановено заемане, не пари;
       • settings.lbl_gap и settings.lbl_margin — милиметри по листа с етикети.
     Преобразуването им би счупило наказанията и печата на етикети.

     Резервно копие: миграциите текат в транзакция (виж runMigrations()), а
     програмата прави автоматично копие при пускане — този ред е необратим по
     съдържание, затова копието отпреди обновяването остава единственият път
     назад и е нарочно оставено недокоснато. */
  { version: 15, run: () => {
    const вЕвро = (таблица, колона) => db.prepare(
      `UPDATE ${таблица} SET ${колона} = ROUND(${колона} / ${EUR_RATE}, 2) WHERE ${колона} IS NOT NULL`
    ).run();
    /* Сборът на цените по партида се снима ПРЕДИ преобразуването на цените —
       после същата заявка дава вече евро и сравнението „обявена = сбор“ би
       сравнявало лева с евро. */
    const сборПоПартида = () => new Map(db.prepare(`
      SELECT b.acquisition_id AS id, COALESCE(SUM(b.price * COALESCE(inv.quantity, 1)), 0) AS v
      FROM books b LEFT JOIN inventory inv ON inv.book_id = b.id
      WHERE b.acquisition_id IS NOT NULL GROUP BY b.acquisition_id
    `).all().map(r => [r.id, r.v]));
    const сборПреди = сборПоПартида();
    const партиди = db.prepare('SELECT id, sum FROM acquisitions WHERE sum IS NOT NULL').all();

    вЕвро('books', 'price');

    /* Обявената по документа стойност се преобразува така, че партида, в която
       тя Е СЪВПАДАЛА със сбора на цените, да съвпада и след това. Самостоятелно
       закръгляне ги разминава с цент (5 × 2.99 лв. и обявени 14.95 лв. дават
       7.65 € срещу 7.64 €) и печатът добавя абзаца „обявената стойност се
       различава от сбора“ върху ВЕЧЕ ПОДПИСАН акт за дарение (открито при
       прегледа на кръга). Партида, при която двете и преди са се разминавали,
       се закръгля самостоятелно — разминаването е факт от документа. */
    const сборСлед = сборПоПартида();
    const цент = (x) => Math.round((Number(x) || 0) * 100);
    const поставиОбявена = db.prepare('UPDATE acquisitions SET sum = ? WHERE id = ?');
    for (const п of партиди) {
      const съвпадаше = цент(п.sum) === цент(сборПреди.get(п.id) || 0);
      поставиОбявена.run(
        съвпадаше ? Math.round((сборСлед.get(п.id) || 0) * 100) / 100
                  : Math.round(п.sum / EUR_RATE * 100) / 100, п.id);
    }
    вЕвро('deaccession_items', 'price');
    вЕвро('loans', 'fine');
    /* Сметките на читателите се преобразуват КУМУЛАТИВНО по читател, а не ред по
       ред. Ред по ред закръглянето се натрупва и оставя фантомно салдо по вече
       погасена сметка: +0.05, +0.05, −0.10 лв. (баланс 0) дава +0.03, +0.03,
       −0.05 = +0.01 €, тоест читател, който не дължи нищо, светва в червено и не
       може да бъде изтрит. Тук всеки ред се смята така, че ТЕКУЩИЯТ баланс след
       него да е точното закръгляне на стария баланс — стандартният начин за
       преобразуване на валута по редове. Крайният баланс съвпада винаги, а
       отделният ред се разминава най-много с един евроцент. */
    const редове = db.prepare('SELECT id, reader_id, amount FROM account_lines ORDER BY reader_id, id').all();
    const поставиСума = db.prepare('UPDATE account_lines SET amount = ? WHERE id = ?');
    let текущЧитател = null, кумЛева = 0, кумЕвро = 0;
    for (const р of редове) {
      if (р.reader_id !== текущЧитател) { текущЧитател = р.reader_id; кумЛева = 0; кумЕвро = 0; }
      kумулирай(р);
    }
    function kумулирай(р) {
      kумЛеваДобави(р.amount);
      const целЕвро = Math.round(кумЛева / EUR_RATE * 100) / 100;
      const нов = Math.round((целЕвро - кумЕвро) * 100) / 100;
      кумЕвро = Math.round((кумЕвро + нов) * 100) / 100;
      поставиСума.run(нов, р.id);
    }
    function kумЛеваДобави(x) { кумЛева = Math.round((кумЛева + (Number(x) || 0)) * 1e6) / 1e6; }
    вЕвро('inventory_session_missing', 'price');
...
const backupHandlers = require('./handlers/backup')(ipcMain, {
  ...
  /* readConfig/updateConfig — за втората (незадължителна) папка за копие: пътят
     ѝ е настройка на ТОЗИ компютър (USB/друг диск), не на общата база, затова
...
function createWindow() {
  ...
  /* Отварянето в НОВ ПРОЗОРЕЦ НА ПРИЛОЖЕНИЕТО се отказва винаги — виж одита
     v2.4.14 по-долу. Пропускането към СИСТЕМНИЯ БРАУЗЪР е разрешено само за
     изрично изброени адреси, а не с общо правило за http(s): „Настройки“ →
     „Помощ и обратна връзка“ показва <a target="_blank"> към сайта на
     разработчика (виж DEV_SITE_URL в src/views/settings.js) и, докато този
     списък беше празен, линкът тихо не правеше нищо — открито при проверка
     след жалба, че „сайтът от Настройки не се отваря“. Само за адреси ТОЧНО
     от списъка (не префикс, не поддомейн) и само https. */
  const ALLOWED_EXTERNAL_URLS = new Set(['https://invlib.com/']);
  win.webContents.setWindowOpenHandler(({ url }) => {
    /* Отварянето в нов прозорец се отказва винаги. Пропускането към браузъра е
       ОГРАНИЧЕНО (одит v2.4.14 + поправка v2.4.57): в програмата няма нито
       една връзка с target="_blank" извън списъка по-горе — проверено с
       търсене — тоест никой друг клон не обслужва нищо, а оставяше изходен
       канал: всяка бъдеща дупка в екранния слой можеше да извика
       window.open('https://…?d=' + данни) и главният процес щеше послушно да го
       отвори в браузъра. Единственото друго външно отваряне, което програмата
       прави, е mailto: в handlers/notices.js, където адресът минава през
       проверка. */
    if (ALLOWED_EXTERNAL_URLS.has(url)) {
      shell.openExternal(url).catch(err => console.warn('Отварянето на', url, 'в браузъра не се получи:', err.message));
      return { action: 'deny' };
    }
...
/* ---------------- Проверка на базата при стартиране (PRAGMA quick_check) ----------------
   Одитът намери НУЛА проверки за цялост в цялата програма: SQLite открива
   повреда чак когато се допре до счупената страница, тоест библиотекарката
   научава за нея по средата на работния ден — при отваряне на един конкретен
   читател или при печат на един конкретен регистър. Дотогава програмата е
   работила „нормално“ и е направила няколко АВТОМАТИЧНИ КОПИЯ на вече
   повредената база, изтласквайки здравите извън прозореца на пазене.

   quick_check (а не integrity_check) е нарочно: прави същия обход на
   страниците, но без скъпата проверка на всички индекси — при база от
   няколко десетки MB отнема части от секундата и не бави стартирането.
   Прави се ПРЕДИ прозореца и преди авто-копието: повредена база не бива да
   стане днешното резервно копие. */
function checkDbIntegrityAtStartup() {
  let res;
  try {
    res = String(db.pragma('quick_check', { simple: true }) || '');
  } catch (err) {
    res = 'проверката не можа да бъде извършена: ' + err.message;
  }
  if (res.toLowerCase() === 'ok') return true;
  logToFile('error', 'PRAGMA quick_check върху базата: ' + res);
  try { logAudit('База данни', 'ВНИМАНИЕ: проверката на базата при стартиране откри повреда: ' + res); }
  catch (e) { /* одитната следа е в същата база — може и да не се запише */ }
  /* Суровият отговор на SQLite („*** in database main *** Page 412: btreeInitPage
     () returns error code 11“) не казва нищо на човек и звучи като обвинение.
     Затова: какво е станало, какво значи и кои са ДВАТА безопасни изхода. */
  const choice = dialog.showMessageBoxSync({
    type: 'warning',
    noLink: true,
    buttons: ['Възстанови резервно копие…', 'Продължи на свой риск', 'Изход'],
    defaultId: 0,
    cancelId: 2,
    title: 'Базата данни показва признаци на повреда',
    message: 'Проверката на базата данни при стартиране откри повреда във файла.',
    detail: 'Това обикновено става при спиране на тока насред запис, при прекъсната връзка с мрежовия '
      + 'диск или при повреден сектор на диска.\n\n'
      + '• „Възстанови резервно копие…“ — безопасният избор. Програмата показва списък с наличните '
      + 'резервни копия; текущият файл се запазва настрани, преди да бъде подменен.\n'
      + '• „Продължи на свой риск“ — програмата ще работи, но част от данните може да липсват или '
      + 'да се появят грешки. Направете резервно копие ВЕДНАГА и го запишете на друг диск.\n'
      + '• „Изход“ — затваря програмата, без да променя нищо.\n\n'
      + 'Технически отговор на проверката (за поддръжката): ' + res
  });
  if (choice === 0) { openEmergencyRestoreWindow('Проверката при стартиране откри повреда в базата данни.'); return false; }
  if (choice === 2) { app.exit(0); return false; }
  return true; // изрично „продължавам на свой риск“
}

/* ---------------- Авариен прозорец за възстановяване ----------------
   Дотук при неотваряема база програмата показваше диалог с инструкция
   библиотекарката САМА да преименува library.db в %APPDATA%\… и чак после да
   пусне програмата отново. За човек, който няма работа с папки на Windows,
   това е задънена улица: екранът за възстановяване е ВЪТРЕ в програмата, а
   програмата не тръгва — тоест точно в мига, в който резервните копия са
   единствената надежда, до тях няма никакъв път през интерфейса.

   Сега: малък прозорец със списъка на наличните копия и бутон „Възстанови
   това“. Целият механизъм по възстановяването е в handlers/backup.js и се
   ползва както си е — през същите канали backup:list и backup:restoreFromList,
   които екранът „Настройки“ вика (предпазно копие, проверки на файла,
   разшифроване, атомарна подмяна, рестарт). Тук няма нито ред логика по
   възстановяване, само списък и бутон.

   Страницата се сглобява тук и се записва във ВРЕМЕННАТА папка, а не в src/:
   аварийният екран трябва да работи и когато самата инсталация е повредена, а
   и няма смисъл да стои като отделен файл, който някой ден ще се разминава с
   останалите изгледи. preload.js се зарежда по обичайния начин, затова
   window.api работи. */
function emergencyRestoreHtml(reason) {
  return `<!doctype html><html lang="bg"><head><meta charset="utf-8">
<title>InvLib — възстановяване на базата данни</title>
<style>
  body { font-family: "Segoe UI", system-ui, sans-serif; background:#f4f1ea; color:#20242c; margin:0; padding:24px; }
  h1 { font-size:19px; margin:0 0 6px }
  .sub { color:#6b6257; font-size:13px; margin:0 0 16px }
  .warn { background:#fdecea; border-left:4px solid #b3261e; padding:10px 12px; border-radius:4px; margin-bottom:16px; font-size:13.5px }
  table { border-collapse:collapse; width:100%; background:#fff; font-size:13px }
  th, td { text-align:left; padding:7px 9px; border-bottom:1px solid #e5e0d6 }
  th { background:#efe9dd; font-weight:600 }
  button { font:inherit; padding:6px 12px; border:1px solid #8a7a5c; background:#8a7a5c; color:#fff; border-radius:4px; cursor:pointer }
  button.sec { background:#fff; color:#20242c }
  input[type=password] { font:inherit; padding:6px; width:230px; border:1px solid #c8bfae; border-radius:4px }
  .row { margin:14px 0 }
  #msg { margin-top:14px; font-size:13.5px }
  .mono { font-family: Consolas, monospace; font-size:12px }
</style></head><body>
<h1>Базата данни не може да бъде отворена</h1>
<p class="sub">${reason ? String(reason).replace(/[<&]/g, '') : ''}</p>
<div class="warn">Изберете резервно копие и натиснете „Възстанови това“. Преди подмяната програмата
записва копие на сегашния файл (<span class="mono">before-restore-…</span>), така че нищо не се губи
безвъзвратно. След възстановяването програмата се стартира наново сама.</div>
<div class="row">Ако копието е криптирано (🔒), въведете паролата за защита на личните данни:
  <input type="password" id="pw" autocomplete="off"></div>
<div id="list">Зареждане на списъка…</div>
<div id="msg"></div>
<script>
  const esc = (s) => String(s == null ? '' : s).replace(/[&<>"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
  const fmt = (ms) => new Date(ms).toLocaleString('bg-BG');
  const kb = (n) => (n / 1048576).toFixed(1) + ' MB';
  async function load() {
    const box = document.getElementById('list');
    const res = await window.api.backup.list();
    if (!res || !res.ok) { box.textContent = 'Списъкът с копия не можа да бъде прочетен: ' + ((res && res.error) || ''); return; }
    const rows = res.data || [];
    if (!rows.length) {
      box.innerHTML = '<b>В папката с резервните копия няма нито един файл.</b> Ако имате копие на USB '
        + 'или на друг компютър, копирайте го в папката с копията и пуснете програмата отново.';
      return;
    }
    box.innerHTML = '<table><thead><tr><th>Файл</th><th>Дата и час</th><th>Размер</th><th>Вид</th><th></th></tr></thead><tbody>'
      + rows.map((b, i) => '<tr><td class="mono">' + esc(b.name) + '</td><td>' + fmt(b.mtime) + '</td><td>' + kb(b.size)
        + '</td><td>' + (b.encrypted ? '🔒 криптирано' : 'обикновено') + '</td>'
        + '<td><button data-i="' + i + '">Възстанови това</button></td></tr>').join('')
      + '</tbody></table>';
    box.querySelectorAll('button').forEach(btn => {
      btn.onclick = () => restore(rows[+btn.dataset.i]);
    });
  }
  async function restore(b) {
    const msg = document.getElementById('msg');
    if (!window.confirm('Сегашната база ще бъде заменена с копието от ' + fmt(b.mtime) + '. Да продължа ли?')) return;
    msg.textContent = 'Възстановяване…';
    const pw = document.getElementById('pw').value;
    const res = await window.api.backup.restoreFromList({ path: b.path, password: pw });
    if (res && res.ok && res.data && res.data.needsPassword) {
      msg.textContent = 'Копието е криптирано — въведете паролата в полето горе и натиснете бутона пак.';
      return;
    }
    if (!res || !res.ok) { msg.textContent = 'Възстановяването не се получи: ' + ((res && res.error) || 'неизвестна грешка'); return; }
    msg.textContent = 'Готово. Програмата се стартира наново…';
  }
  load();
</script></body></html>`;
}
let emergencyWindow = null;
function openEmergencyRestoreWindow(reason) {
  try {
    const file = path.join(app.getPath('temp'), 'invlib-restore-' + process.pid + '.html');
    fs.writeFileSync(file, emergencyRestoreHtml(reason), 'utf8');
    emergencyWindow = new BrowserWindow({
      width: 900, height: 620, title: 'InvLib — възстановяване на базата данни',
      icon: path.join(__dirname, 'icon.ico'),
      webPreferences: { preload: path.join(__dirname, 'preload.js'), contextIsolation: true, nodeIntegration: false }
    });
    emergencyWindow.setMenuBarVisibility(false);
    emergencyWindow.loadFile(file);
    return true;
  } catch (err) {
    logToFile('error', 'Аварийният прозорец за възстановяване не се отвори: ' + err.message);
    /* Последна възможност — старият текстов съвет. По-добре инструкция за ръчна
       работа с файлове, отколкото нищо на екрана. */
    try {
      dialog.showErrorBox('InvLib не можа да отвори екрана за възстановяване',
        'Резервните копия са в папка „backups“ до базата данни:\n' + path.join(resolveDbDir(), 'backups')
        + '\n\nПреименувайте library.db на library-повреден.db (не я триете) и копирайте най-новото копие '
        + 'с име library.db на нейно място. Криптирано копие (.invbak) НЕ става за това — при него се '
        + 'обадете на поддръжката.\n\nГрешка: ' + err.message);
...
app.whenReady().then(() => {
  ...
  /* Проверката на целостта е ПРЕДИ авто-копието и преди прозореца: повредена
     ...
     работи половин ден върху файл, който вече се разпада. Връща false, когато е
     избрала „Изход“ или е тръгнала към аварийното възстановяване — тогава
     прозорецът на програмата изобщо не се отваря. */
  if (!checkDbIntegrityAtStartup()) return;
  ...
  /* И по таймер (на 3 часа, само при променена база), защото компютър в
     ...
     седем дни. Спира се в 'window-all-closed' по-долу, където се прави и
     последното копие за деня. */
  ...
  try {
    ...
    if (locked) {
      /* Заключена база НЕ е повреда — другото работно място просто пише в
         момента. Тук не се предлага възстановяване: то би изтрило работата на
         другия компютър. */
    ...
    /* Дотук на този ред стоеше съвет библиотекарката сама да преименува
       library.db в %APPDATA% и чак после да пусне програмата пак — тоест точно
       когато резервните копия са единствената надежда, пътят до тях минаваше
       през файловите папки на Windows, а екранът за възстановяване оставаше
       недостъпен (той е ВЪТРЕ в програмата, която не тръгва). Сега вместо
       инструкция се отваря самият екран: списък с копията и бутон. */
    const choice = dialog.showMessageBoxSync({
      type: 'error',
      noLink: true,
      buttons: ['Възстанови резервно копие…', 'Изход'],
      defaultId: 0,
      cancelId: 1,
      title: 'InvLib не можа да се стартира',
      message: 'Базата данни не можа да бъде отворена.',
      detail: detail + '\n\n'
        + 'Най-честата причина е повреден файл или недостъпна папка (изключен мрежов диск).\n\n'
        + '• Ако базата е на мрежов диск — проверете дали дискът е включен, затворете това съобщение '
        + 'и пуснете програмата отново.\n'
        + '• Ако дискът е наред, файлът най-вероятно е повреден. Натиснете „Възстанови резервно копие…“: '
        + 'програмата показва наличните копия и връща избраното вместо повредения файл. Сегашният файл '
        + 'се запазва настрани (before-restore-…), нищо не се изтрива безвъзвратно, а криптираните '
        + 'копия се разшифроват автоматично.\n'
        + '• Ако проблемът остава, изпратете дневника от папката „logs“ на разработчика.'
    });
    if (choice === 0 && openEmergencyRestoreWindow('Базата данни не можа да бъде отворена: ' + detail)) {
      return; // прозорецът поема нататък; програмата НЕ излиза
    }
  } catch (e) { /* ако и диалогът не тръгне, поне не увисваме */ }
  app.exit(1);
});
...
app.on('window-all-closed', () => {
  ...
  /* Последно копие за деня — ПРЕДИ db.close(), докато връзката още работи (копието
     ...
     чак при следващото пускане, тоест повреда през нощта отнасяше цялото днешно
     обслужване — заемания, връщания, новопостъпили. Прави се само ако базата
     наистина е променяна след последното копие (виж backup.js). */
...
const { countOverduePeriodicals } =
  /* v2.4.56: периодиката вече инвентира годишни комплекти във фонда, затова ѝ
     трябват същите зависимости като на книгите — общата проверка за лимита на
     записите (иначе модулът носи СВОЕ копие на правилото и двете се разминават
     при първата промяна на съобщението) и насрочването на записа в онлайн
     каталога (иначе новият комплект се появява в katalog.json чак при следващата
     промяна по книга). yearOf е за годината на комплекта. */
  require('./handlers/periodicals')(ipcMain, {
    getDb: () => db, run, logAudit, today, yearOf, checkRecordLimit, scheduleCatalogWrite
  });
...
require('./handlers/export-all')(ipcMain, {
  getDb: () => db, logAudit, today, dialog, getMainWindow: () => mainWindow, fs, csvCell
});
