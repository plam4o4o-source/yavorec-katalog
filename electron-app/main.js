...
function runMigrations() {
  ...
  /* Пазач НАПРЕД (одит v2.4.18, преглед на поправките от v2.4.17). Дотук по-стара
     версия отваряше без възражение база, мигрирана от по-нова: филтърът по-долу
     просто не намираше нищо за прилагане. Това не е теоретично — режимът с две
     работни места към обща мрежова база е изрично поддържан, а те се обновяват
     една по една, тоест разминаването е НОРМАЛНОТО състояние в деня на
     обновяването. Пример от самата версия 2.4.17: миграция 11 обръща смисъла на
     `acquisitions.sum = 0` от „празно поле“ на „обявена нула“. Станция на 2.4.16
     срещу вече мигрирана база продължава да пише 0 за непопълнено поле, а
     обновената станция после печата „Обща стойност по документа: 0.00 лв.“ като
     ОБЯВЕНА нула върху подписан счетоводен документ — точно объркването, което
     миграцията премахна.
     Затова: базата не се докосва и стартирането спира с ясно съобщение. Кодът
     `DB_NEWER_SCHEMA` се разпознава от диалога при старта — генеричният съвет там
     („възстановете резервно копие“) е ОПАСЕН за този случай: базата е напълно
     здрава и по-нова, а възстановяването би изтрило работата на другите станции. */
  if (from > CURRENT_SCHEMA_VERSION) {
    const err = new Error('Базата данни е с версия на схемата ' + from + ', а тази инсталация на InvLib '
...
app.whenReady().then(() => {
  ...
  try {
    ...
    /* Базата е ЗДРАВА, просто по-нова от тази инсталация (виж пазача напред в
       runMigrations). Отделен диалог, защото съветите по-долу — „преименувайте
       library.db“ и „възстановете резервно копие“ — тук биха унищожили работата
       на обновените работни места. */
    if (err && err.code === 'DB_NEWER_SCHEMA') {
      dialog.showErrorBox('InvLib на този компютър е по-стар от базата данни',
        detail + '\n\nБазата НЕ е повредена и не ѝ е направено нищо — програмата спря, за да не '
        + 'запише в нея данни по стария си начин.\n\nКакво да направите:\n'
        ...
        + 'работата, въведена от обновените работни места.');
      app.exit(1);
      return;
    }
