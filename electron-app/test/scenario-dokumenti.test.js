...
test('3. инвентаризация (пълна) и актове за отчисляване — данните в регистрите', async () => {
  ...
  ids.act1 = ok(await h.api.deaccessionActs.create({
    act: Object.assign({ no: 1, date: T, order_no: '12', reason_code: 6,
...
test('5. инвентарна книга: колоните по чл. 16, редовете, сборовете, диапазонът, екранирането', async () => {
  ...
  await soft('5a. глава и сборове', () => {
    ...
    // Отчислени с акт № 1 са 1004 (3 екз.) и 1006 → остават 6 записа = 6 документа.
    assert.equal(copies, 6, 'неотчислените документи'); assert.equal(rows.length - active.length, 2);
  ...
  await soft('5c. редовете — цени, партиди, актове, отметки за проверка', () => {
    ...
    assert.match(p, seq('01.02.' + Y, '1004', 'Стар многоекземплярен запис', '2020', '3', E.mny(4)), 'ред 1004 с 3 бройки и ЕДИНИЧНА цена');
    assert.match(p, seq('1004', 'Стар многоекземплярен запис', '2020', '3', E.mny(4), 'Б-1004', '№ 1 / ' + E.bgDate(T), 'отчислен'), 'ред 1004 носи акт № 1');
...
test('6. акт за отчисляване: списъкът по чл. 35, ал. 2 с бройки, сборът, реквизитите, анулираният', async () => {
  ...
  await soft('6a. екранът на акта — сборът с бройките', () => {
    assert.match(h.modal(), /ОБЩО 4 \(2 заглавия\)/, 'екранът брои документи, не заглавия');
  ...
  await soft('6b. реквизити: номер, дата, заповед, ръководител, комисия, чл. 30, чл. 36', () => {
    ...
    assert.match(p, seq('4', 'библиотечни документа (2 заглавия) на обща стойност', E.mny(15.5)));
  ...
  await soft('6c. таблицата — колона „Бр.“, единична цена с означение, ред ОБЩО', () => {
    for (const col of ['№', 'Инв. №', 'Автор, заглавие, том', 'Година', 'УДК', 'Бр.', 'Стойност']) assert.ok(p.includes(col), 'липсва колона „' + col + '“');
    assert.match(p, seq('1', '1004', 'Стар многоекземплярен запис', '2020', '821.163.2-31', '3', '3 ×', E.mny(4)), 'ред с 3 бройки');
    assert.match(p, seq('2', '1006', 'Намерена при подреждане', '2020', '821.163.2-31', '1', E.mny(3.5)));
    assert.match(p, seq('ОБЩО', '4', E.mny(15.5)));
...
test('7. протокол от инвентаризация: обхват, проверени, липсващи, заети, чл. 41', async () => {
  ...
  await soft('7c. липсващите се броят по ДОКУМЕНТИ, както в акта и в КДБФ', () => {
    ...
    const fq = q('SELECT COALESCE(SUM(COALESCE(i.quantity,1)),0) AS n, COALESCE(SUM(b.price*COALESCE(i.quantity,1)),0) AS v FROM inventory_session_missing m JOIN books b ON b.id=m.book_id LEFT JOIN inventory i ON i.book_id=b.id WHERE m.session_id = ?', ids.sess);
...
test('14. CSV: читатели (без ЕГН), дневник (със сборове), фонд (бройки и стойности)', async () => {
  ...
  await soft('14c. фонд в CSV — бройки, цена в € и лв., обща стойност, състояние', () => {
    ...
    assert.match(row1004, /;"3";"4\.00";"7\.82";"12\.00";"отчислен"$/, 'бройки 3, цена 4 €, 7.82 лв., общо 12 €');
