...
test('5. инвентарна книга: колоните по чл. 16, редовете, сборовете, диапазонът, екранирането', async () => {
  ...
  await soft('5e. диапазонът е обявен на листа', () => {
    ...
    /* v2.4.65 (кръг 42, находка Б7) — ТУК НАРОЧНО Е ПРОМЕНЕНО ОЧАКВАНЕТО.
       Дотук листът на ДИАПАЗОН изписваше своя сбор с думите „Фонд по
       инвентарната книга (без отчислените)“ — тоест обявяваше числото на
       диапазона за фонд на библиотеката. Този лист се прошнурова, номерира и
       заверява по чл. 26, ал. 2, а от v2.4.64 диалогът за печат предлага една
       година по подразбиране: обичайната разпечатка обявяваше фонда на една
       година за фонд на цялата библиотека. Самото ЧИСЛО не е променено (3) —
       променена е ДУМАТА, а веднага след нея листът дава и сбора по цялата
       книга. Печатът на ЦЯЛАТА книга остава дума по дума какъвто беше — виж
       1b и 5a по-горе, които не са пипани. */
    // Отпечатани са 1001, 1002, 1003 (по 1 екз.) и отчисленият 1004 → 3 неотчислени документа.
    assert.match(r, seq('Фонд в отпечатания диапазон (без отчислените):', '3', 'библиотечни документа на стойност', E.mny(25.5 + 20 + 12)));
    assert.ok(r.includes('Това НЕ е фондът на библиотеката'), 'листът казва чий е сборът');
    assert.match(r, new RegExp('по цялата инвентарна книга[^]*?неотчислените са\\s+' + copies + '\\s+библиотечни документа'),
      'и дава фонда на цялата книга (' + copies + ')');
  });
  closePreview();
...
test('16. ръбове: дарение без инвентирани документи, акт без комисия/заповед, картон без номер на карта', async () => {
  ...
  /* v2.4.65: към същото важи и съгласието на родител/настойник за дете до 14 г.
     (assertConsent в handlers/readers.js). Записва се с него и се маха веднага
     след това — заварената картотека съдържа точно такива редове и картонът им
     трябва да излиза смислено. */
  const r3 = ok(await h.api.readers.create({ name: 'Без Карта', category: 'дете до 14 г.', guarantor_name: 'Майка Ѝ', gdpr_consent: 1, parent_consent: 1 }), 'читател без карта');
  h.db.prepare('UPDATE readers SET gdpr_consent = 0, gdpr_consent_date = NULL, parent_consent = 0, parent_consent_date = NULL WHERE id = ?').run(r3);
  await h.go('readers');
