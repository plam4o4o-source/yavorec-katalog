...
test('изгледът предлага проект и отказва „Изтрий“ върху утвърден акт', () => {
  ...
  /* v2.4.61: към същото извикване се прибави и потвърждението за акт от
     ПРИКЛЮЧЕНА година (confirmClosedYear), затова изразът вече не е на един ред
     и не завършва веднага след `by`. Проверява се това, което твърдението
     винаги е значело: основанието и името стигат до главния процес. */
  assert.match(ACTS_VIEW, /revoke\(id, \{\s*reason: d\.reason, by: d\.by/,
