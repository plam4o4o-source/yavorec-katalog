...
test('пълно изтриване: всяка таблица с данни е празна, а решенията „остава“ са спазени', () => {
  ...
  /* ВРЪЗКАТА С ОНЛАЙН КАТАЛОГА ВЕЧЕ НЕ Е В ТОЗИ СПИСЪК (v2.4.65, кръг 42, А13).
     Дотук тестът заковаваше `catalog_folder`, `gh_user` и `gh_repo` като
     „настройки на инсталацията, които остават“. Проверката на кръга показа, че
     точно това прави ВРЕДЕН втория от трите повода, за които модулът е писан
     (втора библиотека получава компютъра на първата): новата библиотека
     публикуваше `katalog.json` в работното копие на СТАРАТА, таймерът го качваше
     в чуждия профил в GitHub и полето `library` във файла оставаше името на
     старата. Затова трите (плюс `gh_branch`) вече се нулират БЕЗУСЛОВНО и се
     проверяват поотделно точно под този цикъл.
     Самоличността (`org`, `lib_name`, `place`) ОСТАВА в списъка тук: тя се чисти
     само по изрична отметка „това е нова библиотека“ — виж отделния тест за нея
     в test/lichni-danni-v2465.test.js. */
  for (const f of ['org', 'lib_name', 'place', 'bulstat', 'director', 'librarian', 'work_days',
    'loan_days', 'max_books', 'theme',
    'pdp_salt', 'pdp_verifier', 'holidays_seeded']) {
    assert.equal(s[f], settingsBefore[f], 'настройка „' + f + '“ е загубена при изтриването');
  }
  assert.equal(s.catalog_folder, null, 'папката на онлайн каталога на старата библиотека трябва да се развърже');
  assert.equal(s.gh_user, null, 'профилът в GitHub на старата библиотека трябва да се развърже');
  assert.equal(s.gh_repo, null, 'хранилището в GitHub на старата библиотека трябва да се развърже');
...
test('прозорецът показва числата, оставащото, пътя на копието и предупреждението — преди потвърждението', async () => {
  ...
  try {
    ...
    /* Изтичането, което изтриването не поправя — И ВРЪЗКАТА, КОЯТО ПОПРАВЯ
       (v2.4.65, кръг 42, А13). Дотук надписът гласеше „Онлайн каталогът остава
       публикуван“ и това беше цялата истина: оставаше и връзката към чуждата
       папка и чуждия профил, тоест новата библиотека започваше да публикува
       фонда си там. От v2.4.65 връзката се прекъсва безусловно, затова блокът
       различава двете: какво ИЗЧЕЗВА (връзката) и какво ОСТАВА (публикуваният
       файл). */
    assert.match(text, /Онлайн каталогът: връзката се прекъсва, публикуваният файл остава/);
