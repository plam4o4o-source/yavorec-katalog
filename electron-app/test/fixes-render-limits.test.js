...
test('Екрани, които по устройство не растат, остават БЕЗ „Покажи още“', () => {
  /* Излишният бутон е влошаване: библиотекарят търси нещо, което го няма отвъд
     ...
     ДВА ЕКРАНА ИЗЛЯЗОХА ОТ ТОЗИ СПИСЪК във v2.4.64, защото измерването опроверга
     довода, с който бяха в него:

       periodicals.js— доводът беше, че разрезът по ГОДИНА (v2.4.61) вече решава
                       въпроса и „Покажи още“ би бил втори, по-слаб отговор. Само
                       че изборът на година има и опция „всички години“, която
                       минаваше ПОД този разрез: измерено 1 200 броя → 7 295 DOM
                       възела в едно тяло, в кутия с превъртане 240 px. Разрезът
                       по година остава предпочитаният път; прозоречният рендер е
                       таванът под него, за случая, в който библиотекарят поиска
                       всичко;
       mzs.js        — доводът беше „рядка операция, единици годишно“. Регистърът
                       обаче НЕ СЕ ЧИСТИ (за разлика от holds.js, откъдето
                       приключилите резервации излизат) — приключилите заявки
                       остават в него завинаги, тоест той расте, а не се върти.
                       Измерено: 500 заявки → 5 516 DOM възела, без нито един
                       таван по пътя. */
  const noWindow = ['holds.js', 'loans.js', 'deaccession-acts.js', 'audit.js'];
