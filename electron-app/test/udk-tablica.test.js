...
test('дървото следва класификацията: детето продължава кода на родителя', () => {
  ...
  walk(TREE, (n, parent) => {
    if (!parent) return;
    /* Изключение: код-диапазон („017/019“) започва низово със собствения си
       пръв член („017“) — той стои МЕЖДУ по дължина, но не е пропуснат родител,
       а брат на диапазона (виж build-udk.js: parentOf() нарочно го прескача).
       Диапазонът е равностоен на членовете си, не техен потомък. */
    const slash = n[0].indexOf('/');
    for (let i = parent[0].length + 1; i < n[0].length; i++) {
      if (slash >= 0 && i === slash) continue;
...
test('код-диапазон („017/019“) не се влага под собствения си пръв член', () => {
  /* Наивно най-дълго-съвпадение по низ би направило „017/019“ дете на „017“ —
     низово „017“ Е префикс на „017/019“, но смислово диапазонът ОБОБЩАВА
     017+018+019, не е подраздел на 017. Проверено срещу истинските данни: 102
     диапазона в изданието получаваха грешен родител преди поправката. */
  const findParent = (code) => {
    let found = null;
    walk(TREE, (n, parent) => { if (n[0] === code) found = parent ? parent[0] : null; });
    return found;
  };
  assert.equal(findParent('017/019'), '01', '„017/019“ трябва да е под „01“, не под „017“');
  assert.notEqual(findParent('017/019'), '017');
  assert.equal(findParent('271/279'), '27', '„271/279“ трябва да е под „27“, не под „271“');
  assert.equal(findParent('003.01/.09'), '003', '„003.01/.09“ трябва да е под „003“, не под „003.01“');

  // И общо: нито един код-диапазон не е дете на собствения си пръв член.
  let violations = 0;
  walk(TREE, (n, parent) => {
    if (!parent) return;
    const slash = n[0].indexOf('/');
    if (slash >= 0 && n[0].slice(0, slash) === parent[0]) violations++;
  });
  assert.equal(violations, 0, violations + ' диапазона все още са деца на собствения си пръв член');
});
