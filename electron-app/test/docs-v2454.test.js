...
test('дългото пояснение слиза под таблицата, вместо да разтяга едноколонна група', () => {
  ...
  const tpl = DNEVNIK_JS.slice(DNEVNIK_JS.indexOf('const tableHtml'), DNEVNIK_JS.indexOf('setPrintPage({ name: `Дневник'));
  /* От v2.4.65 dnevnikNotesHtml приема и ВТОРИ довод — допълнителни бележки за
     самия лист (находки В3 и В4): защо трите „Всичко“ на Раздел Б се разминават
     и кои дни са затворени по календара. Дотук това обяснение стоеше само в
     прозореца „Подробно за деня“ и изчезваше с него, а проверяващият виждаше три
     различни числа под един надпис „Всичко“ на подписан лист. Твърдението тук е
     непроменено — бележките пак слизат под всяка таблица. */
...
test('годишният отчет ползва същото разделяне, а не свое собствено', () => {
  ...
  // Вторият довод е от v2.4.65 — виж бележката в теста по-горе.
...
test('прозорецът показва предупреждението, а не съобщение за успех', () => {
  ...
  /* v2.4.57: условието се РАЗШИРИ, а не се смени. Дотук тук стоеше дословно
     `n ? 'warn' : 'ok'`, тоест проверката приковаваше СРИЧКАТА, а не правилото.
     Междувременно анулирането научи още едно нещо, което не се връща само —
     витрините в онлайн каталога, от които съставянето на акта е извадило
     документа (shelvesToRestore) — и то трябва да оцвети съобщението по същия
     начин: и то е недовършена ръчна работа, оставена на библиотекарката.
     Затова се проверява ПРАВИЛОТО („има ли какво да се доправи → жълто, иначе
     зелено“), а не точният му изказ. */
  assert.match(fn, /\?\s*'warn'\s*:\s*'ok'/,
    'жълто предупреждение, не зелено „готово“ — иначе никой не го чете');
  assert.match(fn, /shelvesToRestore/,
    'витрините, от които актът е извадил документа, също не се връщат сами и също се казват');
});
