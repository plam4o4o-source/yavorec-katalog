...
test('отчислен документ не се изтрива — минали години не се променят със задна дата', () => {
  ...
  /* Наличният документ продължава да МОЖЕ да се изтрие — отказът по-горе е само
     за отчисления. Одит v2.4.56: тук тестът очакваше изтриване още на първото
     натискане, а инв. № 10 е ВПИСАН ред в инвентарната книга и точно това беше
     дефектът — вписан ред си отиваше без въпрос и без следа. Старото очакване
     проверяваше вярното нещо („отказът важи за отчислените, не за всички“), но с
     грешна мярка: сега първото натискане предупреждава (и сочи акта за
     отчисляване по чл. 30 – 35), а второто до 2 минути изтрива. Важното за ТОЗИ
     тест — че наличният документ не пада под забраната по чл. 39 — се проверява
     на второто натискане. */
  const ok = db.prepare("INSERT INTO books (inv_number, title, status) VALUES (10, 'Сгрешена', 'наличен')").run().lastInsertRowid;
  const warn = ipcMain.invoke('books:delete', ok);
  assert.equal(warn.ok, false, 'вписаният ред първо предупреждава');
  assert.ok(!/е отчислен с акт/.test(warn.error), 'но не с отказа за вече отчислените');
  assert.match(warn.error, /още веднъж/, 'а с въпрос, който има изход');
  assert.equal(ipcMain.invoke('books:delete', ok).ok, true);
  assert.equal(db.prepare('SELECT COUNT(*) AS n FROM books').get().n, 1, 'останал е само отчисленият');
});
