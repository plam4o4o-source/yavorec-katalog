...
test('Й-фамилия през целия модул: знак „Й“ с числото от И, и без фалшива тревога в проверката', async () => {
  ...
  assert.equal(sug.letter, 'Й', 'готовата буква за екрана — не basis.charAt(0) директно');

  /* Проверка при прегледа: заварен ред с водещ препинателен знак пред
     фамилията („-Йовков“ от лош внос) — keyOf() го маха при самото търсене
     (mark/fromLetter излизат верни и оттук), но екранът четеше буквата за
     подсказката направо от basis.charAt(0) и щеше да покаже „- се търси от
     буква И“ вместо „Й се търси от буква И“. */
  const dirty = await s.ok('authorMark:suggest', { author: '-Йовков, Йордан' });
  assert.equal(dirty.mark, 'Й-77');
  assert.equal(dirty.fromLetter, 'И');
  assert.equal(dirty.letter, 'Й', 'буквата за подсказката е Й, не водещият знак на заварения запис');
