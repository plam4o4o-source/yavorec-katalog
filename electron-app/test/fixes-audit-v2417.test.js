...
test('формата на книгата не предлага „отчислен“ като избор', async () => {
  ...
  /* v2.4.56: добавен е и „изгубен“ — статусът, който слага приключването на
     заемане като изгубен документ. Без него библиотекарят вижда документа като
     изгубен, но не може да го върне на „наличен“, ако книгата все пак се намери.
     „отчислен“ остава извън списъка по същата причина, както досега: то е
     формален акт по чл. 30 – 39, а не избор от падащо меню. */
  assert.deepEqual(opts, ['наличен', 'липсващ', 'изгубен', 'за реставрация']);
});
...
test('CSV на фонда носи бройките и оставя следа в одита', async () => {
  ...
  assert.equal(row[h.indexOf('Обща стойност (€)')], '"12.00"', '3 × 4.00 € — иначе сборът в Excel лъже');
  /* Левът е СПРАВОЧНА колонка до еврото (v2.4.51) и се смята от записаната
     стойност, не обратно: 4.00 € × 1.95583 = 7.82 лв. */
  assert.equal(row[h.indexOf('Цена (€)')], '"4.00"');
  assert.equal(row[h.indexOf('Цена (лв.)')], '"7.82"');
