...
test('миграция 15 преобразува ТОЧНО деветте парични колони', () => {
  /* v2.4.61: тестът се вързваше за ТОЧНАТА текуща версия на схемата
     (`= 15;`) и падаше при всяка следваща миграция — а той не проверява коя е
     последната версия, а че МИГРАЦИЯ 15 преобразува точно деветте парични
     колони. Затова сега се иска само схемата да е стигнала поне до 15 (преди
     това миграцията още не е писана) и самата миграция 15 да съществува. */
  const схемаВерсия = Number(/const CURRENT_SCHEMA_VERSION = (\d+);/.exec(MAIN)[1]);
  assert.ok(схемаВерсия >= 15, 'схемата трябва да е поне на версия 15, а е ' + схемаВерсия);
  assert.match(MAIN, /\{ version: 15, run: \(\) => \{/);
...
test('трите REAL колони, които НЕ са пари, остават непипнати', () => {
  ...
  /* ПАРИЧНИ колони, ВЪВЕДЕНИ СЛЕД миграция 15 — присъствието им тук НЕ е пропуск.
     Тестът е прав да ги забележи (за това е написан: „нова парична колона утре ще
     провали този тест, вместо да остане в лева мълчаливо“), но заключението за тях
     е обратното на заключението за старите колони. Миграция 15 преобразува в евро
     СЪЩЕСТВУВАЩИ редове в бази отпреди 2026 г.; таблицата `periodical_volumes`
     (годишните комплекти на периодиката, v2.4.56) не съществува в нито една такава
     база — тя се създава СЛЕД миграцията, а първата ѝ стойност се записва вече в
     евро от handlers/periodicals.js, като сбор от `periodical_issues.price`, която
     миграция 15 вече е преобразувала. Добавянето ѝ към преобразуваните не е просто
     излишно — то би разделило сумата на 1.95583 ВТОРИ път и би занижило снимката
     на стойността на комплекта. */
  const следМиграцията = ['periodical_volumes.issue_sum',
    /* Същото и за обезщетението за изгубен документ (v2.4.56): `loans.lost_amount`
       се ражда в евро — смята се от `books.price`, която миграция 15 вече е
       преобразувала — а `settings.lost_price_multiplier` изобщо не е пари, а
       кратност (безразмерно число). `settings.lost_fallback_amount` е сума, но се
       въвежда от библиотекаря СЛЕД въвеждането на еврото. И трите в миграцията
       биха разделили стойността на 1.95583 втори път. */
    'loans.lost_amount', 'settings.lost_price_multiplier', 'settings.lost_fallback_amount',
    /* И `loans.deaccession_fine` (v2.4.61): колко от забавата по едно заемане е
       начислил актът за отчисляване по чл. 30, т. 5 — за да върне анулирането
       точно толкова. Смята се от `settings.fine_per_day`, която миграция 15 вече
       е преобразувала, тоест ражда се в евро; колоната не съществува в нито една
       база отпреди миграцията. В миграцията би разделила сумата втори път. */
    'loans.deaccession_fine'];
  const останали = непипнати.filter(c => !отделно.includes(c) && !следМиграцията.includes(c));
...
test('колоните „Стойност“ в документите по Наредба № 3 са в евро', () => {
  for (const файл of ['acquisitions.js', 'deaccession-acts.js', 'kdbf.js']) {
    const src = fs.readFileSync(path.join(APP_DIR, 'src', 'views', файл), 'utf8');
    /* v2.4.61: заглавието на колоната вече е „Стойност, € / лв.“ — клетките под
       него се пълнят от mny(), който печата и двете валути („4.00 € / 7.82 лв.“),
       и одитът на разпечатките намери, че едно заглавие само с € описва невярно
       съдържанието на собствената си колона. Проверката остава същата по смисъл:
       еврото трябва да е ПЪРВО (основната валута по регламента), а заглавие само
       в лева е забранено. */
    assert.match(src, /<th>Стойност, €(?: \/ лв\.)?<\/th>/, файл);
...
test('напомнителните писма не делят повторно по курса', () => {
  ...
  /* v2.4.65: трите суми в напомнянето (начислено, платено, остава) се изписват
     през ЕДИН помощник `eur()`, вместо низът да се преписва на четири места —
     иначе същото делене можеше да се върне в едно от тях, без проверката да го
     забележи. Проверява се самият помощник: евро от записаното число, левът
     справочно в скоби, умножение по курса, никакво делене. */
  assert.match(src, /const eur = \(n\) => `\$\{n\.toFixed\(2\)\} € \(\$\{\(n \* EUR_RATE\)\.toFixed\(2\)\} лв\.\)`/);
  assert.match(src, /fine: fine > 0 \? eur\(fine\) : ''/);
