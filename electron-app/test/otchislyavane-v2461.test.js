...
test('ПЛАТЕНОТО обезщетение не се трие при анулиране — редът остава и следата го казва', () => {
  ...
  /* v2.4.61 (втора поправка): актът е начислил ДВЕ неща — 4.00 € забава и 37.50 €
     за документа. Плащането покрива най-старото задължение първо (същото правило,
     по което се води всяка сметка), тоест 20.00 € покриват изцяло забавата и 16.00
     € от стойността. Нито едно от двете не се трие: по всяко от тях вече е
     плащано, а платеното е факт. */
  assert.equal(db.prepare("SELECT COUNT(*) AS n FROM account_lines WHERE reader_id = ? AND kind = 'начисление'").get(r).n, 2,
    'начисленията, по които е плащано, остават');
  assert.equal(info.keptCharges.length, 2);
  assert.equal(info.keptCharges.find(c => c.kind === 'забава').covered, 4, 'забавата е покрита изцяло');
  assert.equal(info.keptCharges.find(c => c.kind !== 'забава').covered, 16, 'останалите 16.00 € отиват по стойността');
  assert.match(lastAudit(audit, 'Анулиране на акт').detail,
    /начисленията ОСТАВАТ в сметката на Петрова, Мария \(инв\. № 6, забава, начислено 4\.00 €, събрано 4\.00 €\); Петрова, Мария \(инв\. № 6, за невърнат документ, начислено 37\.50 €, събрано 16\.00 €\)/);
});
