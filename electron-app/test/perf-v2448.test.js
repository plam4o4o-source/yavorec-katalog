...
/* v2.4.48 — тридесет и пети кръг: производителност.
   ...
   ПРАГОВЕТЕ ПО ВРЕМЕ СА МАХНАТИ (v2.4.63). Коментарът тук ги описваше още при
   прегледа на v2.4.49: „те НЕ ловят връщането назад… със стария код
   приключването отнема ~158 ms при праг 250 ms, тоест тестът минава и с
   дефекта; при натоварена машина същият праг пада и при поправения код“.
   Тоест прагът няма разграничителна сила в НИТО ЕДНА посока — минава с дефекта
   и пада без него. Оставаше като „груб предпазител“, а всъщност беше
   единственият източник на трепкащи падания в цялата поредица.

   КАК СЕ ХВАНА. На 21.09.2026 задачата за UTC падна с „# pass 1900 · # fail 1“
   — същият отпечатък като необяснимото падане на PR #166. Новото резюме „Кой
   тест падна“ в ci.yml го назова веднага: ТОЗИ тест, „приключването отне
   316 ms“ при праг 250. На същата машина цялата поредица отне 574 s вместо
   212 s, тоест runner-ът е бил ~2,7 пъти по-бавен. Прагът не е измерил
   програмата, а натоварването на чужда машина.

   ЧИСЛОТО НЕ СЕ ГУБИ: измерва се и се отпечатва като диагностичен ред на
   пускането, така че забавяне пак се вижда от човек — просто не проваля
   поредицата заради машина, върху която проектът няма власт.
   Истинската преграда срещу връщането назад е и беше правилото за db.prepare()
   в обхождането (последният тест в този файл) — то чете текста на кода и не
   зависи от часовник.
...
test('приключената проверка отбелязва СЪЩИТЕ документи като липсващи — и то с една заявка, не с една на документ', async (t) => {
  await boot();
  ...
  /* Числото се ЗАПИСВА, не се твърди. Старият начин (db.prepare вътре в
     обхождането + по едно UPDATE на ред) отнемаше ~200 ms при тези 4 000
     документа, поправеният — под 60 ms; праг между двете би минал и с дефекта,
     а на натоварена машина пада и без него (виж бележката в началото на файла).
     Връщането назад се лови от правилото за db.prepare() в края на файла. */
  t.diagnostic('приключване на инвентаризация при ' + BOOKS + ' документа: ' + ms + ' ms');
});
...
test('акт за много документи се съставя и анулира вярно — и без по една сглобена заявка на документ', async (t) => {
  await boot();
  db.prepare("UPDATE books SET status='наличен' WHERE status='липсващ'").run();
  ...
  /* Пак записване, не твърдение — и тук разликата е твърде малка за праг:
     измерено при 2 000 документа преди поправката 223 ms за съставянето и
     212 ms за анулирането, след нея 126 ms и 45 ms. Праг между 126 и 223 не
     оцелява на чужда натоварена машина. */
  t.diagnostic('акт за ' + ids.length + ' документа: съставяне ' + msCreate
    + ' ms, анулиране ' + msRevoke + ' ms');
});
