...
test('19. всеки документ, издаден от src/views/, е покрит от някой от двата регистъра', () => {
  ...
  /* И ЧИСЛОТО В README ТРЪГВА ОТТУК.
     =====================================================================
     README обещаваше „~14 печатни документа“, а официалният сайт — „над 20“;
     истинските бяха 24. Разминаването е тихо: никой не брои функции, за да
     провери едно изречение, а то е първото, което чете човек, който още не е
     отворил програмата. Регистърът по-горе вече знае точния брой — остава
     числото в README да се сверява с него, вместо да се поддържа на ръка.
     Сайтът се пише отделно (той е в друго хранилище) и взима числата оттук —
     виж „Съдържание, взето от продуктовото хранилище“ в неговия README. */
  const readme = fs.readFileSync(path.join(__dirname, '..', '..', 'README.md'), 'utf8');
  const claim = /(\d+)\s+печатни документа/.exec(readme);
  assert.ok(claim, 'README.md вече не казва колко са печатните документи — ако редът е преместен, '
    + 'обновете и този пазач');
  assert.equal(Number(claim[1]), PRINTED.size,
    'README.md обещава ' + claim[1] + ' печатни документа, а регистърът изброява ' + PRINTED.size);
  const exp = /(\d+)\s+изнасяния/.exec(readme);
  assert.ok(exp, 'README.md вече не казва колко са изнасянията на файл');
  assert.equal(Number(exp[1]), EXPORTED.size,
    'README.md обещава ' + exp[1] + ' изнасяния, а регистърът изброява ' + EXPORTED.size);
