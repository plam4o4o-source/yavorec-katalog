...
test('отчисляването отказва/анулира активна резервация ("чака") на отчисления документ и го отбелязва в одитната следа (одит #10)', async () => {
  ...
  // Съгласуване по число (v2.4.25): „отказана 1 резервация“.
  assert.match(entry.detail, /отказана\s+1\s+резервация/, 'одитната следа трябва да спомене отказаните резервации');
});
