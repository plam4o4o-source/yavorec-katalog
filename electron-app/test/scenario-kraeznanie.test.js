...
test('4. Персоналии: формата, дати, търсене, картон', async () => {
  ...
  /* ПРОМЕНЕНО ПОВЕДЕНИЕ (v2.4.65, находка Б20). Дотук редът идваше от
     `ORDER BY p.name`, тоест от ДВОИЧНОТО сравнение на SQLite — и точно то
     слагаше латиницата пред кирилицата (латинските букви са с по-малки кодове).
     Същото двоично сравнение обаче нареждаше „Ѝлчев“ ПРЕДИ „Ангелов“, а
     „вълчев, стефан“ СЛЕД „Янков“ — тоест указателят не беше азбучен и един и
     същ човек, вписан с малка буква, се озоваваше в края на картотеката.
     Сега persons:list подрежда с localeCompare(…, 'bg') — образецът от летописа.
     В българската подредба записите на кирилица вървят първи, а тези на
     латиница накрая, като отделна поредица — така се води и азбучният каталог
     на хартия. */
  assert.deepEqual(list.map(x => x.name), ['Петров, Георги Иванов', 'Стоянов, Иван', "O'Neil, Джон"],
    'по българската азбука: кирилицата, а след нея латиницата като отделна поредица');
