...
test('5. ежедневник — 300+ броя, 31.12/01.01 в различни години, кардексът с 300+ реда', async () => {
  ...
  /* ПРОМЕНЕН НАДПИС (v2.4.65, находка Б14). Дотук знаменателят беше целият
     кардекс (302 броя за двете години), а числителят — показаното за ИЗБРАНАТА
     година: „Показани 1 от 302 броя — 2026 г.“, докато бутонът отдолу брои по
     съвсем друга бройка. Сега надписът брои в избрания обхват (както бутонът), а
     целият кардекс се назовава отделно, накрая. */
  assert.match(h.text('#perIssueCount'), rx('Показани 1 от 1 брой — ' + Y + ' г.'));
  assert.match(h.text('#perIssueCount'), rx('целият кардекс: 302 броя'));
  ...
  /* Двете числа вече идват от един и същ въпрос: 300 от 301 в надписа и „1 от
     общо 301“ на бутона (v2.4.65, находка Б14). Дотук надписът казваше 302. */
  assert.match(h.text('#perIssueCount'), rx('Показани 300 от 301 броя — ' + Y1 + ' г.'));
...
test('13. отчисляване на годишния комплект с акт; кардексът, КДБФ и каталогът; анулиране връща всичко', async () => {
  ...
  /* ПРОМЕНЕНО ПОВЕДЕНИЕ (v2.4.65, находка Б13). Дотук редът на ОТЧИСЛЕНАТА
     година нямаше нито един бутон — и точно това беше дефектът: обработчикът от
     v2.4.61 (находка 16) нарочно допуска заместващ комплект след отчисляване
     (дарен или откупен том), но екранът не оставяше път дотам и документът
     нямаше как да влезе във фонда. Сега редът предлага вписване на ЗАМЕСТВАЩ
     том — с пояснение до бутона, — а отказът за ЖИВ комплект си остава
     (проверява се в теста за повторно инвентиране по-долу). */
  const deaccBtns = Array.from(volumeRow(Y).querySelectorAll('button')).map(b => b.textContent.trim());
  assert.deepEqual(deaccBtns, ['Инвентирай годишния комплект'],
    'отчислената година предлага вписване на заместващ комплект');
  assert.match(h.text(volumeRow(Y)), /заместващ том — предишният е отчислен/,
    'до бутона се казва, че става дума за заместващ том, а не за повторно вписване на същия');
  await h.clickButton('Затвори', '#modal footer');
  await h.settle();
