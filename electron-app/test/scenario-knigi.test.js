...
/* ==================================================================
   ...
   ================================================================== */
test('8. УДК от таблицата, авторски знак без таблица, ISBN без интернет — нищо не гърми', async () => {
  await openNewBookForm();
  ...
  // Авторски знак: няма внесена таблица → ясно съобщение, полето остава празно.
  let n = h.toasts.length;
  await h.clickButton('Предложи', '#bookF');
  await h.settle();
  assert.match(h.text('#amHint'), /Няма как да предложа/);
  assert.ok(h.toastsSince(n).some(t => t.type === 'err' && /Няма внесена таблица за авторски знак/.test(t.msg)));
  assert.equal(h.$('#bookF [name=author_mark]').value, '');
  // ISBN: няма мрежа.
