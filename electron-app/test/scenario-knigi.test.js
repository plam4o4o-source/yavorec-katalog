...
test('20. онлайн каталог: katalog.json след вписване, след редакция и при недостъпна папка', async () => {
  ...
  /* v2.4.65 (кръг 42, находка Г1) — ТУК НАРОЧНО Е ПРОМЕНЕНО ОЧАКВАНЕТО.
     Дотук тестът заковаваше „katalog.json е записан ВЕДНАГА след вписването“.
     Измерено върху истинска база (15 000 документа, katalog.json 4,82 МБ): това
     струваше 133 ms на книга и 5 132 ms за партида от 40 книги, защото всяко
     вписване пренаписваше целия файл — и при папка на мрежов дял го пренасяше
     цял по мрежата. Намерението от v2.4.57 (постъплението да не се проваля
     мълчаливо) е запазено дословно: вписването прави евтина проба дали папката
     приема запис и предупреждава по същия начин при изключен диск — виж
     проверката за недостъпна папка по-долу в същия тест, която е непроменена.
     Самият запис вече минава по нормалния отложен път за промени по фонда
     (4 s), точно както редакцията малко по-надолу. Затова тук се вика „Ръчен
     запис“ — това е дословно работата, която върши таймерът от 4 секунди. */
  assert.ok(!fs.existsSync(file), 'вписването вече не пренаписва целия katalog.json на всяка книга');
  ok(await h.api.catalog.writeNow(), 'отложеният запис на каталога');
  assert.ok(fs.existsSync(file), 'katalog.json е записан, щом отложеният запис мине');
