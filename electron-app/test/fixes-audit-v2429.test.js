...
test('dnevnik:suggest — романът е „Художествена литература“, детската — „Детска художествена“, критиката остава в 82', async () => {
  await boot();
  /* v2.4.65: читател от категория „дете до 14 г.“ вече не се записва без
     отбелязано съгласие на родител/настойник — съгласието за обработване на
     лични данни на дете под 14 години се дава от него, не от детето (виж
     assertConsent в handlers/readers.js). Добавя се тук, защото проверяваното е
     предложението на Дневника по УДК, а не правилото за съгласието. */
  const kid = await reader({ category: 'дете до 14 г.', parent_consent: 1 });
...
test('periodicalIssues:add отказва невалидна дата, празен номер и цена, която не е число; delete вписва в следата', async () => {
  ...
  /* Очакването беше ТОЧЕН низ: 'Родна реч — бр. 2 от 2026-02-10'. Заковаваше не
     смисъла („изтриването оставя следа“), а буквата — и с това заключваше следата
     в най-бедния ѝ вид. Дотук тя не казваше нито годината (по година се подвързва
     годишният комплект и по година се отчита фондът — точно това се пита при
     проверка, а след изтриването редът го няма и годината не може да се извади
     отникъде), нито цената (тя е стойност, извадена от фонда), нито дали броят е
     част от вече инвентиран комплект с инвентарен номер. Затова тук се проверяват
     ФАКТИТЕ, които следата трябва да носи, а не конкретната им подредба —
     обогатяване на съобщението вече не се брои за регресия, а обедняването му
     пада. */
  const a = lastAudit();
  assert.equal(a.action, 'Изтрит брой');
  for (const част of ['Родна реч', 'бр. 2', '2026-02-10', '2026 г.', '1.50 €']) {
    assert.ok(a.detail.includes(част), 'следата не носи „' + част + '“: ' + a.detail);
  }
});
...
test('Читатели: колона „Заети“ с „!“ при просрочие, бутоните на реда са в .rowActs, таблицата е readersTable', async () => {
  ...
  /* Одит v2.4.56 — проверката тук ИЗИСКВАШЕ дефекта. Дотук редът гласеше
     `assert.match(css, /table\.ledger\.readersTable td:first-child\{white-space:nowrap\}/)`,
     тоест заковаваше точно правилото, заради което таблицата с читателите
     излизаше извън екрана: колоната с ИМЕТО не се пренасяше, растеше със
     съдържанието си и избутваше копчетата „Заемане“/„Редакция“ надясно извън
     прозореца (измерено при 1366×768: таблица 1082 px в поле 1034 px, „⋯“ на
     1375 px при екран 1366 px). Намерението на първоначалния ред е било
     „заглавията на колоните и имената да не се цепят на срички“ — то остава в
     сила за <th> (редът по-горе), но за самото име вече е обратното: името се
     пренася, за да не разтяга таблицата. Затова проверката е сменена, а не
     изтрита — тя продължава да пази, че за колоната с името има ИЗРИЧНО
     правило, само че вече правилното. Виж коментара при това правило в
     src/style.css. */
  assert.match(css, /table\.ledger\.readersTable td:first-child\{white-space:normal; overflow-wrap:break-word; max-width:\d+px\}/,
    'колоната с името се пренася и има таван на ширината — иначе дългите имена изнасят действията на реда извън екрана');
  /* Колоната с действията е залепена за десния ръб на превъртащото поле, за да
     остане достъпна и когато таблицата пак се окаже по-широка от екрана. */
  assert.match(css, /table\.ledger td\.actsCell[^{]*\{[^}]*position:sticky[^}]*right:0/,
    'действията на реда остават на екрана при хоризонтално превъртане');
