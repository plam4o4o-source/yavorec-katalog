...
test('картата остава вярна и за старите три състояния (заключено/изключено/криптирано)', async () => {
  ...
  /* v2.4.65 (кръг 42, находка В11): думата е „автоматични“, а не „дневни“.
     Числото брои И МЕЖДИННИТЕ копия (auto-ГГГГ-ММ-ДД-ЧЧММ.db — от таймера на
     всеки 3 часа и при всяко затваряне на програмата), а те не са дневни и не
     са „отпреди включването на защитата“: правят се и след минути. Надписът
     „некриптирани дневни копия“ караше библиотекарката да брои файловете в
     папката, да намери повече от обявеното с днешни дати и да реши, че екранът
     бърка. Числото не се променя — сменя се само името, с което се назовава. */
  assert.match(html, /30<\/b>\s*некриптирани автоматични копия/, 'експозицията се показва с число');
