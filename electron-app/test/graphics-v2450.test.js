...
test('надписите за състояние (.badge) са 12 px, не 11', () => {
  /* v2.4.54: ЕДНО правило, не две. Тук се очакваха две, защото .badge наистина
     се задаваше два пъти — веднъж без фон и цвят на текста, и още веднъж, пълно,
     седемдесет реда по-надолу. Тестът закова дублирането, вместо да го улови:
     побеждаваше второто правило, тоест първото беше мъртъв код, който само
     чакаше някой да размени реда им. Останало е пълното. */
  const rules = CSS.match(/^\.badge\{[^}]*\}$/gm) || [];
  assert.equal(rules.length, 1, '.badge се задава на ЕДНО място');
  assert.match(rules[0], /font-size:12px/, rules[0]);
  assert.match(rules[0], /background:var\(--paper3\)/, 'празният .badge трябва да има фон');
  assert.match(rules[0], /color:var\(--ink2\)/, '… и цвят на текста');
});
