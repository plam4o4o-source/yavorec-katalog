...
test('13. дневник — редакция на клетка се записва, сборовете се преизчисляват, стойността се пази', async () => {
  ...
  /* Очакваното допуска и белега „·затв.“ след числото на деня (v2.4.65, находка В3):
     Дневникът вече пита календара и отбелязва дните, в които библиотеката е
     затворена — дотук такъв ден изглеждаше като всеки друг и вписаната в него
     работа минаваше без дума. Фикстурата на този тест затваря днешния ден, затова
     белегът се появява именно тук; самото твърдение (че колоната „Всичко“ по
     възраст се преизчислява на 3) остава непроменено. */
  const tr = cell.closest('tr');
  assert.match(h.text(tr), new RegExp('^' + new Date(T).getUTCDate() + '( ·затв\\.)? 3 0 0 0$'),
    'колоната „Всичко“ по възраст не е преизчислена');
