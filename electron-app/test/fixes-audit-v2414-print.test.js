...
test('екранът за съставяне на акта показва СЪЩИТЕ числа, които ще влязат в акта', async () => {
  /* Това е находката, заради която файлът съществува: библиотекарят сглобяваше
     ...
     v2.4.62: изходът вече е друг и по-правилен. Старите записи с по 3 екземпляра
     под един номер не влизат в акта наведнъж — актът отчислява само екземпляра с
     номера, записан в него (всеки документ има свой инвентарен номер в
     инвентарната книга, чл. 16). При сканиране екранът разделя записа (с
     потвърждение) и в списъка влиза ЕДИН екземпляр на номер. Твърдението на
     теста остава същото по смисъл — екранът и актът казват едно и също число —
     но числото вече е 3 документа за 30 лв., защото точно толкова излизат от
     фонда; останалите шест екземпляра остават в него с нови номера. */
  const splits = [];
  const dom = buildDom({
    'settings.get': {},
    ...
    'books.splitCopies': (args) => {
      splits.push(Number(args[0]));
      const base = 900 + splits.length * 2;
      return { created: [base, base + 1], createdIds: [base, base + 1], inv_number: Number(args[0]) };
    }
  });
  const { window } = dom;
  await settle();
  ...
  assert.deepEqual(splits, [101, 102, 103], 'всеки стар запис се разделя, преди да влезе в акта');
  const list = window.document.querySelector('#actList').textContent.replace(/\s+/g, ' ');
  assert.match(list, /ОБЩО 3 документа/, 'в акта влиза по един екземпляр на номер: ' + list);
  assert.ok(list.includes('30.00'), 'сборът е Σ(цена × бройка) = 3 × 1 × 10');
  assert.ok(!/ОБЩО 9 документа/.test(list), 'трите записа не бива да отчисляват всичките си екземпляри');
  assert.ok(!/заглавия/.test(list), 'всеки ред е един документ — пояснение „N заглавия“ няма какво да пояснява');
});
