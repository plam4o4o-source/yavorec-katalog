...
- **Преглед преди печат** на всички 24 печатни документа (плюс 7 изнасяния
  на файл: CSV, UNIMARC/MARCXML, Dublin Core)
...
- Интернет е нужен само за: автоматично обновяване, търсене по ISBN,
  публикуване на онлайн каталога и анонимното отчитане на инсталацията
  (случаен номер + версия + вид Windows, без никакви лични данни — виж
  [ARCHITECTURE.md](docs/ARCHITECTURE.md#анонимно-отчитане-на-инсталациите-install-reportjs-v2456))
  — всичко останало работи изцяло офлайн
...
кода и 739 общи определителя — на Универсалната десетична класификация,
българска версия, хармонизирана с официалното национално издание на УДК от
2017 г. Разпространява се със самата програма, с посочване на източника и при
лиценза CC BY-SA 3.0 (пълните подробности стоят в `src/udk.json`/`src/udk.js`,
където се редактират). Пълната схема на УДК (над 70 000 индекса) остава чужда
собственост и НЕ е тук. Кодовете в `additions` на същия файл не идват оттам —
те са от собствената съкратена таблица на проекта (GPL-3.0) и са отбелязани
отделно.

**Авторската таблица** (двузначните числа за авторския знак) **идва с
програмата** от v2.4.63 — `electron-app/db/author-table.js`, зарежда се при
първото пускане. Библиотека, която работи по свое издание на авторските
таблици, внася своя файл от „Настройки → Фонд → Авторски знак“; внесеното
застава върху вграденото и не се подменя при обновяване.
