...
async function main() {
  ...
  /* --- Сценарий (з) — v2.4.27 (поправен при прегледа, кръг 17): кавичка в
     споделен адрес (#zapis=…) ---
     ...
     (CSS.escape).
     ВАЖНО — за да мине през querySelector(), кавичката трябва да е в стойност,
     която REALLY СЪВПАДА с inv на съществуващ запис: revealShared() има
     `if(idx<0)return;` веднага след търсенето по `String(R[i].inv)===target` и
     излиза, преди изобщо да стигне до селектора, ако не намери съвпадение.
     Първата версия на този сценарий пращаше `#zapis=%22` срещу запис с inv:1 —
     идентификаторите никога не съвпадат, `idx` си остава -1, и querySelector()
     изобщо не се извиква. Той минаваше еднакво и със, и без CSS.escape (проверено
     с revert-and-retest на v2.4.27 — без него): не проверяваше нищо. Истинският
     (макар и рядък) път е повреден запис в самия katalog.json, чийто inv вече
     съдържа кавичка, и връзка към точно него. */
  {
    var r = await runScenario('z: quote in #zapis= share link (matches a real inv)', {
      primary: 'ok',
      primaryBody: { generated: '2026-08-21', items: [{ inv: '1"', a: 'X', t: 'Y', v: 'книга' }] },
      localStorage: {},
      hash: '#zapis=' + encodeURIComponent('1"')
    });
